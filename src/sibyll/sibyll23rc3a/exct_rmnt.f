...
C..   Sample masses
      SELECT CASE(KRMNT)
      CASE(1)
         XM2MAX = MIN(XSFRAC*S,XMFRAC*AM2(ABS(KB)))
...
c     remnant mass only increased by multiple interactions..
            Xmsqmin = MAX(AM2(ABS(KB)),XM2)
         ENDIF
c     sample beam mass
         XMB2 = XM2DIS(XMSQMIN,XM2MAX,ALPHA)
         IF(NDEBUG.gt.2)
     &        WRITE(LUN,*)' EXCT_RMNT: XM2min,XM2max,XM2:',
     &        Xmsqmin,XM2max,XMB2
c     check if resonance or massive hadron has to be formed
      ...
      CASE(2)
c     target side mass
         XM2MAX = MIN(XSFRAC*S,XMFRAC*AM2(ABS(KT(JW))))
         XM2MAX = MAX(XM2MAX,ONE)
...
c     m2_max should be smaller than m2_min
         ...
         IF(JDIF(jw).gt.1)THEN
            deltae = ee-am(13)
            XMMIN=max(XM2MIN(1),(am(abs(l0))+am(7)+0.02D0)**2)
         else
            deltae = EE-AM(ABS(L0))
            xmmin=max(XM2MIN(K),(am(abs(l0))+am(7)+0.02D0)**2)
         endif
