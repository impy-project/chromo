 ...
 * Generation date: Mon Aug 15 13:17:07 2022
...
/*frompyobj*/
  ...
  if (capi_sigdif_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(_sib21_error,"failed in converting hidden `sigdif' of _sib21.sib_sigma_hp to C/Fortran array" );
  } else {
...
/*frompyobj*/
  ...
  if (capi_p0_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(_sib21_error,"failed in converting 2nd argument `p0' of _sib21.decpar to C/Fortran array" );
  } else {
  ...
  if (capi_ll_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(_sib21_error,"failed in converting 4th argument `ll' of _sib21.decpar to C/Fortran array" );
  } else {
  ...
  if (capi_p_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(_sib21_error,"failed in converting 5th argument `p' of _sib21.decpar to C/Fortran array" );
  } else {
...
#else
#define RETVAL
PyMODINIT_FUNC init_sib21(void) {
#endif
  int i;
  PyObject *m,*d, *s, *tmp;
#if PY_VERSION_HEX >= 0x03000000
...
#else
  m = _sib21_module = Py_InitModule("_sib21", f2py_module_methods);
#endif
  Py_TYPE(&PyFortran_Type) = &PyType_Type;
  import_array();
  if (PyErr_Occurred())
    {PyErr_SetString(PyExc_ImportError, "can't initialize module _sib21 (failed to import numpy)"); return RETVAL;}
