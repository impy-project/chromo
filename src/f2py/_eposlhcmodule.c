 ...
 * Generation date: Sun Nov 27 10:50:56 2022
...
extern void F_FUNC(initepos,INITEPOS)(float*,float*,string,int*,int*,int*,int*,size_t);
...
initepos(seed,emax,datpath,lpath,ifram,idbg,iou)\n\nWrapper for ``initepos``.\
\n\nParameters\n----------\n"
"seed : input float\n"
"emax : input float\n"
...
static PyObject *f2py_rout__eposlhc_initepos(const PyObject *capi_self,
                           PyObject *capi_args,
                           PyObject *capi_keywds,
                           void (*f2py_func)(float*,float*,string,int*,int*,int*,int*,size_t)) {
    PyObject * volatile capi_buildvalue = NULL;
    volatile int f2py_success = 1;
/*decl*/
    ...
    static char *capi_kwlist[] = {"seed","emax","datpath","lpath","ifram","idbg","iou",NULL};
...
#endif
    if (!PyArg_ParseTupleAndKeywords(capi_args,capi_keywds,\
        "OOOOOOO|:_eposlhc.initepos",\
        capi_kwlist,&seed_capi,&emax_capi,&datpath_capi,&lpath_capi,&ifram_capi,&idbg_capi,&iou_capi))
        return NULL;
/*frompyobj*/
    /* Processing variable seed */
        f2py_success = float_from_pyobj(&seed,seed_capi,"_eposlhc.initepos() 1st argument (seed) can't be converted to float");
    if (f2py_success) {
    /* Processing variable emax */
        f2py_success = float_from_pyobj(&emax,emax_capi,"_eposlhc.initepos() 2nd argument (emax) can't be converted to float");
    if (f2py_success) {
    /* Processing variable datpath */
    slen(datpath) = -1;
    f2py_success = string_from_pyobj(&datpath,&slen(datpath),"",datpath_capi,"string_from_pyobj failed in converting 3rd argument`datpath' of _eposlhc.initepos to C string");
    if (f2py_success) {
        STRINGPADN(datpath, slen(datpath), '\0', ' ');
    /* Processing variable lpath */
        f2py_success = int_from_pyobj(&lpath,lpath_capi,"_eposlhc.initepos() 4th argument (lpath) can't be converted to int");
    if (f2py_success) {
    /* Processing variable ifram */
        f2py_success = int_from_pyobj(&ifram,ifram_capi,"_eposlhc.initepos() 5th argument (ifram) can't be converted to int");
    if (f2py_success) {
    /* Processing variable idbg */
        f2py_success = int_from_pyobj(&idbg,idbg_capi,"_eposlhc.initepos() 6th argument (idbg) can't be converted to int");
    if (f2py_success) {
    /* Processing variable iou */
        f2py_success = int_from_pyobj(&iou,iou_capi,"_eposlhc.initepos() 7th argument (iou) can't be converted to int");
    if (f2py_success) {
/*end of frompyobj*/
#ifdef F2PY_REPORT_ATEXIT
f2py_start_call_clock();
#endif
/*callfortranroutine*/
                (*f2py_func)(&seed,&emax,datpath,&lpath,&ifram,&idbg,&iou,slen(datpath));
if (PyErr_Occurred())
  f2py_success = 0;
#ifdef F2PY_REPORT_ATEXIT
f2py_stop_call_clock();
