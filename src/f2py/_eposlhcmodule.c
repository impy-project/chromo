 ...
 * Generation date: Mon Aug 15 13:17:09 2022
...
#else
#define RETVAL
PyMODINIT_FUNC init_eposlhc(void) {
#endif
  int i;
  PyObject *m,*d, *s, *tmp;
#if PY_VERSION_HEX >= 0x03000000
...
#else
  m = _eposlhc_module = Py_InitModule("_eposlhc", f2py_module_methods);
#endif
  Py_TYPE(&PyFortran_Type) = &PyType_Type;
  import_array();
  if (PyErr_Occurred())
    {PyErr_SetString(PyExc_ImportError, "can't initialize module _eposlhc (failed to import numpy)"); return RETVAL;}
...
#else
      s = PyString_FromString("getcharge");
#endif
      PyObject_SetAttrString(o,"__name__", s);
      Py_DECREF(s);
    }
