...
/*frompyobj*/
  ...
  if (capi_ep0_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(_qgs01_error,"failed in converting 1st argument `ep0' of _qgs01.xxreg to C/Fortran array" );
  } else {
...
#else
#define RETVAL
PyMODINIT_FUNC init_qgs01(void) {
#endif
  int i;
  PyObject *m,*d, *s, *tmp;
#if PY_VERSION_HEX >= 0x03000000
...
#else
  m = _qgs01_module = Py_InitModule("_qgs01", f2py_module_methods);
#endif
  Py_TYPE(&PyFortran_Type) = &PyType_Type;
  import_array();
  if (PyErr_Occurred())
    {PyErr_SetString(PyExc_ImportError, "can't initialize module _qgs01 (failed to import numpy)"); return RETVAL;}
