...
cqgsini(iseedin,datdir,lun,debugnum)\n\nWrapper for ``cqgsini``.\
\n\nParameters\n----------\n"
"iseedin : input int\n"
"datdir : input string(len=256)\n"
"lun : input int\n"
...
/*decl*/
  ...
  static char *capi_kwlist[] = {"iseedin","datdir","lun","debugnum",NULL};
...
f2py_start_clock();
  ...
  if (!PyArg_ParseTupleAndKeywords(capi_args,capi_keywds,\
    "OOOO|:_qgs01.cqgsini",\
    capi_kwlist,&iseedin_capi,&datdir_capi,&lun_capi,&debugnum_capi))
    return NULL;
/*frompyobj*/
  /* Processing variable iseedin */
    f2py_success = int_from_pyobj(&iseedin,iseedin_capi,"_qgs01.cqgsini() 1st argument (iseedin) can't be converted to int");
  if (f2py_success) {
  /* Processing variable datdir */
  slen(datdir) = 256;
  f2py_success = string_from_pyobj(&datdir,&slen(datdir),"",datdir_capi,"string_from_pyobj failed in converting 2nd argument `datdir' of _qgs01.cqgsini to C string");
  if (f2py_success) {
  /* Processing variable lun */
    f2py_success = int_from_pyobj(&lun,lun_capi,"_qgs01.cqgsini() 3rd argument (lun) can't be converted to int");
  if (f2py_success) {
  /* Processing variable debugnum */
    f2py_success = int_from_pyobj(&debugnum,debugnum_capi,"_qgs01.cqgsini() 4th argument (debugnum) can't be converted to int");
  if (f2py_success) {
/*end of frompyobj*/
#ifdef F2PY_REPORT_ATEXIT
f2py_start_call_clock();
#endif
/*callfortranroutine*/
        (*f2py_func)(&iseedin,datdir,&lun,&debugnum,slen(datdir));
if (PyErr_Occurred())
  f2py_success = 0;
#ifdef F2PY_REPORT_ATEXIT
f2py_stop_call_clock();
...
/******************************** init_rmmard ********************************/
static char doc_f2py_rout__qgs01_init_rmmard[] = "\
init_rmmard(iseedin)\n\nWrapper for ``init_rmmard``.\
\n\nParameters\n----------\n"
"iseedin : input int";
/* extern void F_FUNC_US(init_rmmard,INIT_RMMARD)(int*); */
static PyObject *f2py_rout__qgs01_init_rmmard(const PyObject *capi_self,
                           PyObject *capi_args,
                           PyObject *capi_keywds,
                           void (*f2py_func)(int*)) {
  PyObject * volatile capi_buildvalue = NULL;
...
#endif
  if (!PyArg_ParseTupleAndKeywords(capi_args,capi_keywds,\
    "O|:_qgs01.init_rmmard",\
    capi_kwlist,&iseedin_capi))
    return NULL;
/*frompyobj*/
  /* Processing variable iseedin */
    f2py_success = int_from_pyobj(&iseedin,iseedin_capi,"_qgs01.init_rmmard() 1st argument (iseedin) can't be converted to int");
  if (f2py_success) {
/*end of frompyobj*/
#ifdef F2PY_REPORT_ATEXIT
f2py_start_call_clock();
...
static FortranDataDef f2py_routine_defs[] = {
  ...
  {"init_rmmard",-1,{{-1}},0,(char *)F_FUNC_US(init_rmmard,INIT_RMMARD),(f2py_init_func)f2py_rout__qgs01_init_rmmard,doc_f2py_rout__qgs01_init_rmmard},
