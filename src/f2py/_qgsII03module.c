 ...
 * Generation date: Mon Aug 15 13:17:04 2022
...
/*frompyobj*/
  ...
  if (capi_ep0_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(_qgsII03_error,"failed in converting 1st argument `ep0' of _qgsII03.qgreg to C/Fortran array" );
  } else {
...
#else
#define RETVAL
PyMODINIT_FUNC init_qgsII03(void) {
#endif
  int i;
  PyObject *m,*d, *s, *tmp;
#if PY_VERSION_HEX >= 0x03000000
...
#else
  m = _qgsII03_module = Py_InitModule("_qgsII03", f2py_module_methods);
#endif
  Py_TYPE(&PyFortran_Type) = &PyType_Type;
  import_array();
  if (PyErr_Occurred())
    {PyErr_SetString(PyExc_ImportError, "can't initialize module _qgsII03 (failed to import numpy)"); return RETVAL;}
