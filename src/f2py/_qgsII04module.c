 ...
 * Generation date: Mon Aug 15 13:17:03 2022
...
/*frompyobj*/
  ...
  if (capi_ep0_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(_qgsII04_error,"failed in converting 1st argument `ep0' of _qgsII04.qgreg to C/Fortran array" );
  } else {
...
#else
#define RETVAL
PyMODINIT_FUNC init_qgsII04(void) {
#endif
  int i;
  PyObject *m,*d, *s, *tmp;
#if PY_VERSION_HEX >= 0x03000000
...
#else
  m = _qgsII04_module = Py_InitModule("_qgsII04", f2py_module_methods);
#endif
  Py_TYPE(&PyFortran_Type) = &PyType_Type;
  import_array();
  if (PyErr_Occurred())
    {PyErr_SetString(PyExc_ImportError, "can't initialize module _qgsII04 (failed to import numpy)"); return RETVAL;}
