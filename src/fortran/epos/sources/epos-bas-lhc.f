...
C...User process event common block.
      INTEGER MAXNUP
c...extend array for file production
      PARAMETER (MAXNUP=50000)
 ...
 111  rexdif(iii)=abs(rexdifi(iii))
  ...
  11      z=0.19*sqrt(-2*alog(rangen()))*cos(2*pi*rangen())
      ...
      elseif(noebin.lt.0)then  !fake e-A with pi0-A for hadron production
        ...
        idproj=1120  !110
c        if(model.eq.2.or.model.eq.6)idproj=idtarg   
!for qgsjet01, projectile and target are not symetric and we want to look what happen on the projectile side (used for CR)
        laproj=-1
        maproj=1
        engy=-1
        ekin=-1
