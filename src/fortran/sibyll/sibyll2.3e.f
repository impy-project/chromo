...
C       for Nucleus-Nitrogen and Nucleus-Oxygen
...
c     last in use:94
     ...
     &     9*0,0,2,9*0,                 ! 60
     &     100,25*0,2,1,0,0,0,1,0,1,6*0/! 100   
...
C.          = 4  "coherent beam diffraction (for nuclei only)"      
...
C...Single diffraction
      IF (JDIF1.EQ. 1 .OR. JDIF1.EQ. 4)  THEN
         K = MAX(1,2-IBAR(LA))
         IF(Irec.eq.1) K = KK(LA)
         EM  = AM(13)
         EM2 = AM2(13)
...
C.                  (3 : double diffraction)
C.                  (4 : coherent beam diffraction (for nuclei))      
C.
C-----------------------------------------------------------------------
      IMPLICIT NONE
...
c     read hadron-nucleon cross section from table
      ...
      IF (IA .GT. 1)  THEN !  nuclear target ?
c     use inelastic screening
         IF(IPAR(12).NE.0)THEN
c     distinguish between nuclear cross sections..            
            IF(IPAR(12).eq.3)THEN
               IF(IAFLG.eq.0)THEN !      is not air?
...
c     if target is air read hadron-air cross section from table
               ...
               ENDIF
            ELSE
c     always use air cross section (with screening)
               CALL SIB_SIGMA_HAIR(L,SQS,SIGprod,SIGbdif)
            ENDIF
...
c     single nucleon interaction in target
         IF(NW.eq.1)THEN
            IF(IPAR(12).NE.0)THEN
c     high mass (incoherent) diffraction?
     ...
     &              dLOG(S*PAR(13)/XM2MIN(L)))*SIGDIF(1)/SIGINEL
            ...
            ENDIF
         ELSE
c     multiple nucleon interaction in target (Nw>1)
...
C--------------------------------------------------------------------
      ...
      PZT=0.D0
      DO J=N1,N2
c     check energy conservation for fragmenting systems
         IF (LPID(J,2) .EQ. 0)  THEN
            NF = NF+1
...
c     cm energy
         ...
         XM1 = AM(IABS(L0))
         XM2 = P2(5)
         CALL TRANSFONSHELL(ECM,XM1,XM2,1.D0,1,P1N,P2N,LBD)
         IF(LBD.eq.1) THEN
            IF(NDEBUG.gt.2)
...
c     external types
      DOUBLE PRECISION ECM
      INTEGER K_beam, NW, Irec, IREJ, ICHK
...
C..............................................................
Cf2py integer,intent(in) :: iat
Cf2py integer,intent(in) :: iap
Cf2py integer,intent(in) :: nw
Cf2py double precision,intent(in) :: b
Cf2py integer,intent(out) ::  nf
Cf2py integer(60), intent(out) :: iaf      
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      IMPLICIT INTEGER(I-N)
...
C-----------------------------------------------------------------------------
C     f2py integer,intent(in) :: ia
C     f2py integer,intent(in) :: ib
C     f2py double precision,intent(in) :: sig0
C     f2py double precision,intent(in) :: sigel      
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      IMPLICIT INTEGER(I-N)
...
C.           Prob(n_A), Prob(n_B), Prob(n_int)
C..........................................................................
Cf2py integer, intent(in) :: ia
Cf2py integer, intent(in) :: ib
Cf2py double precision, intent(in) :: sig0
Cf2py double precision, intent(in) :: sigel
Cf2py integer, intent(in) :: kint
Cf2py double precision, intent(out) :: sigma
Cf2py double precision, intent(out) :: dsigma
Cf2py double precision, intent(out) :: sigqe
Cf2py double precision, intent(out) :: dsigqe
Cf2py integer, intent(parameter) :: iamax2      
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      IMPLICIT INTEGER(I-N)
