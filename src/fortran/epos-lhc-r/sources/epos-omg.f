...
! Z_parton_projectile (zparpro) and Z_parton_target (zpartar)-----------
      ...
      if(iscreen.eq.1.or.isplit.eq.1)then
        ...
        rho0t=conrho(2,0.)
        bcut=0.
        !going the nuclei, the nucleons on the same side may hide the other beam nucleus and reduce nuclear screening (AA xs will go to glauber while pA xs being reduced (maximum screening))
        zbcutp=max(log(float(max(1,ng11evt)))*zbrads,abs(zbcutx))
        zbcutt=max(log(float(max(1,ng12evt)))*zbrads,abs(zbcutx))

        if(ish.ge.10)write(ifch,*)
c        print *,
     &  'koll,zb',koll,ng11evt,ng12evt,zbcutx,zbcutp,zbcutt
        ...
        !....... targ partons seen by proj
          if(lproj(ip).gt.0)then
            ...
            zpartar(k)=min(zkt,epscrx)
c           znucl(k)=zpartar(k) !zkt
            if(lproj3(ip).gt.1.and.coltrp(ip).lt.dble(bk(k)
     &                                             /zbcutp))then
              do lt=lproj3(ip),1,-1
                kp=kproj3(ip,lt)
                if(kp.ne.k)then
c                  do m=1,lt-1
c                    km=kproj3(ip,m)
c                    if(kp.ne.km.and.bk(kp).ge.frdmzcut(bk(km))
c     &          .and.phik3(kp).ge.phik3(km)-thetak3(km)
c     &          .and.phik3(kp).le.phik3(km)+thetak3(km))then
c                      goto 300   !skip if pair in screened by another pair
c                    endif
c                  enddo
                  ikt=itarg(kp)
                  ...
                  if(coltrt(ikt).lt.dble(bk(kp)/zbcutt))
     .            zpartar(k)=min(5.,zpartar(k)+min(zkt,epscrx))
c     .                             ,epscrx*(1.+log10(1.+znurho))))
                  znucl(k)=znucl(k)+zkt
c 300              continue
                 endif
...
c            zpartar(k)=zpartar(k)+min(znucl(k),epscrx)
          else
            zpartar(k)=0.
          endif
          ztav=ztav+zpartar(k)
         ...
         !...........proj partons seen by targ
          if(ltarg(it).gt.0)then
            ...
            zparpro(k)=min(zkp,epscrx)
c            znucl(k)=znucl(k)+zparpro(k)!zkp
            if(ltarg3(it).gt.1.and.coltrt(it).lt.dble(bk(k)
     &                                             /zbcutt))then
              do lp=ltarg3(it),1,-1
                kt=ktarg3(it,lp)
                if(kt.ne.k)then
c                  do m=1,lp-1
c                    km=ktarg3(it,m)
c                    if(kt.ne.km.and.bk(kt).ge.frdmzcut(bk(km))
c     &          .and.phik3(kt).ge.phik3(km)-thetak3(km)
c     &          .and.phik3(kt).le.phik3(km)+thetak3(km))then
c                      goto 400   !skip if pair in screened by another pair
c                    endif
c                  enddo
                  ikp=iproj(kt)
                  ...
                  if(coltrp(ikp).lt.dble(bk(kt)/zbcutp))
     .            zparpro(k)=min(4.,zparpro(k)+min(zkp,epscrx))
c     .                             ,epscrx*(1.+log10(1.+znurho))))
                  znucl(k)=znucl(k)+zkp
c 400              continue
                endif
...
c            zparpro(k)=zparpro(k)+min(znucl(k),epscrx)
          else
           zparpro(k)=0.
          endif
          zpav=zpav+zparpro(k)
...
c        gamV0=min(zbrmax,zparpro(k)+zpartar(k))
c        gammaV(k)=max(0.1d0,1d0-coltrp(ip)*dble(gamV0))
c       .           *max(0.1d0,1d0-coltrt(it)*dble(gamV0))
c        gammaV(k)=min(dble(zbrmax),
c     .            max(0.01d0,1d0-2d0*coltrp(ip)*dble(zparpro(k)))
c     .           *max(0.01d0,1d0-2d0*coltrt(it)*dble(zpartar(k))))
c        gammaV(k)=min(1d0,
c     .            max(0.1d0,1d0-dble(zbrmax)*coltrp(ip)*dble(znucl(k)))
c     .           *max(0.1d0,1d0-dble(zbrmax)*coltrt(it)*dble(znucl(k))))
c      gammaV(k)=max(0.1d0,1d0-coltrp(ip)*dble(min(zbrmax,0.5*znucl(k))))
c     .         *max(0.1d0,1d0-coltrt(it)*dble(min(zbrmax,0.5*znucl(k))))
c      gammaV(k)=max(0.1d0,1d0-coltrp(ip)*dble(min(zbrmax,zparpro(k))))
c     .         *max(0.1d0,1d0-coltrt(it)*dble(min(zbrmax,zpartar(k))))
c      gammaV(k)=max(0.1d0,1d0-coltrp(ip)*dble(zbrmax))
c     .         *max(0.1d0,1d0-coltrt(it)*dble(zbrmax))
c        gammaV(k)=min(dble(zbrmax),
c     .            max(0.01d0,1d0-2d0*coltrp(ip)*dble(znucl(k)))
c     .           *max(0.01d0,1d0-2d0*coltrt(it)*dble(znucl(k))))
c        gammaV(k)=coltrp(ip)*coltrt(it)
c        print *,k,gammaV(k),znucl(k)
        gammaV(k)=1d0
...
c          if(i.eq.0) atildg(i,k)=atildg(i,k)
   ...
   !...........prepare plot in xepsilon
          if(irea.eq.0.and..not.chargex)then
...
c            epsG=epsilongs(k,i)
          ...
          atilde(i,k)=dble(alpD(i,iclpro,icltar))  !gamv in function "omega" in ems
          if(i.lt.2)then
            epsGp=epsilongp(k,i)
            epsGt=epsilongt(k,i)
            epsG=min(0.,epsGp+epsGt)
...
c        gamV0=min(zbrmax,zzp+zzip)
c        if(gamV0.gt.0.9)then
c          gamV=0.45*0.9/gamV0+0.1*(1.-0.9/gamV0)
c        else
c          gamV=1.-0.5*gamV0
c        endif
...
c     &               min(epscrx,epscrs*x))
        ...
        if(i.ne.2)epsG=min(0.,epsGp+epsGt)
c        gamV=gamV
      else
        zzp=0.
        zzt=0.
...
c      if(x2.gt.1.)fscro=sqrt(log(x2)**2+fscro**2)
      if(x2.gt.1.)fscro=log(x2)*(1.+znurho*rho)
      end
...
c----------------------------------------------------------------------
      ...
      double precision xh,yp,om51p,omGamk,xp,xm
      ...
      if(iqq.eq.-1)then
        om5Jk=omGamk(k,xp,xm) 
      elseif(iqq.ne.5)then
        om5Jk=2d0*om51p(sy,xh,yp,b,iqq)*gammaV(k)
...
c          xm=xh/xp
c          i1=min(iqq,1)
cc use pp screening even for nuclei (avoid to many diffractive collisions)
cc          call Gfunpar(0.,0.,1,i1,b,sngl(s),alp,bet,betp,epsp,epst,epss
cc     &                 ,gamv)
cc          epsGp=epsp
cc          epsGt=epst
ccc          epsG=epsilongs(k,i1)
c          if(iqq.eq.0)then
c            epsGp=epsilongp(k,i1)
c            epsGt=epsilongt(k,i1)
c          elseif(gfactor.ge.0.)then
c           epsGp=epsilongp(k,i1)*exp(-min(50.,gfactor*max(0,npp(ip)-1)))
c           epsGt=epsilongt(k,i1)*exp(-min(50.,gfactor*max(0,npt(it)-1)))
cc         epsGp=epsilongp(k,i1)*exp(-min(50.,gfactor*zparpro(k))
cc         epsGt=epsilongt(k,i1)*exp(-min(50.,gfactor*zpartar(k))
c          endif
c          om5Jk=om5Jk*xp**dble(epsGp)*xm**dble(epsGt)!*s**dble(epsG))
c        endif
      else
        om5Jk=atilde(2,k)*xp**btildep(2,k)*xm**btildepp(2,k)
      endif
      return
      end
...
c----------------------------------------------------------------------
      ...
      elseif(iq.ne.5)then
        i1=min(iq,1)
c        call Gfunpar(zzp,zzt,1,i1,b,sy,alp,bet,betp,epsp,epst,epss,gamv)
c        call Gfunpar(0.,0.,1,i1,b,sy,alp,bet,betp,epsp,epst,epss,gamv)
c        epsG=epss
c        if(iq.eq.0)then
c          epsGp=epsp
c          epsGt=epst
c        elseif(gfactor.gt.0.)then
c          epsGp=epsp*exp(-min(50.,gfactor*zzp))
c          epsGt=epst*exp(-min(50.,gfactor*zzt))
c        endif
        om5J=om51p(sy*sngl(xh),xh,yp,b,iq)
c     &      *xp**dble(epsGp)*xm**dble(epsGt)!*sy**dble(epsG)
      else
...
c-----------------------------------------------------------------------
      ...
      include 'epos.incems'
      include 'epos.incsem'
      doubleprecision xh,yp,om5Jk,w(-1:7)!,ww
      ...
      ip=iproj(k)
      it=itarg(k)
      if(npp(ip)+npt(it).gt.2)w(5)=0d0   !no diff if more than one Pom on a nucleon
c      if(nprt(k).gt.1)w(5)=0d0   !no diff if more than one Pom on a pair
      nmin=2
c      if(ioquen.ne.0)nmin=4     !without fusion, no need to increase soft
     
c      w(6)=max(0d0,w(-1)-(w(0)+w(5))/dble(1.+gfactor*max(0,nprt(k)-1))) !what is left for semi-hard after soft and diff are taken from all
      w(6)=max(0d0,w(-1)-(w(0)+w(5))
     &               /dble(max(1.,0.5*(npp(ip)+npt(it)-nmin)))**gfactor) !what is left for semi-hard after soft and diff are taken from all
      
c      w(5)=dble(log(engy)/3.)*w(5)    !??????? correction to have MC=eq
c      if(gfactor.lt.0..and.w(1).gt.xggfit*w(0))then      !??????????????
c        corfac=exp(-dble(abs(gfactor)*(zparpro(k)+zpartar(k)))
c     &                          *max(0d0,1d0-sqrt(xggfit*w(0)/w(1))))
c        ww=w(0)+w(1)     !gg interaction probability
c        w(0)=w(0)*corfac        !soft suppressed
c        w(1)=ww-w(0)            !semi-hard increased
c
...
c----------------------------------------------------------------------
      ...
      if(ish.ge.1.and..not.chargex)write(ifmt,*)"Fit xkappa ..."
      if(ish.ge.2.and..not.chargex)then
