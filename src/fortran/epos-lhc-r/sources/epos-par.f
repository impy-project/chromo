...
c Variables used for xparg (only graphical application)
      ...
      sfshlim=4.*spmin          !transition energy for soft->hard
...
c because of quark exchange, we need alpD, ... for
c iclpro and icltar = 1 and 2 and we need iclpro=3 only for Kaon projectile 
        iiiclpro1=max(iclpro,2)
        iiicltar1=max(icltar,2)
        iiidirec1=1
        iiidirec2=1
        if(imod.eq.0)then
          iiiclpro1=iclpro
          ...
          iiiclpro2=iclpro
          iiicltar2=icltar
        elseif(ichargex.eq.1)then
          iiiclpro2=1
          iiicltar2=1
        else
          iiiclpro2=min(2,iclpro)
          iiicltar2=min(2,icltar)
        endif
        if(iiiclpro1.lt.iiiclpro2)iiidirec1=-1
        if(iiicltar1.lt.iiicltar2)iiidirec2=-1
        iclprosave=iclpro
        icltarsave=icltar

        do iiipro=iiiclpro2,iiiclpro1,iiidirec1
        do iiitar=iiicltar2,iiicltar1,iiidirec2
...
c Print results
      ...
      if(ish.ge.4)then
        write(ifch,*)"parameters for iclpro,icltar:",iclpro,icltar
...
c of nuclear shadowing
        ...
        bglaubx=0.!zbrads*sqrt(max(0.1,satrad))
        zbcutx=zbcut*sqrt(max(0.0001,satrad))!*bglaubx
!        print *,'---->',bglaubx,zbcutx,satrad,b2xscr
      endif
...
c----------------------------------------------------------------------
      ...
      if(iqq.eq.0.or.xshmin.gt.0.95d0)then  !cannot use xminDf to xmaxDf because a break appear in the cross-section, but better to use lower end of energy for a better match between formula and MC
        X0=xminDf !sqrt(xminDf)
        xmax=sqrt(xminDf) !xmaxDf
