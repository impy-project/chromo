...
cc----------------------------------------------------------------------
c      subroutine amicro(iret)
cc----------------------------------------------------------------------
cc  microcanonical decay of cluster specified via keu...ket, tecm, volu
cc----------------------------------------------------------------------
c      include 'epos.inc'
c      parameter(maxp=6000)
c      common/confg/np,amass(maxp),ident(maxp),pcm(5,maxp),wtxlog,wtlog
c      double precision seedp
c      data ncntmi/0/
c      save ncntmi
c      call utpri('amicro',ish,ishini,4)
c      ncntmi=ncntmi+1
c
c      if(ncntmi.eq.1)then
c        call ranfgt(seedp)      !not to change the seed ...
c        if(hydt.ne.'---')then
c          call HydroTable2(0)
c          call DefineParticles
c        else
c          call ManiParticles(93,0)
c        endif
...
c      nptl=0
cc      dez=0.5e-4
c      call InitializeHyperbola !does not affect results but necessary
c
cc  50  continue
c      call GraCan
c      energ=0
c      do i=1,np
c        energ=energ+pcm(4,i)
c      enddo
c      !print*,'+++++',energ,tecm,tecm/volu
cccc      if(abs(energ-tecm).gt.0.1) goto50   !uncomment for energy conservation
c
c      do n=1,np
c        nptl=nptl+1
c        if(nptl.gt.mxptl)call utstop('StaHadShort: mxptl too small&')
c        idptl(nptl)=ident(n)
c        do j=1,4
c          pptl(j,nptl)=pcm(j,n)
c          xorptl(j,nptl)=0
c        enddo
c        pptl(5,nptl)=amass(n)
c        ityptl(nptl)=19
c      enddo
c
c      do n=1,nptl
...
c      enddo
c
c      call utprix('amicro',ish,ishini,4)
c      return
c      end
...
cc------------------------------------------------------------------------------
c      subroutine xFreezeOutTauX(meta)
cc------------------------------------------------------------------------------
c      include 'epos.inc'
c      include 'epos.inchy'
c      common/cxyzt/xptl(mxptl),yptl(mxptl),zptl(mxptl),tptl(mxptl)
c     *,optl(mxptl),uptl(mxptl),sptl(mxptl),rptl(mxptl,3)
c      common/cen/ncentr /ctauhu/ntauhu(ncenthx,1-netahx:netahx-1)
c      !..........................................................................
c      nhis=1
c      npl=0
c      nplx=0
c      deleta=etahy(2)-etahy(1)
c      eta1=zetahy(meta)-deleta/2
c      eta2=zetahy(meta)+deleta/2
c      etaav=zetahy(meta)
c      taumax=tauhoc(ncentr,ntauhu(ncentr,meta))+4
c      do n=1,nptl
c        if(ityptl(n).eq.60
c     * .and.istptl(n).ne.12.and.istptl(n).ne.11)then
c         if(istptl(iorptl(n)).eq.11)then
c          tau=0
c          tau2=xorptl(4,n)**2-xorptl(3,n)**2
c          if(tau2.gt.0.)tau=sqrt(tau2)
c          if(tau.lt.taumax)then
c           rap=
c     *     .5*alog((xorptl(4,n)+xorptl(3,n))/(xorptl(4,n)-xorptl(3,n)))
c           if(rap.ge.eta1.and.rap.le.eta2)then
c           if(abs(xorptl(2,n)).le.2)then
c            npl=npl+1
c            nplx=nplx+1
c            if(npl.eq.1)then
c              if(nplx.gt.1)
c     *        write(ifhi,'(a)')       '  endarray closehisto plot 0-'
c              write(ifhi,'(a)')       '!-------------------------------'
c              write(ifhi,'(a)')       '!   tau-x      '
c              write(ifhi,'(a)')       '!-------------------------------'
c              write(ifhi,'(a)')       '!newpage'
c              write(ifhi,'(a,i1)')    'openhisto name t-r-0-',nhis
c              write(ifhi,'(a)')       'htyp prl xmod lin ymod lin'
c              write(ifhi,'(a)')       'xrange -10 10'
c              write(ifhi,'(a,f5.1)')  'yrange 0 ',taumax
c              write(ifhi,'(a)')       'txt  "xaxis x (fm)"'
c              write(ifhi,'(a)')       'txt  "yaxis [t] (fm/c)"'
c              write(ifhi,'(a,f4.1,a)')'text 0.35 0.90 "[c]=',etaav,'"'
c              write(ifhi,'(a)')    'text 0.02 0.9 """B#y"B#"L#2fm""'
c          write(ifhi,'(a,f4.1,a)')'text 0.65 0.9 "',centhy(ncentr),'fm"'
c              write(ifhi,'(a)')       'array 2'
c            endif
c            write(ifhi,'(2e11.3)')xorptl(1,n),tau
c            if(npl.eq.1000)then
c              nhis=nhis+1
c              npl=0
c            endif
c           endif
...
c        endif
c      enddo
c      if(nplx.gt.0)write(ifhi,'(a)')  '  endarray closehisto plot 0-'
c    !..........................................................................
c       nhis=1
c      npl=0
c      nplx=0
c      npli=20
c      do n=1,nptl
c        if(dezptl(n).lt.1e3.and.n.le.maxfra
c     *  .and.(istptl(n).eq.0.or.istptl(n).eq.1))then
c           rap=1e10
c           xp=tptl(n)+zptl(n)
c           xm=tptl(n)-zptl(n)
c           if(xp.gt.0.0.and.xm.gt.0.0)rap=.5*alog(xp/xm)
c           if(rap.ge.-0.5.and.rap.le.0.5)then
c           if(abs(xorptl(2,n)).le.2)then
c            npl=npl+npli
c            nplx=nplx+1
c            if(npl.eq.npli)then
c              if(nplx.gt.1)
c     *        write(ifhi,'(a)')       '  endarray closehisto plot 0-'
c              write(ifhi,'(a)')       '!-------------------------------'
c              write(ifhi,'(a)')       '!   tau-x    corona  '
c              write(ifhi,'(a)')       '!-------------------------------'
c              write(ifhi,'(a,i1)')    'openhisto name t-r-0-cor-',nhis
c              write(ifhi,'(a)')       'htyp pgl '
c              write(ifhi,'(a)')       'array 2'
c            endif
c            x=xorptl(1,n)
c            y=xorptl(2,n)
...
c            if(tau2.gt.0.)tau=sqrt(tau2)
c            if(abs(y).le.2)
c     .      write(ifhi,'(2e11.3)')x,tau
c            dt=0.1
c            do k=1,npli-1
c            x=x+pptl(1,n)/pptl(4,n)*dt
c            y=y+pptl(2,n)/pptl(4,n)*dt
c            z=z+pptl(3,n)/pptl(4,n)*dt
c            t=t+dt
c            tau=0
c            tau2=t**2-z**2
c            if(tau2.gt.0.)tau=sqrt(tau2)
...
c      write(ifhi,'(a)') '!-------------------------------'
c      write(ifhi,'(a)') '!newpage'
c      write(ifhi,'(a)') 'openhisto name feff'
c      write(ifhi,'(a)') 'htyp lin xmod lin ymod lin'
c      write(ifhi,'(a)') 'xrange -6 6'
c      write(ifhi,'(a)') 'yrange 0 1.2'
c      write(ifhi,'(a)') 'txt  "xaxis [c] "'
c      write(ifhi,'(a)') 'txt  "yaxis f?eff!"'
c      write(ifhi,'(a,f4.1,a)') 'text 0.65 0.9 "',centhy(ncentr),'fm"'
c      write(ifhi,'(a)') 'array 2'
c      do meta=-netahy+1,netahy-1
c        neta=abs(meta)+1
c        eta=etahy(neta)
c        if(meta.lt.0)eta=-eta
c        write(ifhi,'(2e11.3)') eta,feff(ncentr,meta)
c      enddo
c      write(ifhi,'(a)')    '  endarray closehisto plot 0'
c      end
c
cc------------------------------------------------------------------------------
c      subroutine xFreezeOutTauEta
cc------------------------------------------------------------------------------
c      include 'epos.inc'
c      include 'epos.inchy'
c      common/cxyzt/xptl(mxptl),yptl(mxptl),zptl(mxptl),tptl(mxptl)
c     *,optl(mxptl),uptl(mxptl),sptl(mxptl),rptl(mxptl,3)
c      common/cen/ncentr /ctauhu/ntauhu(ncenthx,1-netahx:netahx-1)
c      taumax=tauhoc(ncentr,ntauhu(ncentr,0))+4
c    !..........................................................................
c      nhis=1
c      npl=0
c      do n=1,nptl
...
c            if(npl.eq.1)then
c              write(ifhi,'(a)')      '!-------------------------------'
c              write(ifhi,'(a)')      '!   tau-eta      '
c              write(ifhi,'(a)')      '!-------------------------------'
c              write(ifhi,'(a)')      '!newpage'
c              write(ifhi,'(a,i1)')   'openhisto name t-eta-',nhis
c              write(ifhi,'(a)')      'htyp prl xmod lin ymod lin'
c              write(ifhi,'(a)')      'xrange -4 4'
c              write(ifhi,'(a,f5.1)')  'yrange 0 ',taumax
c              write(ifhi,'(a)')    'txt  "xaxis [c] "'
c              write(ifhi,'(a)')    'txt  "yaxis [t] (fm/c)"'
c      write(ifhi,'(a,f4.1,a)')'text 0.65 0.9 "',centhy(ncentr),'fm"'
...
c        endif
c      enddo
c      if(npl.ne.0)write(ifhi,'(a)')  '  endarray closehisto plot 0'
c      if(npl.eq.0)stop'xFreezeOutTZ: no particles!!!!!            '
c      end
c
cc------------------------------------------------------------------------------
c      subroutine xFreezeOutTZ
cc------------------------------------------------------------------------------
c      include 'epos.inc'
c      include 'epos.inchy'
c      common/cxyzt/xptl(mxptl),yptl(mxptl),zptl(mxptl),tptl(mxptl)
...
c              write(ifhi,'(a)')      '!-------------------------------'
c              write(ifhi,'(a)')      '!newpage'
c              write(ifhi,'(a,i1)')   'openhisto name t-z-',nhis
c              write(ifhi,'(a)')      'htyp prl xmod lin ymod lin'
c              write(ifhi,'(a)')      'xrange -25 25'
c              write(ifhi,'(a)')      'yrange 0 25 '
c              write(ifhi,'(a)')    'txt  "xaxis z (fm)"'
c              write(ifhi,'(a)')    'txt  "yaxis t (fm/c)"'
c      write(ifhi,'(a,f4.1,a)')'text 0.70 0.22 "',centhy(ncentr),'fm"'
c              write(ifhi,'(a)')       'array 2'
c            endif
c            write(ifhi,'(2e11.3)') xorptl(3,n),xorptl(4,n)
c            if(npl.eq.1000)then
c              write(ifhi,'(a)')    '  endarray closehisto plot 0-'
c              nhis=nhis+1
c              npl=0
...
c
cc------------------------------------------------------------------------------
c      subroutine xFoMass(meta)
cc------------------------------------------------------------------------------
c      include 'epos.inc'
c      include 'epos.inchy'
c      common/cen/ncentr /ctauhu/ntauhu(ncenthx,1-netahx:netahx-1)
c      taumax=tauhoc(ncentr,ntauhu(ncentr,meta))+2
cc      netahyxx=netahy
c      write(ifhi,'(a)')    '!----------------------------------------'
c      write(ifhi,'(a,i3)') '!   hydro freeze out rate     '
c      write(ifhi,'(a)')    '!----------------------------------------'
c      write(ifhi,'(a)')       '!newpage'
c      do ii=1,3
c        if(ii.eq.1)nphi=2
c        if(ii.eq.2)nphi=1+nphihy/4
c        if(ii.eq.3)nphi=1+nphihy/2
c        write(ifhi,'(a,3i1)')'openhisto htyp lin name w-',meta,nphi,ii
c        if(ii.eq.1)then !----------------------
c         write(ifhi,'(a,f4.1)')'xmod lin xrange 0. ',taumax
c         write(ifhi,'(a)')    'txt  "xaxis [t] (fm/c)"'
c         write(ifhi,'(a)') 'ymod lin yrange auto auto '
c         write(ifhi,'(a,f4.1,a)')'text 0.1 0.9 "  [c]=',zetahy(meta),'"'
c         write(ifhi,'(a)')'txt "yaxis w "'
c         write(ifhi,'(a,f4.1,a)')'text 0.65 0.9 "',centhy(ncentr),'fm"'
c        endif       !-------------------------------
c        write(ifhi,'(a)')'array 2'
c        deltau=tauhoc(ncentr,2)-tauhoc(ncentr,1)
c        do ntau=2,ntauhu(ncentr,meta)
c         dy=waa(ncentr,meta,ntau,nphi)
c         write(ifhi,'(2e13.5)')tauhoc(ncentr,ntau)-deltau/2,dy
c        enddo
c        write(ifhi,'(a)') 'endarray closehisto '
c        if(ii.ne.3)write(ifhi,'(a)') 'plot 0-'
c        if(ii.eq.3)write(ifhi,'(a)') 'plot 0'
c      enddo
c      end
c
cc------------------------------------------------------------------------------
c      subroutine xFoRadius(meta)
cc------------------------------------------------------------------------------
c      include 'epos.inc'
c      include 'epos.inchy'
c      common/cen/ncentr /ctauhu/ntauhu(ncenthx,1-netahx:netahx-1)
...
cc      netahyxx=netahy
c      write(ifhi,'(a)')    '!----------------------------------------'
c      write(ifhi,'(a,i3)') '!        hydro freeze out radius     '
c      write(ifhi,'(a)')    '!----------------------------------------'
c      write(ifhi,'(a)')    '!newpage'
c      do ii=1,3
c        if(ii.eq.1)nphi=2
c        if(ii.eq.2)nphi=1+nphihy/4
c        if(ii.eq.3)nphi=1+nphihy/2
c        write(ifhi,'(a,3i1)')'openhisto htyp lin name r-',meta,nphi,ii
c        if(ii.eq.1)then !----------------------
c         write(ifhi,'(a,f4.1)')'xmod lin xrange 0. ',taumax
c         write(ifhi,'(a)')'txt  "xaxis [t] (fm/c)"'
c         write(ifhi,'(a)') 'ymod lin yrange auto auto '
c         write(ifhi,'(a,f4.1,a)')'text 0.1 0.9 "  [c]=',zetahy(meta),'"'
c         write(ifhi,'(a)')'txt "yaxis r  (fm) "'
c         write(ifhi,'(a,f4.1,a)')'text 0.65 0.9 "',centhy(ncentr),'fm"'
c        endif !-------------------------------
c        write(ifhi,'(a)')'array 2'
c        do ntau=2,ntauhu(ncentr,meta)
c         write(ifhi,'(2e13.5)')tauhoc(ncentr,ntau)
c     .     ,raa(ncentr,meta,ntau,nphi)
c         enddo
c        write(ifhi,'(a)') 'endarray closehisto '
c        if(ii.ne.3)write(ifhi,'(a)') 'plot 0-'
c        if(ii.eq.3)write(ifhi,'(a)') 'plot 0'
...
c
cc------------------------------------------------------------------------------
c      subroutine xFoRadVelocity(meta)
cc------------------------------------------------------------------------------
c      include 'epos.inc'
c      include 'epos.inchy'
c      common/cen/ncentr /ctauhu/ntauhu(ncenthx,1-netahx:netahx-1)
...
cc      netahyxx=netahy
c      write(ifhi,'(a)')    '!----------------------------------------'
c      write(ifhi,'(a,i3)') '!     hydro freeze out rad velocity      '
c      write(ifhi,'(a)')    '!----------------------------------------'
c      write(ifhi,'(a)')    '!newpage'
c      do ii=1,3
c        if(ii.eq.1)nphi=2
c        if(ii.eq.2)nphi=1+nphihy/4
c        if(ii.eq.3)nphi=1+nphihy/2
c        write(ifhi,'(a,3i1)')'openhisto htyp lin name y-',meta,nphi,ii
c        if(ii.eq.1)then !----------------------
c         write(ifhi,'(a,f4.1)')'xmod lin xrange 0. ',taumax
c         write(ifhi,'(a)')'txt "xaxis [t] (fm/c)"'
c         write(ifhi,'(a)') 'ymod lin yrange auto auto '
c         write(ifhi,'(a,f4.1,a)')'text 0.1 0.9 "  [c]=',zetahy(meta),'"'
c         write(ifhi,'(a)')'txt "yaxis v?rad!  "'
c         write(ifhi,'(a,f4.1,a)')'text 0.65 0.9 "',centhy(ncentr),'fm"'
c        endif !-------------------------------
c        write(ifhi,'(a)')'array 2'
c        do ntau=2,ntauhu(ncentr,meta)
c         write(ifhi,'(2e13.5)')tauhoc(ncentr,ntau)
c     .     ,vaa(1,ncentr,meta,ntau,nphi)
c        enddo
c        write(ifhi,'(a)') 'endarray closehisto '
c        if(ii.ne.3)write(ifhi,'(a)') 'plot 0-'
c        if(ii.eq.3)write(ifhi,'(a)') 'plot 0'
...
c      end
cc------------------------------------------------------------------------------
c      subroutine xFoTanVelocity(meta)
cc------------------------------------------------------------------------------
c      include 'epos.inc'
c      include 'epos.inchy'
c      common/cen/ncentr /ctauhu/ntauhu(ncenthx,1-netahx:netahx-1)
...
cc      netahyxx=netahy
c      write(ifhi,'(a)')    '!----------------------------------------'
c      write(ifhi,'(a,i3)') '!    hydro freeze out tang velocity      '
c      write(ifhi,'(a)')    '!----------------------------------------'
c      write(ifhi,'(a)')    '!newpage'
c      do ii=1,4
c        if(ii.eq.1)nphi=2
c        if(ii.eq.2)nphi=1+nphihy/8
c        if(ii.eq.3)nphi=1+nphihy/8*3
c        if(ii.eq.4)nphi=1+nphihy/2
c        write(ifhi,'(a,3i1)')'openhisto htyp lin name y-',meta,nphi,ii
c        if(ii.eq.1)then !----------------------
c         write(ifhi,'(a,f4.1)')'xmod lin xrange 0. ',taumax
c         write(ifhi,'(a)')'txt "xaxis [t] (fm/c)"'
c         write(ifhi,'(a)') 'ymod lin yrange auto auto '
c         write(ifhi,'(a,f4.1,a)')'text 0.1 0.9 "  [c]=',zetahy(meta),'"'
c         write(ifhi,'(a)')'txt "yaxis v?tan!  "'
c          write(ifhi,'(a,f4.1,a)')'text 0.65 0.9 "',centhy(ncentr),'fm"'
c        endif !-------------------------------
c        write(ifhi,'(a)')'array 2'
c        do ntau=2,ntauhu(ncentr,meta)
c         write(ifhi,'(2e13.5)')tauhoc(ncentr,ntau)
c     .     ,vaa(2,ncentr,meta,ntau,nphi)
c        enddo
c        write(ifhi,'(a)') 'endarray closehisto '
c        if(ii.ne.4)write(ifhi,'(a)') 'plot 0-'
c        if(ii.eq.4)write(ifhi,'(a)') 'plot 0'
c      enddo
c      end
...
c        write(ifhi,'(a)')     'plot 0-'
c        write(ifhi,'(a)')       '!----------------------------------'
c        write(ifhi,'(a)')       '!   collisions            '
c        write(ifhi,'(a)')       '!----------------------------------'
c        write(ifhi,'(a)')      'openhisto name coord'
c        write(ifhi,'(a,2e11.3)')'xrange',xunt,xob
c        write(ifhi,'(a,2e11.3)')'yrange',yunt,yob
c        write(ifhi,'(a)')       'htyp pks xmod lin ymod lin'
c        write(ifhi,'(a)')       'array 2'
c        do k=1,koll
c          if(nprt(k).gt.0)then
c            if(loopeta.eq.7)then 
c              i=iproj(k)
c              j=itarg(k)
c              write(ifmt,'(a,2i4,$)')'Dipole',i,j
c              x=diproj(i,1)*0.5
c              y=diproj(i,2)*0.5 
c              write(ifmt,'(2f8.2,$)')           
c     *          cc*x+ss*y+xshft
