...
c     functions and subroutines
      ...
      real*8  tformii(2),pxii(2),pyii(2)             !### kw ###
      integer indii(2),iso3ii(2),itypii(2),ityii(2)  !### kw ###
      common /saveii/ tformii,pxii,pyii,ityii,indii,iso3ii,itypii !### kw ###
      integer iitotcross                   !###### kw ######
      common /ciitotcross/iitotcross        !###### kw ######
      real*8 esave                       !###### kw ######
      common /cesave/esave              !###### kw ######
      integer iuchaskip                     !### kw ###
      common/ciuchaskip/iuchaskip           !### kw ###
      !call traceity(11)
      !call traceori(11,230) !### kw ###
      logical lpr                           !### kw ###
      lpr=.false.                           !### kw ###
...
c     prepare output to decay file
         ...
         if((betax**2+betay**2+betaz**2).gt.1.0d0)then
            write(*,*)'beta > 1 (dec)', ind1, e1,betax,
     $           betay,betaz
            betaz=dsqrt(0.99999999-betax**2-betay**2)
         endif
...
c     transformation betas into two particle CMS system
         ...
         if((betax**2+betay**2+betaz**2).gt.1.0d0)then
            write(*,*)'beta > 1 (scat)', ind1, e1,e2,
     $           pz(ind1),pz(ind2),betax,betay,betaz
            betaz=dsqrt(0.99999999-betax**2-betay**2)
         endif
...
c           get  process-id (iline) from sigmaLN array in blockres
     ...
     &                     ityp2,iso32,fmass(ind2),sigma(ii-2))
      ...
      if(sqrts.le.1.94d0.and.isigline.le.2.and.lpr)                 !### kw ###                 
     .write(ifch,'(a,3i3,f7.3,2f8.4)') ' xscatter  ' ,ii            !### kw ###   
     .,isigline,iline,sngl(sqrts), sngl(sigma(ii-2)),sngl(sigma(0)) !### kw ###
...
c     select partial cross section ii
     ...
     .    ' xscatter  --> ',ii                         !### kw ###
         ...
         if(isigline.eq.9.and.SigmaLN(ii+2,1,isigline).eq.10.and.lpr) !### kw ###
     &   write(ifch,*)'xscatter ',isigline,ii                  !### kw ###
     &   ,SigmaLN(ii+2,1,isigline),sngl(sqrts),sngl(sigma(ii)) !### kw ###
     &   ,sngl(sigma(0)),iuchaskip          !### kw ###
...
c     or rejection due to energy non-conservation
      call saveinfo(ind1,1,0)
      call saveinfo(ind2,2,0)
...
c     functions:
     ...
     &                 bar,nm1,nm2
      ...
      common /saveii/ tformii,pxii,pyii,ityii,indii,iso3ii,itypii !### kw ###
      real*8  pzii(2)                                !### kw ###
      common /saveii2/pzii                           !### kw ###
      integer phiidii(2)                             !### kw ###
      common /saveii3/phiidii                        !### kw ###
      integer itypart(nmax)                          !### kw ###
      common /city/itypart                           !### kw ###
      integer ilinexxx                               !### kw ###
      common /cilinexxx/ilinexxx                     !### kw ###
      integer      ifop,ifmt,ifch,ifcx,ifhi,ifdt,ifcp,ifdr,ifio!###### kw ######
      common/files/ifop,ifmt,ifch,ifcx,ifhi,ifdt,ifcp,ifdr,ifio!###### kw ######
      integer ninit                              !###### kw ######
...
c     reset some pointer arrays
 ...
 10   continue
      ...
      !###### the following is needed to keep information     !### kw ###
      !######  lost after calling add/delpart                 !### kw ###
      
      indii(1)=ind1
      tformii(1)=tform(ind1)
      pxii(1)=px(ind1)
      pyii(1)=py(ind1)
      pzii(1)=pz(ind1)
      ityii(1)=itypart(ind1)
      iso3ii(1)=iso3(ind1)
      itypii(1)=ityp(ind1)
      phiidii(1)=phiid(ind1) 
      !write(*,*)'INFO 04062014 --1-- ',indii(1),itypii(1),phiidii(1)
      ...
      if(ind2.gt.0) then
        ninit=2  
        indii(2)=ind2
        tformii(2)=tform(ind2)
        pxii(2)=px(ind2)
        pyii(2)=py(ind2)
        pzii(2)=pz(ind2)
        ityii(2)=itypart(ind2)
        iso3ii(2)=iso3(ind2)
        itypii(2)=ityp(ind2)
        phiidii(2)=phiid(ind2) 
        !write(*,*)'INFO 04062014 --2-- ',indii(2),itypii(2),phiidii(2)
      endif
...
c formation time
         tauf(1)=0.d0     
            if(pnew(4,1).ne.pnew(4,1))then
               write(*,*)'NaN in energy (1) ',pnew(4,1),1,pxi1,
     $              pxi2,xnew(4,1),xnew(1,1),nexit
               stop
            endif    
...
c compute formation time (as a eigentime)
            ...
            if(pnew(4,j).ne.pnew(4,j))then
               write(*,*)'NaN in energy (2) ',pnew(4,j),j,pxi1,
     $              pxi2,xnew(4,j),xnew(1,j),nexit
            endif
...
c     rescale momenta of particles
            ...
            if(pnew(4,j).ne.pnew(4,j))then
               write(*,*)'NaN in energy (3) ',pnew(4,j),j,pxi1,
     $              pxi2,xnew(4,j),xnew(1,j),nexit,theta3,phi3,pabs
               stop
            endif
...
c     rotate in the from z-axis to th&phi given by angdis 
     ...
     &                     pnew(1,j),pnew(2,j),pnew(3,j),pnew(4,j))
            ...
            if(pnew(4,j).ne.pnew(4,j))then
               write(*,*)'NaN in energy (7) ',pnew(4,j),j,
     $              phi1
               stop
            endif              
...
c     decays should not be rotated
     ...
     &                     pnew(1,j),pnew(2,j),pnew(3,j),pnew(4,j))
            ...
            if(pnew(4,j).ne.pnew(4,j))then
               write(*,*)'NaN in energy (8) ',pnew(4,j),j,
     $              phi2,betax,betay,betaz
               stop
            endif
...
c end of delay-if
            if(pnew(4,j).ne.pnew(4,j))then
               write(*,*)'NaN in energy (4) ',pnew(4,j),j,
     $              phi2
               stop
            endif
...
c     write momenta to global arrays
         ...
         pz(inew(i))=pnew(3,itmp(i))
            ...
            if(pnew(4,itmp(i)).ne.pnew(4,itmp(i)))then
               write(*,*)'NaN in energy (5) ',pnew(4,itmp(i)),
     $              itmp(i),p0(inew(i))
               stop
            endif
            if(p0(inew(i)).ne.p0(inew(i)))then
              write(*,*)'NaN in energy (6) ',pnew(4,itmp(i)),
     $              itmp(i),p0(inew(i))
               stop
            endif
...
c      ivirtual=1 ! 1= virtual scattering is active, otherwise not

      if(ivirtual.eq.1)then
         if(iline.eq.40)then
            if(abs(ityp(ind1)).lt.133)then
               call saveinfo(ind1,-1,1)
            endif
            if(abs(ityp(ind2)).lt.133)then
               call saveinfo(ind2,-2,1)
            endif
         endif
...
c     call to Pauli-Blocker
            if(paulibl(inew(i),colldens,-1000d0)) then
...
c     get the baryon density at the interaction point, colldens
      if(dnscall) dnsdum=paulibl(inew(1),colldens,-1000d0)
