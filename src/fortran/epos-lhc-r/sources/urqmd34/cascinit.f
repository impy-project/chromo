c $Id: cascinit.f,v 1.13 2005/03/24 16:46:58 bleicher Exp $
      ...
      real*8 R,P,R2,ranff,sint,phi,nucrad,cost,drr
...
18    continue
      ...
      izcnt=0
      do 12 j=1,A
         P = PT_pmax(j,nucleus)*ranff(0)**(1./3.)
         cost = 1.-2.*ranff(0)
         sint = sqrt(1.-cost**2)
         phi = 2.*Pi*ranff(0)
...
cccccCcc1ccccccccc2ccccccccc3ccccccccc4ccccccccc5ccccccccc6ccccccccc7cc
      ...
      real*8 zmin,zmax,ranff,a,rr,yf,z,nucrad
      integer AA
      parameter (a=0.54d0)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
 ...
 1      zuk=ranff(0)*cr1-1.
        if(zuk.le.0.)then
          tt=rad*(ranff(0)**.3333-1.)
        elseif(zuk.le.cr2 )then
          tt=-log(ranff(0))
        elseif(zuk.lt.cr3 )then
          tt=-log(ranff(0))-log(ranff(0))
        else
          tt=-log(ranff(0))-log(ranff(0))-log(ranff(0))
        endif
        if(ranff(0).gt.1./(1.+exp(-abs(tt))))goto 1
        rim=tt+rad
        z=rim*(2D0*ranff(0)-1D0)
        rim=dsqrt(rim*rim-z*z)

        PT_r0(I,JJ)=0d0
        PT_rz(I,JJ)=Z * AM

 2      s1=2d0*ranff(0)-1d0
        s2=2d0*ranff(0)-1d0
        s3=s1*s1+s2*s2
        if(s3.gt.1d0)goto 2
        s3=dsqrt(s3)
