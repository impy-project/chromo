...
c     shrink collision tables in case 80% load to counter possible overflow
      if(dble(nct)/ncollmax.ge.0.8) then
         if(nct.ge.ncollmax)then
            write(*,*)"ERROR : increase ncollmax",nct,ncollmax,actcol
            stop'16072014'
         endif
         if(actcol.eq.0.and.nct.le.ncollmax)then
            write(*,*)"getnext warning, already large nct in beginning"
     $           ,nct, ncollmax, actcol
         else 
            call collshrink
         endif
...
c                  print *,'nxtcoll',i,j,dst2,tcoll,tmin,sigt,colfac
c
...
c                  print *,'coll',i,j,dst2,tcoll,tmin,sigt,colfac
c     *   ,(sigt/CTParam(67)).gt.max(1.d-8,(10.d0*pi*dst2))
c            write(*,'(a,i6,4f9.2,i6)')
c     .    ' i',i,tform(i),rx(i),ry(i)
c     .           ,rz(i),pdgid(ityp(i),iso3(i))
c            write(*,'(a,i6,4f9.2,i6)')
c     .    ' j',j,tform(j),rx(j),ry(j)
c     .           ,rz(j),pdgid(ityp(j),iso3(j))
...
c     this collision is to beat, now
 ...
 101     continue
         if (imin.gt.0) then
c            print *,''
c       write(*,*) 'before colload: acttime, tmin',acttime,tmin,imin,nct      
c            print *,''
