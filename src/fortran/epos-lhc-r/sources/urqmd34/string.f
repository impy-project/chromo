...
ctp       if(abs(smem).gt.1.0D-5)then
ctp        write(*,*)'! stringdec: energy difference=',smem 
ctp        write(*,*)'ifa,ifb,smass=',ifa,ifb,smass
ctp       endif
       return
...
c.. nrep=number of tries to break string
      ...
      LRET=.TRUE.
ctp      write(*,*)'!! STRING: no fragmentation.'
      return
102   CONTINUE
      IFL(1)=IFL1
      IFL(2)=IFL2
...
c.. suppr. double strange di-quarks(ss) with certain prob. (acc. to ctp 29)
     ...
     &     .and.ranff(0).gt.CTParam(29))goto 140
...
c... single strange di-quark suppresion: 
     ...
     &  .and.ranff(0).gt.CTParam(49))goto 140
c
...
c..quark-flip included (to describe some phi data) 
      if(CTParam(5).gt.ranff(0).and.mod(ifln,100).ne.0.and.
...
c.. cto 21 chooses the fragmentation fct.
      if ((.not.leading).or.(ifln.eq.3)) then
         affm=ctparam(47)
         bffm=ctparam(48)
 5108  ZFRAGS=ZMIN+ranff(0)*(ZMAX-ZMIN)
       YF=((1d0-ZFRAGS)**bffm*(bffm+1)*affm+1-affm)/3d0
       if(yf.gt.1d0.or.yf.lt.0d0)then 
ctp        write(6,*)'ZFRAGS: wrong norm:',yf,zmin,zmax,zfrags
       end if
       IF(ranff(0).LE.YF) RETURN
       GO TO 5108
      endif
...
c.. suppress low momentum particles
 ...
 108  ZFRAGS=ZMIN+ranff(0)*(ZMAX-ZMIN)
c.. gaussian distr.
...
c..lund-fragmentation fct.
 1008 ZFRAGS=ZMIN+ranff(0)*(ZMAX-ZMIN)
...
c.. kaidalov's fragmentation fct. 
      ...
      GO TO(1,2,3,4),ID2
C  UU-TRAJECTORY
 1    ZFRAGS=ZMIN+ranff(0)*(ZMAX-ZMIN)
      YF=(1.0-ZFRAGS)**(ALFT-ARHO)
      IF(ranff(0).LE.YF) RETURN
      GO TO  1
C DD-TRAJECTORY
 2    ZFRAGS=ZMIN+ranff(0)*(ZMAX-ZMIN)
      YF=(1.-ZFRAGS)**(ALFT-ARHO)
      IF(ranff(0).LE.YF) RETURN
      GO TO  2
C SS-TRAJECTORY
 3    ZFRAGS=ZMIN+ranff(0)*(ZMAX-ZMIN)
      YF=(1.-ZFRAGS)**(ALFT-APHI)
      IF(ranff(0).LE.YF) RETURN
      GO TO  3
C CC-TRAJECTORY
 4     ZFRAGS=ZMIN+ranff(0)*(ZMAX-ZMIN)
...
C UUUU-TRAJECTORY
16    ZFRAGS=ZMIN+ranff(0)*(ZMAX-ZMIN)
      YF=(1.-ZFRAGS)**(ALFT-(2.*AN-ARHO))
      IF(ranff(0).LE.YF) RETURN
      GO TO 16
C UDUD-TRAJECTORY
17    ZFRAGS=ZMIN+ranff(0)*(ZMAX-ZMIN)
      YF=(1.-ZFRAGS)**(ALFT-(2.*AN-ARHO))
      IF(ranff(0).LE.YF) RETURN
      GO TO 17
C USUS-TRAJECTORY
18    ZFRAGS=ZMIN+ranff(0)*(ZMAX-ZMIN)
      YF=(1.-ZFRAGS)**(ALFT-(2.*ALA-ARHO))
      IF(ranff(0).LE.YF) RETURN
      GO TO 18
C UCUC-TRAJECTORY
19    ZFRAGS=ZMIN+ranff(0)*(ZMAX-ZMIN)
      YF=(1.-ZFRAGS)**(ALFT-(2.*ALAC-ARHO))
      IF(ranff(0).LE.YF) RETURN
      GO TO 19
C DDDD-TRAJECTORY
20    ZFRAGS=ZMIN+ranff(0)*(ZMAX-ZMIN)
      YF=(1.-ZFRAGS)**(ALFT-(2.*AN-ARHO))
      IF(ranff(0).LE.YF) RETURN
      GO TO 16
C DSDS-TRAJECTORY
21    ZFRAGS=ZMIN+ranff(0)*(ZMAX-ZMIN)
      YF=(1.-ZFRAGS)**(ALFT-(2.*ALA-ARHO))
      IF(ranff(0).LE.YF) RETURN
      GO TO 21
C DCDC-TRAJECTORY
22    ZFRAGS=ZMIN+ranff(0)*(ZMAX-ZMIN)
      YF=(1.-ZFRAGS)**(ALFT-(2.*ALAC-ARHO))
      IF(ranff(0).LE.YF) RETURN
      GO TO 22
C SSSS-TRAJECTORY
23    ZFRAGS=ZMIN+ranff(0)*(ZMAX-ZMIN)
      YF=(1.-ZFRAGS)**(ALFT-(2.*AKSI-ARHO))
      IF(ranff(0).LE.YF) RETURN
      GO TO 23
C SCSC-TRAJECTORY
24    ZFRAGS=ZMIN+ranff(0)*(ZMAX-ZMIN)
      YF=(1.-ZFRAGS)**(ALFT-(2.*AUSC-ARHO))
      IF(ranff(0).LE.YF) RETURN
      GO TO 24
C CCCC-BARYON
25    ZFRAGS=ZMIN+ranff(0)*(ZMAX-ZMIN)
...
ctp      write(6,*)'! amass: no mass for part.id:',id
      return
      END
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
 ...
 1    continue
      ix=int(mini+int((maxi-mini+1)*ranff(0)))
      y=ranff(0)
      if (y.lt.p(ix)) then 
        it=ix
        return
