...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real*8 sqrts, m1, m2, costh, costhcoll, phi, ranff, pi, s
      real*8 anginter
      integer iline
      logical symlog(64)
...
cbl only for intermediate masses, otherwise no deflection (zero degree scattering)
      ...
      implicit none 
      real*8 costhcoll, s, m1, m2, x, dct, ct, dsigma, ranff
      integer j
      logical sym
...
c
      costhcoll = costhcoll+ranff(0)*dct
      return
      end
...
c inverse transform method is used:
      ...
      anginter=1d0/a*log( ranff(0)*(exp(a)-exp(-a))+exp(-a) )
