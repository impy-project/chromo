...
c          configuration space 
         r0(i)=0.d0     
         rx(i)=lboxhalbe*(1-2*ranff(0)) 
         ry(i)=lboxhalbe*(1-2*ranff(0)) 
         rz(i)=lboxhalbe*(1-2*ranff(0))                  
...
c loop over all particles
         do 60 i=npart+1,npart+bptpart(ibox)
            P=bptpmax(ibox)/bptpart(ibox)*ranff(0)**(1./3.)
