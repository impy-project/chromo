...
c-----------------------------------------------------------------------
     ...
     *,wzerox,rrr,eps,xprem,xmrem,om1intgck,drangen,xmin,xmax!,xzc
      parameter(eps=1.d-20)
      common/col3/ncol,kolpt
c      logical modu
      common/cems5/plc,s
      double precision xp,xm,anip,anit,s,px,py,pomass,plc,at!,PhiExpo
...
c     ------
      ...
      kint=int(2.*engy**0.25)
c      kint=kint*int(1+log(float(min(500,koll))))
c      if(koll.gt.500)kint=500*kint/koll
      kint=max(15,kint)
      if(koll.gt.50)kint=3*kint/int(log(float(koll)))
      kmcmx=nSprmx*kint!*10        !50*kint  !100*kint
c      print *,"---->",kmcmx,nSprmx,kint,koll,log(float(koll))
...
c            do n=1,nprmx(k)
c              if(xpr(n,k).gt.xzcutpar(k))itpr(k)=1 !for nuclear splitting
c            enddo
c          endif
          ip=iproj(k)
          it=itarg(k)
...
c      if(isplit.eq.1.and.ncol.gt.0.and.zrminc.gt.0.)then
c
c        if (iLHC.eq.1)then  !make random selection to avoid assymetry
c          
c          nishuff(1)=0
c          nishuff(2)=0
c          do ip=1,maproj
c            nishuff(1)=nishuff(1)+1
c            ishuff(nishuff(1),1)=ip 
c          enddo
c          do it=1,matarg
c            nishuff(2)=nishuff(2)+1
c            ishuff(nishuff(2),2)=it 
c          enddo
c
c          do while(nishuff(1)+nishuff(2).gt.0)
c
cc random selection
c            if(nishuff(1).gt.0.and.nishuff(2).gt.0)then
c              ir=1+int(rangen()+0.5)
c            elseif(nishuff(1).gt.0)then
c              ir=1
c            else
c              ir=2
c            endif
c
c            indx=1+int(rangen()*float(nishuff(ir)))
c            if(ir.eq.1)then
c              ip=ishuff(indx,ir)
c              if(lproj3(ip).ne.0.and.kolp(ip).eq.0)call ProNucSpl( 1,ip)
c            else
c              it=ishuff(indx,ir)
c              if(ltarg3(it).ne.0.and.kolt(it).eq.0)call ProNucSpl(-1,it)
c            endif
c            ishuff(indx,ir)=ishuff(nishuff(ir),ir)
c            nishuff(ir)=nishuff(ir)-1
c
c          enddo
c
c        else
...
c        endif
c
c        if(ish.ge.6)call XPrint('After ProNucSpl:&')
c
c      endif
...
c -- Fix Pomeron type ---
      ...
      do k=1,koll
        ...
        do n=1,nprmx(k)
          ...
          if(idpr(n,k).eq.3.and.xpr(n,k).gt.1d-4)then
            xkolhp(ip)=xkolhp(ip)+1d0 !number of hard Pomerons
            xkolht(it)=xkolht(it)+1d0 !on remnants
          endif
        enddo
      enddo
...
c          ymean=0.
          do n=1,nprmx(k)
            if(idpr(n,k).eq.-1)then
              idpr(n,k)=1
c              xzc=xzcutpar(k)
c              if(ncol.eq.1.and.ichargex.eq.1.and..not.chargex)xzc=1.d0 !replace Diff Pom by Pomeron exchange when possible
c              if(xpr(n,k).le.xzc)then
                call VirPom(k,n,0)
                if(iep(ip).eq.0.and.iet(it).eq.0.and.nprt(k).eq.0)then !central diffraction (CD)
                  xp=0d0
                  xm=0d0
                  !Generate proper x+ and x- following diff mass
                  xmin=ammn(2)**2/s
                  xmax=min(max(min(10d0*xmin,1d0),0.05d0  !energy taken never larger than 5% of remnant energy (see ProReM)
     &                      *(1d0+drangen(xmin))**2),1d0) !fix the tail of the diffractive peak
                  if(xmax.gt.xmin)then
                    alp=at(idp(ip),2)
                    rr=drangen(xmin)
                    if(dabs(alp-1.d0).lt.eps)then
                      xp=xmax**rr*xmin**(1d0-rr)
                    else
                      xp=(rr*xmax**(1d0-alp)+(1d0-rr)*xmin**(1d0-alp))
     &                     **(1d0/(1d0-alp))
                    endif
                    alp=at(idt(it),2)
                    rr=drangen(xmin)
                    if(dabs(alp-1.d0).lt.eps)then
                      xm=xmax**rr*xmin**(1d0-rr)
                    else
                      xm=(rr*xmax**(1d0-alp)+(1d0-rr)*xmin**(1d0-alp))
     &                     **(1d0/(1d0-alp))
                    endif
                  endif
                  xpr(n,k)=xp*xm
                  if(xpr(n,k)*s.gt.dble(cumpom)**2)then
                    itpr(k)=2
                    ivpr(n,k)=1
                    nvpr(n,k)=0
                    idpr(n,k)=1 !2 string
                    if(rangen().lt.cumpom/sngl(xpr(n,k)*s))idpr(n,k)=2 !simplest one string
                    npp(ip)=npp(ip)+1
                    npt(it)=npt(it)+1
                    npr(0,k)=npr(0,k)-1
...
c --- Count real interactions ---
      ...
      enddo
      if(ish.ge.5)write(ifch,*)'ncol,ncold:',ncol,ncold


c --- fix all variables


c      if(ish.ge.4)write(ifch,*)'fix all variables'
...
c      if(irzptn.eq.1)call recalcZPtn
      ...
      if(maproj+matarg.eq.2)then     !pp
        if(itpr(1).ne.0)then
          if(.not.chargex)anintine=anintine+1.
          if(itpr(1).gt.0)then
            if(ionudi.eq.1
     ...
     &             (iet(1).eq.0.and.iep(1).eq.2))anintsdif=anintsdif+1.
                if(iep(1).eq.2.and.iet(1).eq.2)anintddif=anintddif+1.
              endif
        ...
        if(aidif.gt.0.)then
          if(.not.chargex)anintdiff=anintdiff+1.
          if(.not.chargex)anintine=anintine+1.
          if(aidifp.gt.0.5.and.aidift.le.0.5)then
            if(.not.chargex)anintsdif=anintsdif+1.
            typevt=4                        !SD pro
          endif
      ...
      if(ish.ge.4)write(ifch,*)'Typevt: ',typevt
...
c --- Plot MC pomeron number ---
      ...
      if(nemsi.le.4.and.irea.ge.0.and..not.chargex)then
      ...
      if(iemsb.eq.1.and..not.chargex)then        ! plot
        do k=1,koll
...
c     --- decide for charge exchange
      if(ichargex.eq.1.and..not.chargex)then
        if(ncol.eq.1.and.ncold.eq.ncol)then
c         if only one diffractive pair use fix probability to activate charge exchange
          ip=iproj(kchargex)
          it=itarg(kchargex)
          if(rangen().lt.xschargex(0))then
            if(ish.ge.2)write(ifch,*)'charge exchange !'
c            if((iep(1).eq.0.and.iet(1).eq.2).or.
c     &           (iet(1).eq.0.and.iep(1).eq.2))anintsdif=anintsdif-1.
c            if(iep(1).eq.2.and.iet(1).eq.2)anintddif=anintddif-1.
            iret=-111
            goto 1000
          endif
        endif
...
c --- Check minimum mass ---
      ...
      do k=1,koll
        ...
        it=itarg(k)
      do n=1,nprmx(k)
        if(idpr(n,k).ne.0.and.xpr(n,k)*s.lt.dble(cumpom)**2)then
          nnb=nbkpr(n,k)
          nnv=nvpr(n,k)
          if(nnv.ne.0)then
...
c --- Fix resonance production in remnants (important for LHCf data and be carefull with ratio diff/incl of gamma (exclude forward (diff) pi0 prod)
      if(rexpdif(iclpro).gt.0.)then
        do ip=1,maproj
          if(iep(ip).eq.2)call ProReEx( 2,ip)
c          if(iep(ip).eq.1.or.iep(ip).eq.2)call ProReEx( 2,ip)    !defavored by LHCf data
        enddo
      endif
      if(rexpdif(icltar).gt.0.)then
        do it=1,matarg
          if(iet(it).eq.2)call ProReEx( -2,it)
c          if(iet(it).eq.1.or.iet(it).eq.2)call ProReEx( -2,it)    !defavored by LHCf data
        enddo
      endif
...
c random selection
        ...
        if(iret.eq.1)then
          ...
          if(ntry.lt.30)then
            ...
            iret=0
            goto 123
          elseif(ntry.ge.30.and.maproj.le.20.and.matarg.le.20)then
c     .                                       .and.engy.gt.5.)then
...
c     --- decide for Pomeron exchange via emscharge
      if(.not.chargex.and.iomega.ne.2)then
        if(ncol.eq.1.and.ncold.eq.ncol.and.typevt.ne.1.and.iok.eq.0)then
          k=kchargex
          ip=iproj(k)
          it=itarg(k)
c          print *,iep(ip),iet(it),pptl(5,npproj(ip)),pptl(5,nptarg(it))
c     ,         ,typevt,nprt(1)
          iret0=100+10*iep(ip)+iet(it)
          p5max=0.
          p4dif=0.
          if(iret0.eq.100)then
            do n=1,nprmx(k)
              if(idpr(n,k).eq.2)then
                p5max=max(p5max,pptl(5,nppr(n,k)))
                p4dif=min(0.5*engy-pptl(4,npproj(ip))
     .                   ,0.5*engy-pptl(4,nptarg(it)))
              endif
            enddo
c          elseif(pptl(5,npproj(ip)).gt.pptl(5,nptarg(it)))then
          elseif(rangen().gt.0.5)then
            p5max=pptl(5,npproj(ip))
            p4dif=0.5*engy-pptl(4,nptarg(it))
          else
            p5max=pptl(5,nptarg(it))
            p4dif=0.5*engy-pptl(4,npproj(ip))
          endif
          if(ish.ge.5)write(ifch,*)'Pomeron exchange :',p4dif,p5max
     .                                         ,p4dif.gt.1d0+egymin
          if(p4dif.gt.1d0+egymin)then    !enough energy to produce a rho0-N interaction
            x5max=2.*p5max/engy
            proba=max(2.*egymin/engy,xmindiff)/x5max
c            proba=2.*egymin/engy/x5max
            if(rangen().gt.proba)then
c             if(iret0.eq.100.and.rangen().gt.xmindiff/p5max)then
c              print *,'hard diff'
              iret=-iret0
              goto 1000
            endif
...
c     --- Grey spectators momentum

c        if(maproj.gt.1.or.matarg.gt.1.and.ihacas.eq.0)then
...
c      if(abs(iokoll).le.0)then
c        ihit=0
c        esux=0.
c        idfx=0
c      do loo=1,2
c      einit=maproj*engy/2+matarg*engy/2
c      esu=0
c      do i=1,nptl
c        if(mod(istptl(i),10).eq.0)then
c          esu=esu+pptl(4,i)
c        endif
c      enddo
c      idf=abs(esu-einit)/einit*1000
c      if(idf.gt.10.or.iretshu.eq.10.or.ihit.eq.1)then
c        if(loo.eq.2)then
c          ncount=ncount+1 
c          if((ncount.eq.1.or.ncount.eq.10.or.ncount.eq.100
c     .      .or.ncount.eq.1000).and.ish.ge.1)then
c            lcount=log10(1.*ncount)+1
c            write(ifmt,'(a,i1,a,3x,3f10.1,2i6,i6)')
c     .      'WARNING ',lcount,' ems Energy check'
c     .      ,einit,esux,esu,idfx,idf,iretshu
c          endif
c          if(ish.ge.2)then
c            write(ifch,'(a,5x,3f10.1,2i6,2i7)')
c     .    'ems - energy check',einit,esux,esu,idfx,idf,iretshu,nrevt
c            call blist('list after MC&',1,nptl)
c          endif
c        endif
c        if(loo.eq.1.and.irescl.gt.0)then
c          call utrescxx(iret,0) 
c          !call utresc(iret,2.0)  !taking the default 1.02 will reject almost all
c                                 !events for AuAu at 7 GeV
c                                 !With 2.0 doable, but still many rejections
c          !*****************************************************
c          ! IMPORTANT when updating utresc:
c          !*****************************************************
c          ! In subroutine utresc: errlim changed to errlim=min(0.005,...)
c          !   otherwise too big error accepted at low energies
c          !*****************************************************
c          ! VERY IMPORTANT: In case of many rejections, a bias is introduced,
c          !  pretty much uncontrolled (probably high Npom is more affected etc)
c          !    SHOULD BE AVOIDED (to be discussed!!!)
c          !*****************************************************
c          if(iret.gt.0)then
c            iret=1
c            goto 1000
...
c     ----
       ...
       if(iemspx.eq.1.and..not.chargex)then
       do ko=1,koll
...
c-------------------------------------------------------------------------
      ...
      double precision ww,w0,w1,w2,w3,w4,w5,w6,w(-1:7),aks,eps!,zdiff
     *,xh,yp!,xp,xm
      parameter(eps=1.d-10)
      logical cont
...
c      xm=xmpr(n,k)
      ...
      idpr(n,k)=1
        ...
        if(w(6).gt.0.d0)w6=w(6)

        ww=w1+w2+w3+w4   
        if(ish.ge.4)then
          write(ifch,*)'ProPoTy:ww_i'
     *         ,w(-1),w0/w(-1),w5/w(-1),w6/w(-1),ww
          if(ww.gt.0d0)write(ifch,*)'            ',w1/ww*100.d0
     *         ,w2/ww*100.d0,w3/ww*100.d0,w4/ww*100.d0
        endif


        aks=dble(rangen())*w(-1)  !w(-1) is the cross-section of Pomeron production and each w(i) are individual cross-sections. The diff xs is known absolutely, the semi-hard one are just relative and the rest is given to soft.


        if(aks.le.w5)then !diffractive interaction

          itpr(k)=itpr(k)+2
cc the probability to have a real diffractive Pomeron increase with Z and hard P
cc          zdiff=exp(-min(50.d0,dble(zdfinc*(zparpro(k)+zpartar(k)))*w1))
c          zdiff=1d0/sqrt(1d0+dble(zdfinc*(zparpro(k)+zpartar(k)))*w1)
c          if(ncol.eq.1.and.ichargex.eq.1.and..not.chargex)zdiff=1. !replace Diff Pom by Pomeron exchange when possible
cc          print *,xpr(n,k),zdiff,w1,xpr(n,k).gt.xzcutpar(k)!,zdiff2
c          if(ish.ge.5)write(ifch,*)'ProPoTy:itpr',itpr(k),zdiff
c          if(xpr(n,k).gt.xzcutpar(k).and.rangen().gt.zdiff)then
cc          if(rangen().gt.zdiff)then
cc High mass diffraction : Keep pomeron and choose between soft and semi-hard
c            aks=dble(rangen())*(w0+w1)
c            if(aks.gt.w0)then
c              idpr(n,k)=3
c              npr(3,k)=npr(3,k)+1
c              npr(1,k)=npr(1,k)-1
c              bhpr(n,k)=bk(k)
c              idhpr(n,k)=0
cc            elseif(iLHC.eq.1)then
cc              idpr(n,k)=-1
c            endif
c            if(ish.ge.5)write(ifch,*)'ProPoTy:idpr diff',idpr(n,k)
c          else
c         if(iLHC.eq.1)then     !LHC tune
          
c keep soft Pomeron for later 
          idpr(n,k)=-1
c hard diffraction now fixed in ems (real Pom exchange if only one Pom ... no diffractive Pom otherwise (see WomTy))
          
c           else                     !original CR
cc Low mass diffraction : no pomeron
...
c                  if(xpp(ipp)-1d0.ge.-1d-10)iep(ipp)=0
...
c                  if(xmt(itt)-1d0.ge.-1d-10)iet(itt)=0
...
c            call RemPom(k,n)
c            idfpr(n,k)=0
c            npr(0,k)=npr(0,k)+1 !nr of empty cells
c            kolp(ip)=kolp(ip)-1 !suppress diffractive collision from the remnant
c            kolt(it)=kolt(it)-1 !it will be restored if the pair is diffractive
c            if(ish.ge.6)write(ifch,*)'ProPoTy:idpr',idpr(n,k)
c          endif
c
c         endif

        elseif(ww.gt.eps.and.w6.gt.eps.and.aks.le.w6)then !semi-hard
          ...
          aks=(aks-w5)/w6
          if(w2.gt.eps.and.aks.le.w2/ww)then       !qg-pomeron
            idhpr(n,k)=1
            ivp(ip)=ivp(ip)-1
          elseif(w3.gt.eps.and.aks.le.(w2+w3)/ww)then    !gq-pomeron
            idhpr(n,k)=2
            ivt(it)=ivt(it)-1
          elseif(w4.gt.eps.and.aks.le.(w2+w3+w4)/ww)then !qq-pomeron
            idhpr(n,k)=3
            ivp(ip)=ivp(ip)-1
            ivt(it)=ivt(it)-1
          else                !gg-pomeron
            idhpr(n,k)=0
c            call utstop('ems-unknown pomeron&')
          endif
...
c increase of inelastic remnant excitation in pA needed for Barton and NA49 pc
c            if(r.lt.1.-(1.-rexndi(iclpro))!)then
c     &         **(1.+rexres(iclpro)*float(max(0,npp(ip)-1))))then
            if(r.lt.rexndi(iclpro).or.npp(ip).gt.1)then
              iep(ip)=1
            else
              iep(ip)=0
            endif
...
c increase of diffractive remnant excitation in pA needed for Barton
     ...
     &        **(1.+rexres(iclpro)*float(max(0,lproj(ip)-1))))then
              iep(ip)=2
            else
              iep(ip)=0
            endif
          else
            if(r.lt.rexdif(iclpro))then
              iep(ip)=2
            else
              iep(ip)=0
            endif
...
c for non-excited spectators
     ...
     &          *(1.+rexres(icltar)*log(max(1.,float(ltarg(it)))))))then
          ...
          elseif(iremn.ne.0)then
c increase of inelastic remnant excitation in pA needed for Barton and and NA49 pc
c            if(r.lt.1.-(1.-rexndi(icltar))!)then
c     &         **(1.+rexres(icltar)*float(max(0,npt(it)-1))))then
            if(r.lt.rexndi(icltar).or.npt(it).gt.1)then
              iet(it)=1
            else
              iet(it)=0
            endif
...
c increase of diffractive remnant excitation in pA needed for Barton anb xsection
     ...
     &      **(1.+rexres(icltar)*float(max(0,ltarg(it)-1))))then
                iet(it)=2
              else
                iet(it)=0
              endif
            else
              if(r.lt.rexdif(icltar))then
                iet(it)=2
              else
                iet(it)=0
              endif
      ...
      elseif(ir.eq.2)then                !proj diff excitation (important for NA49 pp and from NA49 pc better not to reduce probability in pA)
c minimum mass excitation
        ip=ii
        r=rangen()
        if(r.lt.rexpdif(iclpro))iep(ip)=4  
c        if(r.lt.rexpdif(iclpro)
c     & **(1.+rexres(iclpro)*float(max(0,lproj(ip)-1))))iep(ip)=4  !proba of light diff should decrease with nuclei
c        if(r.lt.1.-(1.-rexpdif(iclpro))
c     &        **(1.+rexres(iclpro)*float(lproj(ip)-1)))iep(ip)=4  

      elseif(ir.eq.-2)then                !targ diff excitation   (important for NA49 pp and from NA49 pc better not to reduce probability in pA)

cminimum mass excitation
        it=ii
        r=rangen()
        if(r.lt.rexpdif(icltar))iet(it)=4  
c        if(r.lt.rexpdif(icltar)
c     & **(1.+rexres(icltar)*float(max(0,ltarg(it)-1))))iet(it)=4    !proba of light diff should decrease with nuclei
c        if(r.lt.1.-(1.-rexpdif(icltar))
c     &      **(1.+rexres(icltar)*float(ltarg(it)-1)))iet(it)=4 
...
c     Nucleons in active pairs without interaction are used to mimic particles
c     (mostly nucleons) produced during intranuclear cascade which are not
c     treated explicitely in EPOS. In addition the activited nucleons (with iept=6) are
c     used to help to provide mass to other particles. Their number should
c     be relatively constant with energy to reproduce the data and not to
c     change the leading nuclear fragment too much.
c-------------------------------------------------------------------------
...
c      print *,koll,ncol,sqrt(float(mapro+matarg))/koll

c generate p_t for diffractive
      if(iqq.eq.1)then
c        print *,ncol,koll,0.4*(1.-(0.4+0.5/float(ncol)))
c     .       ,0.06/log(engy)*float(max(maproj,matarg))
c     .  *(1.-(0.4+0.5/float(ncol)))/float(min(maproj,matarg))**0.75
        xxe(k)=0d0
        xye(k)=0d0
        if(ptdiff.gt.0.)then
          if(itpr(k).eq.2)then
            pt=ranpt()*ptdiff/(1.+0.02*max(0.,sngl(log(s))))
          elseif(itpr(k).eq.0!.and.ihacas.eq.0
     .       .and.((kolp(ip).eq.0.and.iep(ip).le.0)
     .           .or.(kolt(it).eq.0.and.iet(it).le.0)))then
c Probability to create a free nucleon, usefull in ProRem to avoid problem in utrescl and creates grey particles (which mimic intranuclear cascade, which has a constant yield with energy) and leading mass fragment with lower mass
            Pgrey=min(facgrey*float(koll)/log(engy)
     .                       /(float(min(maproj,matarg)))**0.66,0.9)
c            Pgrey=0.4*(1.-(0.4+0.5/float(ncol)))
c            Pgrey=0.06/log(engy)*float(max(maproj,matarg))
c     .          *(1.-(0.4+0.5/float(ncol)))
            if(rangen().lt.Pgrey)then
              iret=-1
c pt for non-wounded nucleon
              ptnw = 0.35
              pt = ranptk()*ptnw
...
c-----------------------------------------------------------------------
     ...
     *,xmn1,xmn2
      ...
      if((idpr(n,k).ne.1.and.idpr(n,k).ne.2).or.ivpr(n,k).eq.0)return
     ...
     *call utstop('no Pomeron in ProSex&')
      ...
      if(idpr(n,k).eq.2)then    !low mass CD (1 string)
...
c cancel quark and antiquarks to avoid too much remnant excitation
        do i=1,nrflav
...
c update valence quarks (cancel first sea quarks)
          ...
          itest1=itest1+jcpref(i,1,ir)
          itest2=itest2+jcpref(i,2,ir)
          do j=1,2
            jc2(i,j)=jcpref(i,j,ir)
          enddo
        enddo
        if(itest1.eq.0.and.itest2.eq.0)then !do not leave empty remnant
          kmax=1
          if(abs(idproj).gt.1000.and.kolp(ir).gt.1)kmax=2 !create q-qb 2 times to avoid fast forward meson for nucleons (LHCf)
          do k=1,kmax
            idum=idrafl(iclpro,jc2,1,'r',3,iretso) 
            do j=1,2
              do i=1,nrflav
...
c update valence quarks (cancel first sea quarks)
          ...
          itest1=itest1+jctref(i,1,ir)
          itest2=itest2+jctref(i,2,ir)
          do j=1,2
        ...
        if(itest1.eq.0.and.itest2.eq.0)then !do not leave empty remnant
          kmax=1
          if(abs(idtarg).gt.1000.and.kolt(ir).gt.1)kmax=2  !create q-qb 2 times to avoid fast forward meson for nucleons (LHCf)
          do k=1,kmax
            idum=idrafl(icltar,jc2,1,'r',3,iretso)
            do j=1,2
              do i=1,nrflav
        ...
        if(ir.eq.1)then
          ...
          if(lproj3(m).ge.1)then
            do l=1,lproj3(m)
              ...
              absb=max(1.e-9,bk(kpair))
              b2=absb*absb
              zzdrop(m,1)=zzdrop(m,1)+max(zpar,fegypp*exp(-b2/b2xscr))
             enddo
          endif
          zzremn(m,1)=zz
       elseif(ir.eq.-1)then
          ...
          if(ltarg3(m).ge.1)then
            do l=1,ltarg3(m)
              ...
              absb=max(1.e-9,bk(kpair))
              b2=absb*absb
              zzdrop(m,2)=zzdrop(m,2)+max(zpar,fegypp*exp(-b2/b2xscr))
            enddo
          endif
          zzremn(m,2)=zz
        else
        ...
        if(ir.eq.1) zzdrop(m,1)=0
        if(ir.eq.-1)zzdrop(m,2)=0
      endif
      end
...
c  sum of Z of remnant itt linked to ip (high if connected to many other remn)
                 zpaptl(2,is1)=0.5*(xkolhp(ip)+xkolht(it)) !amtot !float(nprt(kkk)) !float(lproj(ip))
c                 zpaptl(2,is1)=0.
...
c  sum of Z of remnant ipp linked to it (high if connected to many other remn)
                 zpaptl(2,is2)=0.5*(xkolhp(ip)+xkolht(it)) !float(nprt(kkk)) !float(ltarg(it))
c                 zpaptl(2,is2)=0.
...
c initial definitions
      ...
      if(ir.eq.1)then
        ...
        zz=1.
c        if(iez(irem,jrem).eq.3.or.iez(irem,jrem).eq.5)
c     &       zz=zz+log(xkcol)*zmsinc
c        print *,jrem,iez(irem,jrem),npp(irem),xkcol,zz
c     &       zz=zz+zzremn(irem,1)*zmsinc
      elseif(ir.eq.-1)then
        cremn='proj'
        jrem=2
        jremo=1
        xkcol=max(xkcol,dble((kolt(irem)-1)*zmsinc))
...
c        if(iez(irem,jrem).eq.3.or.iez(irem,jrem).eq.5)
c     &       zz=zz+log(xkcol)*zmsinc
c        print *,jrem,iez(irem,jrem),npt(irem),xkcol,zz
c     &  zz=zz+zzremn(irem,2)*zmsinc
      endif
      mass0=max(1,mass0)
      drop=.false.
c      if(iremn.ge.2.and.(iez(irem,jrem).eq.3.or.iez(irem,jrem).eq.5))
      if(iremn.ge.2.and.iez(irem,jrem).eq.3)
     &   drop=.true.
      excited=.false.
      if(iez(irem,jrem).gt.0.and.iez(irem,jrem).ne.6)excited=.true.
c     &.and.iez(irem,jrem).ne.4)excited=.true.
...
c  fremnux (+) and not fremnux2 (-) which gives a mass too low in case of gethadron where q and aq do not cancel
      ...
      if(excited)then
c        aremn=dble(amremn)
c        aremnex=dble(max(amremn,fremnux(jc)))
       aremn=dble(max(amremn,fremnux(jc)))
       aremnex=aremn
c       if(iremn.eq.2.and.iez(irem,jrem).eq.3)      !droplet
c     &     aremn=dble(max(amremn,fremnux(jc)))
        if(iremn.ge.2)then
          aremnex=aremnex+amemn(idz(irem,jrem),iez(irem,jrem))
c          aremnex=aremnex*xkcol
         if(drop)aremnex=aremnex+delrex*(xkcol-1d0)
        else
          aremnex=aremnex+amemn(idz(irem,jrem),iez(irem,jrem))
        endif
      elseif(iez(irem,jrem).eq.4)then !minimum mass for spectators should be as low as possible
        aremn=dble(amremn)
        aremnex=dble(max(amremn,fremnux(jc)))
      ...
      if(ish.ge.8)
     &     write(ifch,10)ir,irem,mass0,xkcol,icrmn,iez(irem,jrem),force
     &                         ,amremn,fremnux(jc),aremn,aremnex
     &                         ,xpz(irem,jrem),xpt2rem,sx
 10   format('prorem :  ',i3,2i4,g11.3,2i7,i2,L2,/
     &      ,'    mass :',4g13.5,/
     &      ,' x,pt,sx :',3g13.5)
c ntry
...
c xtest = xminus-max,  corresponding mostly to a remnant mass 0.2
      ...
      xtest0=0.d0
c      fxtest=0.5d0*(1d0+drangen(xxx)) !1.d0 !0.3d0
      fxtest=0.1d0*(1d0+9d0*drangen(xxx)**2) !1.d0 !0.3d0  !main change visible in multiplicity
      do k=1,masso
        j=k2j(k)
        cont=.false.
...
c      xmin=1.1*(aremnex**2d0+xxz(irem,jrem)**2d0+xyz(irem,jrem)**2d0)/sx
      xmin0=(aremn**2d0+xpt2rem)/sx
      if(xmin0.ge.1.d0)xmin0=min(xmin0,0.9d0)
      if(ish.ge.1.and.xmin0.ge.1d0)
     &   write(ifch,*)"Warning in ProReM with xmin0 !"

      xmin=xmin0
      if(iez(irem,jrem).eq.4.or.iez(irem,jrem).eq.4)then !resonance or diff, minim should not change
        xmin=1.1d0*(aremnex**2d0+xpt2rem)/sx
      elseif(excited)then
        xmin=dble(xminremn)*(aremnex**2d0+xpt2rem)/sx
      endif

      if(iez(irem,jrem).eq.3)then
        xmxremn=min(max(10d0*xmin,dble(xmxrem)),1d0)
      elseif(iez(irem,jrem).ne.2.and.iez(irem,jrem).ne.6)then
        xmxremn=min(dble(amdrmax)**2/sx+xmin0,1d0)
      else
        xmxremn=min(max(min(10d0*xmin,1d0),0.1d0
     &               *(1d0+drangen(xmin))**2),1d0) !fix the shape of the diffractive peak
      endif
c      print *,iez(irem,jrem),xmxremn,10d0*xmin,xmindiff
       
      xmax=xtest0
c      if(iez(irem,jrem).eq.2)xmax=min(xmax
c     &     ,max(0.05d0,min(0.4d0,dble(amdrmax**2)/sx))) !to avoid ultra high mass remnants (low maximum mass for pions to get better rho0 spectra) ????
                                !(m*<<sqrt(s/(2.m.R/hbar/c)) (2./hbar/c~10) limit due to coherence lenght in diffraction (Feinberg and Pomeranchuk))
c for diffractive remnant, mass should never exceed 5% of the proj or targ energy  ---> but in pratice it doesn't give good results for mult distributions !
...
c        xtmp=max(min(1d0,dble(xmindiff)),xmin)!*drangen(xmin)
        xtmp=min(1d0,xmxremn*dble(mass0)
     &                      *drangen(xmin)**0.05)
c        xtmp=dble(xmindiff)
      elseif(iez(irem,jrem).eq.1.or.iez(irem,jrem).eq.5)then
        xtmp=min(1d0,xmxremn*dble(mass0)
     &                      *drangen(xmin)**0.05)
...
c       xtmp=max(dble(min(1.,xminremn*float(maproj+matarg-1))),xmin)
c     &               *drangen(xmin)
        xtmp=min(1d0,xmxremn*dble(mass0)!*xkcol
     &                         *drangen(xmin)**0.05)
c       xtmp=dble(xminremn)
      else   !4 (resonances)
        xtmp=min(1d0,(1d0/sx+xmin0)*dble(mass0)
     &                      *drangen(xmin)**0.05)
      endif
      endif
      xmax=min(xmax,max(xtmp,xmin))
      if(ish.ge.8)write(ifch,*)'ntry',ntry,xmin,xmax,xtmp
     *                               ,xmax*dble(mass0),xmin0,excited
      if(koll.eq.1)xmax=min(xmax,xpz(iremo1,jremo))
      xxmax=xmax*dble(mass0)-eps
...
!smooth distribution
c        xmin0=xmin+(1d0-exp(-2d0*drangen(xxx)**2))*(xxx-xmin)
      else
        if(mass0.eq.1)ntry=ntrymx   !xxx is fixed so 1 try is enough
c        xmin=dble(amremn)**2d0/sx                !no pt
c        xxx=xmin+xpt2rem/sx                      !no pt
c        xmin=(dble(aremn)**2d0+xpt2rem)/sx
        xxx=xmin
...
c partition xminus between nucleons of the other side
      ...
      do while(ii.gt.0)
        ...
        if(mass0-1.gt.0)then
         xi=xii*dble(rangen())**(1.d0/dble(mass0-1))
        else
         xi=0d0
        endif
        xme(iro)=xxx*(xii-xi)
        ...
        if((xpz(iro,jremo)-xme(iro)).lt.xmmin)then
          if(ish.ge.8)write(ifch,*)'     skip ',cremn,' ',ii,mass0,ntry
     &                      ,iro,xme(iro),xpz(iro,jremo)-xme(iro),xmmin
          xme(iro)=-1.d0
          if(ii.le.1)goto 1
        else
          xii=xi
          if(ish.ge.8)write(ifch,*)'       ok ',cremn,' ',ii,mass0,ntry
...
c      if(idpr(n,k).eq.2)stop'no Reggeons any more'
      ...
      if(idpr(n,k).eq.2)then      !low mass CD
...
c         if(idpr(n,k).eq.3)then   !no diquark for hard Pom because not really connected to remnant
cc           pdd=0.
c           psvv=0.
c           paas=0.
c         endif
...
c      if(idpr(n,k).eq.2)stop'Reggeon'
...
c         check mass string 1
          ...
          if(ic(1).gt.0.or.ic(2).gt.0)then
           am=sngl(pstg(5,1))
           idr=0
           if(idpr(n,k).eq.2)then    !test resonance case for Reggeon
             id=idtra(ic,0,0,0)
...
c-----------------------------------------------------------------------
     ...
     * ,pp(1),pp(2),pp(3),pp(4),21)
       ...
       zpaptl(1,nptl)=0.
       zpaptl(2,nptl)=0.5*(xkolhp(ip)+xkolht(it))
...
c-----------------------------------------------------------------------
      ...
      logical gdrop, ghadr,gproj,gdiq
 ...
 55   idrop=0
      ...
      flow=ainfin                   !to force not to decay here
      mm=0
      if(ir.eq.1)then
...
c        if(kolt(m).le.0)goto1000
      ...
      gdiq=iept.eq.5            !qq-aqaq string from empty remnant (to have baryons instead of mesons from droplet (not fast meson in LHCf data)
      if(ish.ge.3)
     &write(ifch,*)'remnant particle index:',mm,m,iclpt,isopt

      if(ish.ge.10)call alist('ProRef&',1,nptl)
      antotre=antotre+1.
...
c use RemoveHadron if too many c quarks
      if(nrflav.gt.3)then
        nqc=jcf(4,1)+jcf(4,2)
        if(.not.gdiq.and.(nqu.lt.3.and.jcf(4,1).gt.1.or.
...
c     QGP ???????????
c      xmdrmin=dble(fremnux(jcf)/min(1.,max(0.0001,zd**2*0.02))
      xmdrmin=dble(fremnux(jcf)
     &     +amdrmin)**2
      
c      xmdrmax=dble(min(1000.,(fremnux(jcf)+amdrmax)*max(0.,zd)))**2
c      xmdrmax=dble(fremnux(jcf)+2.*amdrmin)**2 !for droplets, large masses are not good for eta distributions (too narrow)
      xmdrmax=dble(fremnux(jcf)+amdrmax)**2 !for droplets, large masses are not good for eta distributions (too narrow)
c      if(ioclude.eq.3)xmdrmax=min(xmdrmax,1d4)

      if(ish.ge.4)write(ifch,*)
c      print *,
     &  'remnant masses:',am2t,amasini,amasmin
     &                   ,sqrt(xmdrmin),sqrt(xmdrmax),zz,zd,iept
...
c      if(.not.((nqu.eq.3.and.naq.eq.0).or.(nqu.eq.0.and.naq.eq.3)
      if((iept.eq.3.or. !include diffractive remnants !
...
c     &    .and.(jcf(4,1)+jcf(4,2).eq.0)).or.
     ...
     &     .and.amasini.gt.amasmin.and.irmdropx.eq.1.and..not.gdiq)then
...
c charm not possible in droplet
     ...
     &           .or.(nqu.eq.1.and.naq.eq.1))then
          if(amasini.le.xmdrmin.or.iorsdf.ne.3
     &       .or.iept.eq.0.or.iept.eq.4.or.iept.eq.6)then
            goto 500
c          elseif(amasini.ge.xmdrmin.and.iept.le.2)then
c            goto 500
          endif
        endif

        if(iremn.ge.2.and.
     &     (amasini.gt.xmdrmax.or.nqc.ne.0))then
          if(iremn.eq.2)then
...
c        if(amasini.gt.1e4.or.nqc.ne.0)goto 500
        ...
        ifrptl(2,mm)=nptl
        tivptl(2,mm)=t
c  Remnant volume to have eps=dens GeV/fm3
        radptl(nptl)=sngl(ept(5))/dens
        dezptl(nptl)=0.
        do l=1,5
        ...
        if(abs(idx).gt.100)then
         amx=sngl(ept(5))
         call idres(idx,amx,idrx,iadjx,0)
         idx=idrx
        else
         idx=0
        endif
     ...
     *    ,ibptl(1,nptl),ibptl(2,nptl),ibptl(3,nptl),ibptl(4,nptl))
     ...
     &       .or.amasini.le.amasmin*dble(flow))then      !decay here only if no fusion or large mass or mass too low for flow
...
c            print *,'decay in ProReF',amasini,amasmin*flow
c            if(ioclude.eq.3.or.dble(pptl(5,mm)).lt.xmdrmin)then
              call hnbaaa(9,mm,iret)    !remnant decay with higher temperature (for QGP in EAS)
c            else
c              call DropletDecay(mm,iret)!Decay remn
c              iret=0
c            endif
          endif
          if(iret.ne.1.and.nptl.ne.nptlb)then ! ---successful decay---
            istptl(mm)=istptl(mm)+2
            ifrptl(1,mm)=nptlb+1
            ifrptl(2,mm)=nptl
            rini=3./5.*((3.*radptl(mm)/4./pi)**0.3333)**2 !<r**2>=3/5*R**2 for sphere of radius R
            do 21 n=nptlb+1,nptl
              ...
              bex=cos(phi)
              bey=sin(phi)
cc              tau=abs(taurem)+(2.25/sqrt(yr**2+0.04)-1.)   !do not give good result with hacas (probably too large)
c              tau=tauzer1+rangen()
c ------------------------------------------------------------------------
c space-time position based on what is done in hnb
c              call getxorptl(mm,xo1,xo2,xo3,xo4)
c              zeta=sign(ainfin,pptl(3,n))
c              pt2=pptl(1,n)**2+pptl(2,n)**2
c              if(pt2.gt.0)then  
c                pt=sqrt(pt2)
c                pp=sqrt(pt2+pptl(3,n)**2)
c                zeta=sign(1.,pptl(3,n))*log((pp+abs(pptl(3,n)))/pt)
c              endif
c              z=tau*sinh(zeta)
c              t=tau*cosh(zeta)
c              call setxorptl(nptl,xo1-radius*bex,xo2-radius*bey,z,t)
c              call idtau(idptl(n),pptl(4,n),pptl(5,n),taugm)
c              tivptl(1,n)=t
c              r=rangen()
c              tivptl(2,n)=t+taugm*(-alog(r))
c ------------------------------------------------------------------------
c former space-time position 
c              z=xo3+pptl(3,n)/pptl(5,n)*tau
c              t=xo4+pptl(4,n)/pptl(5,n)*tau
              call updateXor(n,n)
              call getxorptl(n,xo1,xo2,xo3,xo4)
              call setxorptl(n,xo1-radius*bex,xo2-radius*bey,xo3,xo4)
c ------------------------------------------------------------------------
c              ityptl(n)=ityptl(n)+1         !already in hnbaaa
   ...
   21       continue
     ...
     *             ,' --> do RemoveHadrons instead.'
          ...
          !redefine radptl to be used for flow calculation
          radptl(mm)=log(1.+dens) !being updated in Decaydroplet by avgdens
          dezptl(mm)=10000.
        endif
       endif
...
c........................remove hadrons.........................
     ...
     &     .or.(nqu.eq.1.and.naq.eq.1).or.gdiq))then
...
c        if(zrminc.lt.0.)stop'ProReF: not supported any more.         '
        ...
        ptt2=0d0
c        if(iLHC.eq.1)then
c          pt=ranptcut(1.)*ptfraqq
c          if(pt.lt.0.5d0*ept(5))then
c            phi=2.*pi*rangen()
c            ptt1=dble(pt*cos(phi))
c            ptt2=dble(pt*sin(phi))
c          endif
c          ptt3=dble(ir)*sqrt((0.5d0*ept(5))**2-ptt1*ptt1-ptt2*ptt2)
c        else
...
cc        ep(4)=0.5d0*ept(5)
     ...
     *     ,ep(1),ep(2),ep(3),ep(4),25)
        ...
        if(gdiq)then !qq-aqaq string, simply chose between diquark or antidiquark
          ...
          call uticpl(icf,iq2,3-j,iret) ! subtract quark 1 forward
          call uticpl(icb,iq2,j,iret) ! add quark 1 backward
...
c        if(.not.gdrop)istptl(mm)=41
        ...
        if(gproj)then
          ...
          if(gdrop)then
            ityptl(nptl)=44
          elseif(iept.eq.4)then
            ityptl(nptl)=46
          elseif(iept.eq.6)then
            ityptl(nptl)=47
c          elseif(iept.eq.2.or.iept.eq.4)then
          elseif(iept.eq.2)then
            ityptl(nptl)=48
          elseif(ghadr)then
            ityptl(nptl)=49
          else
...
c            write(ifch,*)'ici',ifq,ifa,jc,'| ',jcv
            ...
            if(abs(idf).eq.440)goto 777   !exclude j/psi production
...
c         if((nqu.eq.3.and.naq.eq.0).or.(nqu.eq.0.and.naq.eq.3)
c     &          .or.(nqu.eq.1.and.naq.eq.1).and.nlnk.ne.0)then 
...
c Droplet should not have a mass too high.
     ...
     &          ,xremd,xmsmax,xprmd,xmrmd,xptd,yptd
       ...
       else                     !if xm to small, use two body decay (should be rare if not forced)
...
c two body decay
          ...
          qcm=utpcmd(ptm,re(5),a(5),iret)
c--------------------------------------------
          u(3)=-1.d0 !-drangen(qcm) !2.d0*drangen(qcm)-1.d0   !always forward
c          u(3)=2.d0*drangen(qcm)-1.d0
c--------------------------------------------
c          phi=2.d0*dble(pi)*drangen(u(3))
c          u(1)=sqrt(1.d0-u(3)**2)*cos(phi)
c          u(2)=sqrt(1.d0-u(3)**2)*sin(phi)
c          do j=1,3
c            re(j)=qcm*u(j)
c            a(j)=-re(j)
c         enddo
...
c                                   3=nondiffr split, 5=diffr split)
      ...
      at(0,0)=0.0d0
      at(0,1)=dble(alpndi(1))
      at(0,2)=dble(alpdi(1))
      at(0,3)=dble(alpdro(3))
      at(0,4)=4d0 !dble(alpndi(1))
      at(0,5)=dble(alpdro(3))
      at(1,0)=0.0d0
      at(1,1)=dble(alpndi(2))
      at(1,2)=dble(alpdi(2))
      at(1,3)=dble(alpdro(3))
      at(1,4)=4d0!dble(alpndi(2))
...
c                                   6=nondiffr but no pomeron)
      ...
      amemn(0,2)=2d0*xdm2!+dble(delrex)
      amemn(0,3)=2d0*xdm2!+dble(delrex)
      amemn(0,5)=2d0*(xdm2+dble(delrex)) !remnant excited with qq-aqaq

      amemn(1,1)=xdm2!+dble(delrex)
      amemn(1,2)=xdm2!+dble(delrex)
      amemn(1,3)=2d0*xdm2!+dble(delrex)
      amemn(1,5)=2d0*(xdm2+dble(delrex)) !remnant excited with qq-aqaq
...
c       if(lproj(ip).eq.0.or.iep(ip).eq.0)
       if(kolpz(ip).eq.0.and.iep(ip).le.0)
     .  npglb=npglb+1
      enddo
      ntg=0
      ntglb=0
...
c       if(ltarg(it).eq.0.or.iet(it).eq.0)
       if(koltz(it).eq.0.and.iet(it).le.0)
     .     ntglb=ntglb+1
      enddo
c      write(6,*)'npj,ntg,npj+ntg,npglb,ntglb:'
c     *     ,npj,ntg,npj+ntg,nppglb,npglb,ntglb,ntg+ntglb
...
c     ----
     ...
     *,tivptl(1,n),tivptl(2,n)
      enddo
  115 format(1x,'/cptl/',2i6,2i10,2(1x,e10.3))
      endif
...
c-----------------------------------------------------------------------
      ...
      nptl=nptl+1
      npproj(i)=nptl
      idptl(nptl)=idptl(ii)*100+sign(99,idptl(ii))  !100*10**idp(i)+iep(i)
      istptl(nptl)=40
      ityptl(nptl)=40
      iorptl(nptl)=ii
      jorptl(nptl)=0
...
c      if(kolz.eq.0)call utstop(' kolz=0 (proj)&')
     ...
     &'Flavor problem in proj for pseudo-inelastic collision !',seedc
          elseif(iep(i).le.1)then
            iep(i)=1
          endif
        endif

        if(iremn.eq.2)then
          if(nqu.eq.2.and.naq.eq.2.and.abs(idptl(ii)).gt.1000)then
            iep(i)=5
          elseif(.not.((nqu.eq.3.and.naq.eq.0)
     &    .or.(nqu.eq.0.and.naq.eq.3).or.(nqu.eq.1.and.naq.eq.1)))then
            iep(i)=3
          endif
        endif
      endif
...
c     &           ,dble(fremnux(jc)))
     ...
     &       /(xpp(i)*s)
      xpos(i)=xpp(i)*xmptmp
      if(ish.ge.5)write(ifch,*)'Procop mass : ',nptl,aproj,xpos(i)*s
...
c-----------------------------------------------------------------------
      ...
      idptl(nptl)=idptl(jj)*100+sign(99,idptl(jj))   !100*10**idt(j)+iet(j)
      istptl(nptl)=40
      ityptl(nptl)=50
      iorptl(nptl)=jj
      jorptl(nptl)=0
...
c     &           ,dble(fremnux(jc)))
     ...
     &       /(xmt(j)*s)
      xtos(j)=xpttmp*xmt(j)
      if(ish.ge.5)write(ifch,*)'Procot mass : ',nptl,atarg,xtos(j)*s
...
c        kolp(i)=1
     ...
     &         ,'continue with zero mass'
      ...
      if(ish.ge.8)
     &  write(ifch,*)'Update projectile mass:',mm,iep(i),pptl(5,mm)
...
c        kolt(j)=1
     ...
     &            ,'continue with zero mass'
      ...
      if(ish.ge.8)
     &  write(ifch,*)'Update   target   mass:',mm,iet(j),pptl(5,mm)
...
c-----------------------------------------------------------------------
      subroutine emsPzGrey(iret)
c-----------------------------------------------------------------------
c     attribute pz momentum to spectators with iept=6 (ityptl=47/57)
c     sum should be null but introduce fluctuations in slow nucleon
c     distribution (coming from intranuclear cascade which is not treated
c     explicitely).
c-----------------------------------------------------------------------
      ...
      include 'epos.incems'
      include 'epos.incsem'
      DOUBLE PRECISION PFRZ,SPFRZ,RANNORM!,eini,esum,delta
      common /cfragm/rminfrg,emaxfrg,facgrey,p3grey
      
      iret=0
      IF(ish.ge.5)WRITE(ifch,*) 'emsPzGrey : i, ityp, pfrz, p3'

      SPFRZ=0d0
      jfin=0
      eini=0d0
      fac=0.1*min(pnullx,10.)
      DO  i = maproj+matarg+1, nptl       
        IF(ITYPTL(i).eq.47.or.ITYPTL(i).eq.57)THEN
          PFRZ= dble(0.33*p3grey*fac)
          if(ITYPTL(i).eq.47)PFRZ=-PFRZ
          PFRZ= RANNORM(PFRZ,dble(p3grey*fac))
          SPFRZ=SPFRZ+PFRZ
          jfin=jfin+1
          eini=eini+pptl(4,i)
          pptl(3,i)=sngl(dble(pptl(3,i))*(1d0+PFRZ/dble(pptl(3,i))))
          IF(ish.ge.5)WRITE(ifch,*) i,ITYPTL(i),SNGL(PFRZ),SNGL(SPFRZ)
        ENDIF
      ENDDO

      if(jfin.ge.1)then
        
C  CORRECT MOMENTA FOR MOMENTUM CONSERVATION
      SPFRZ = SPFRZ / JFIN
      DO  i = maproj+matarg+1, nptl       
        IF(ITYPTL(i).eq.47.or.ITYPTL(i).eq.57)THEN
          pptl(3,i)=sngl(dble(pptl(3,i))*(1d0-SPFRZ/dble(pptl(3,i))))
          pptl(4,i)=sqrt(pptl(1,i)**2+pptl(2,i)**2
     &                    +pptl(3,i)**2+pptl(5,i)**2)   !energy conservation restored in utresc
...
c-----------------------------------------------------------------------
      subroutine emsfrag(iqq,iret)
c-----------------------------------------------------------------------
c     ?????? Time not properly define if we want to use spectators in hacas ????
c ----> to be updated !      
c-----------------------------------------------------------------------
...
c Projectile fragment(s)
      ...
      if(iqq.eq.1)then          !mark spectators to be used after hacas
        do is=1,maproj
          if(istptl(is).eq.0)then
            if ( iorptl(is) .eq. 0 ) then
              ityptl(is)=4
              if(ihacas.ne.0)then  !copy spectators to be used by hacas
                nptl=nptl+1
        ...
        enddo
        if(ihacas.ne.0.or.infragm.eq.0)goto 100
      elseif(iqq.eq.2)then
c  copy spectators at the end of the list (necessary for hepmc interface)
        do is=1,nptl
          pt2=pptl(1,is)**2+pptl(2,is)**2  !spectator pt should be 0
          if(istptl(is).eq.0.and.pt2.le.0.)then
            if ( ityptl(is).eq.4 .or. ityptl(is).eq.5) then
              nptl=nptl+1
              if(nptl.gt.mxptl)then
                iret=2
                goto 1000
              ...
              ityptl(nptl)=0
              iorptl(nptl)=is
              if(is.gt.0)iorptl(nptl)=iorptl(is)
            endif
          endif
        enddo
        goto 1000
      elseif(infragm.eq.0)then
        goto 1000
      endif
      ...
      do is=1,nptl
        ...
        if(ityptl(is).eq.4)then
          pt2=pptl(1,is)**2+pptl(2,is)**2  !spectator pt should be 0
          if(istptl(is).eq.0.and.pt2.le.0d0)then
c           compose projectile spectators to remaining nucleus
            spec=spec+1d0
            xyzs(nint(spec),1)=xorptl(1,is)-bx/2
            xyzs(nint(spec),2)=xorptl(2,is)-by/2
            xyzs(nint(spec),3)=xorptl(3,is)
            tivptl(2,is)=0.
            ...
            idrest = is
            id=idptl(is)
            if     ( abs(id) .eq. 1120 ) then
              xyzs(nint(spec),4)=101
              inew  = inew + 101
              irest = irest - 101
            elseif ( abs(id) .eq. 1220 ) then
              xyzs(nint(spec),4)=100
              inew  = inew + 100
              irest = irest - 100
            endif
          endif
        elseif( is.le.maproj .and. iorptl(is) .le. 0
     .                       .and. istptl(is) .eq. 1 ) then
...
c  remaining nucleus is evaporating nucleons and alpha particles
         ...
         is=1
         jfin  = 0
         itheo=inew/100 !npglb            !lowest mass spectator nucleus
c         print *,irest,inew,itheo
 50      call epovapor(is*maproj,inew,itheo,xyzs,jfin
     .                ,ityp,pfrx,pfry,pfrz) 
...
c  momenta squared
     ...
     *                                       j,idnucl,ea,ptm,pt2
          if ( pt2 .ge. ptm ) then
            if(nloop.lt.1000)then
              nloop=nloop+1
              is=-1
              if(mod(nloop,100).eq.0)is=1
              nptl=nmin-1
              goto 50
            else
              write(ifmt,*)'Pb in emsfrag p :',nloop
              call utmsg("emsfrag: problem with grey particles (1) !&")
              call utmsgf
              iret=1
              goto 1000
            endif
c            if (ish.ge.2) write(ifch,*) 'emsfrag: pt reject particle',j
c            nnn=0
c            is=0
c            do while (is.lt.maproj.and.nnn.lt.inucl)
c              is=is+1
c              if(istptl(is).eq.1
c     &             .and.jorptl(is).eq.0.and.iorptl(is).eq.0)then
c                nnn=nnn+1
cc  copy spectators at the end of the list (necessary for hepmc interface)
c                nptl=nptl+1
c                if(nptl.gt.mxptl)then
c                  iret=1
c                  goto 1000
c                endif
c                call utrepl(nptl,is)
c                jorptl(is)=1
c                ifrptl(1,is)=nptl
c                ifrptl(2,is)=nptl
c                istptl(nptl)=0
...
c            goto 135
          ...
          idnucl=sign(idnucl,idproj)
          istptl(nptl)=0
          pptl(1,nptl)=sngl(pfrx(j))
          pptl(2,nptl)=sngl(pfry(j))
          pptl(3,nptl)=plong+sngl(pfrz(j))
          pptl(4,nptl)=sqrt(pptl(1,nptl)**2+pptl(2,nptl)**2
     *                     +pptl(3,nptl)**2+am**2)
          pptl(5,nptl)=am    !mass
          ityptl(nptl)=94
          ...
          ifrptl(1,nptl)=0
          ifrptl(2,nptl)=0
          xorptl(1,nptl)=sngl(xmean)
          xorptl(2,nptl)=sngl(ymean)
          xorptl(3,nptl)=sngl(zmean)
          xorptl(4,nptl)=sngl(zmean)
          tivptl(1,nptl)=sngl(zmean)
          tivptl(2,nptl)=tivptl(2,idrest)
          idptl(nptl)=idnucl
          if(inucl.eq.1)esum=esum+pptl(4,nptl)
 135    continue

c       restore energy/momentum conservation
c        if(npart.ne.0)then
c          call utrfrag(1,nmin,iret2)
c          if(iret2.ne.0)then
c            if(nloop.lt.1000)then
c              nloop=nloop+1
c              is=-1
c              if(mod(nloop,100).eq.0)is=1
c              nptl=nmin-1
c              goto 50
c            else
c              write(ifmt,*)'Pb in emsfrag p :',nloop
c              call utmsg("emsfrag: problem with grey particles (2) !&")
c              call utmsgf
c              iret=1
c              goto 1000
c            endif
...
c Target fragment(s)
      ...
      if(iqq.eq.1)then          !mark spectators to be used after hacas
        ...
        enddo
        if(ihacas.ne.0.or.infragm.eq.0)goto 1000
      endif
...
c  compose projectile spectators to remaining nucleus
        ...
        elseif( is.le.maproj+matarg+1 .and. iorptl(is) .le. 0
     .                                .and. istptl(is) .eq. 1 ) then
          if( iorptl(is) .eq. 0 ) jorptl(is)=1
          matar=matar-1
        endif
...
c  remaining nucleus is evaporating nucleons and alpha particles
         ...
         is=1
         jfin  = 0
         itheo=inew/100         !ntglb  !lowest mass spectator nucleus
 150     call epovapor(is*matarg,inew,itheo,xyzs
     .                ,jfin,ityp,pfrx,pfry,pfrz) 
         if ( jfin .eq. 0 )then   !something failed
           iret=1
           goto 1000
         endif
...
c  momenta squared
          ...
          pt2 = sngl( pfrx(j)**2 + pfry(j)**2 )
          if(ish.ge.6)write(ifch,*) 'tar fragment: j,id,ea,ptm,pt2,am=',
     *                                       j,idnucl,ea,ptm,pt2,am
          if ( pt2 .ge. ptm ) then
            if(nloop.lt.1000)then
              nloop=nloop+1
              is=-1
              ...
              write(ifmt,*)'Pb in emsfrag t :',nloop
              call utmsg("emsfrag: problem with grey particles (1) !&")
              call utmsgf
              iret=1
              goto 1000
...
c            nnn=0
c            is=maproj
c            do while (is.lt.maproj+matarg.and.nnn.lt.inucl)
c              is=is+1
c              if(istptl(is).eq.1
c     &             .and.jorptl(is).eq.0.and.iorptl(is).eq.0)then
c                nnn=nnn+1
...
c              goto 150
c            else
c              write(ifmt,*)'Pb in emsfrag t :',nloop
c              call utmsg("emsfrag: problem with grey particles (2) !&")
c              call utmsgf
c              iret=1
c              goto 1000
...
c      call decayall(nptla+1)
 ...
 1000 continue
      ...
      if(iret.eq.1)then         !failed to produce fragments, keep spectators only
        iret=0
        nptl=nptl0
      endif
      ...
      subroutine IniChargex
***************************************************************
*
*  initialization to call Chargex subroutine
*
***************************************************************
      implicit none
      include 'epos.inc'

c      double precision CS,face
c     integer i,imax
c      integer jspin,ind,ifl(3),istra
      real efac,rexchrgpro,rexchrgtar
      ...
      ichargex=ichargexin
      if(ichargex.ne.0)then
        rexchrgpro=rexchrg(iclpro)*rexdif(iclpro) !to make it decreasing as SD
        rexchrgtar=rexchrg(icltar)*rexdif(icltar)
c      call idflav(idprojin,ifl(1),ifl(2),ifl(3),jspin,ind)
c      istra=abs(ifl(1))+abs(ifl(2))
c      if(ifl(3).ne.0)then
c        istra=istra+abs(ifl(3))
c      else
c        istra=istra+3
c      endif
c      if(istra.ge.9)then
        if(abs(idproj).ne.1120.and.abs(idproj).ne.1220
     +  .and.abs(idproj).ne.120.and.abs(idproj)/10.ne.11)rexchrgpro=0.
        if(abs(idtarg).ne.1120.and.abs(idtarg).ne.1220)rexchrgtar=0.
        if(rexchrgpro+rexchrgtar.le.0.)then
          if(ish.ge.1)write(ifmt,*)
     +  'Warning : no charge exchange without u or d quark : '
     +       ,idproj,idtarg
          xschargex(0)=0.
          return
        endif

        efac=1.!/log10(engy)  !small energy dependence from data
        xschargex(0)=efac*rexchrgpro+efac*rexchrgtar
     +     +efac*rexchrgpro*efac*rexchrgtar
        if(xschargex(0).gt.0)then
          xschargex(1)=efac*rexchrgpro/xschargex(0) !single charge exchange probability proj
          xschargex(2)=efac*rexchrgtar/xschargex(0) !single charge exchange probability targ
          xschargex(3)=efac*rexchrgpro*efac*rexchrgtar
     +         /xschargex(0)    !double charge exchange
        endif
      else
         xschargex(0)=0.
         xschargex(1)=0.
...
c      print *,xschargex,rexchrg,xschargex(1)+xschargex(2)+xschargex(3)
      end


***************************************************************

      subroutine emschargex(iqq,iret)

***************************************************************
*
*     interface to call Chargex subroutine
*     iqq>0 first call to set the new collision system
*           and save initial one (11=charge exchange, 20 or 02
*           for SD, 22 for DD, and 00 for CD)
*     iqq<0 second call to restore initial projectile and target
*           and add particles from the pion-nucleon collision
*           in proper frame
*     iret - return status
***************************************************************
      include 'epos.inc'
      ...
      real phi,bimp
      common/emsptl/nppr(npommx,kollmx),npproj(mamx),nptarg(mamx)
      integer nppr,npproj,nptarg


c     Input values
      double precision pframe(5)
      integer istart(2),n1,n2,ispin,ispin0,idresch,ITYPR,nbeam,idbeam
      logical LCE(2)

      integer iqq,iret,ipart,n,k,i,ior,idbaryon,ntry,i1,i2,is,nptl0,iept
     .        ,nppd,nptd,itp,npd,ma!,ifl(3),jspin,ind
      real   m(2),m3,xran,xsratio!,chrg
      double precision e1,e2,pproj,ptarg,mp,decms,fac
      DOUBLE PRECISION PRR(5),PNN(5),P(5,2),PTR,PHIR,ESUM,PSUM,pbeam
      double precision drangen,rannorm

      real engysave,bimpsave,phisave,pnllsave,qsqevtsave,typevtsave
      integer noebinsave,maprojsave,laprojsave,matargsave,latargsave
     .     ,idprojinsave,idtarginsave,ic(2),isave,idprojsave,idtargsave
     .     ,iomegasave,nglevtsave,ng1evtsave,ng2evtsave,ng11evtsave
     .     ,ng12evtsave,eglevtsave,fglevtsave,rglevtsave,sglevtsave
     .     ,irdmprsave
      save noebinsave,maprojsave,laprojsave,matargsave,latargsave,LCE
     .     ,idprojinsave,idtarginsave,engysave,isave,pframe,istart
     .     ,bimpsave,phisave,pnllsave,idprojsave,idtargsave,qsqevtsave
     .     ,iomegasave,nglevtsave,ng1evtsave,ng2evtsave,ng11evtsave
     .     ,ng12evtsave,eglevtsave,fglevtsave,rglevtsave,sglevtsave
     .     ,typevtsave,irdmprsave,P

      if(ish.ge.2)write(ifch,*)'Start emschargex - ',iqq,iret

      if(iqq.ge.0)then
c       first call, save initial conditions
        noebinsave=noebin
        engysave=engy
        maprojsave=maproj
        matargsave=matarg
        ...
        rglevtsave=rglevt
        sglevtsave=sglevt
        chargex=.true.          !before ainit and definition of iret in case of problem to properly restore proj/target ids
        irdmpr=0           !no random charge for projectile

        if(ish.ge.8)
     .call alistc("List after Markov&",1,nptl)

        itp=0
        nppd=0
        nptd=0
        if(iqq.ne.11)then
          nppd=npproj(iproj(kchargex))
          nptd=nptarg(itarg(kchargex))
c       hide particles from original interaction
          if(iqq.eq.0)then
            do n=maproj+matarg+1,nptl
              if(ityptl(n)/10.ne.4.and.ityptl(n)/10.ne.5)
     .             istptl(n)=istptl(n)+101
            enddo
          else
            if(pptl(5,nppd).le.pptl(5,nptd))then
              itp=4
            else
              itp=5
            endif
            do n=maproj+matarg+1,nptl
              if(ityptl(n)/10.ne.itp)istptl(n)=istptl(n)+101
            enddo
          endif
        endif
...
c     then save projectile and target type of pair with CE
      ic(1)=icproj(1,iproj(kchargex))
      ic(2)=icproj(2,iproj(kchargex))
      idprojin=idtra(ic,0,0,0)
      ic(1)=ictarg(1,itarg(kchargex))
      ic(2)=ictarg(2,itarg(kchargex))
      idtargin=idtra(ic,0,0,0)


c     Select single or double charge exchange
      LCE(1)=.true.     !CD
      LCE(2)=.true.
      if(itp.eq.5)then !SD pro or DD
        LCE(1)=.false.
      elseif(itp.eq.4)then !SD tar or DD
        LCE(2)=.false.
      elseif(iqq.ne.0)then  !charge exchange
        xran=rangen()
        if(xran.le.xschargex(1))then
          LCE(2)=.false.        !exchange only on projectile side
        elseif(xran.le.xschargex(1)+xschargex(2))then
          LCE(1)=.false.        !exchange only on target side
        endif
      endif

c     Offset for beam particles
      if(ish.ge.5)write(ifch,*)'index:',nptl,itp
     .             ,kchargex,iproj(kchargex),itarg(kchargex),LCE
...
c     LOOP on both side
      ...
      i2=2
      is=1
      if(LCE(1).and.LCE(2))then
        istart(1)=nptl+3
        istart(2)=nptl+6
c        if(rangen().lt.0.5)then
c          i1=2
c          i2=1
c          is=-1
c        endif
      elseif(LCE(2))then    !to keep the same order for produced particles
        i1=2
        i2=1
        is=-1
      endif
      if(iqq.ne.11)then     !for diffraction, we keep original emitting beam remnant, so no need to produce a new one
        istart(1)=istart(1)-1
        istart(2)=istart(2)-1
      endif
      ESUM=0d0
...
c     Initialize conditions for Chargex
          ITYPR=1               !pion (or pi-pi)  for pion exchange
c          if(rangen().lt.0.1)ITYPR=2 !check LHCf pi0 data
c          if(abs(idbeam).lt.1000.or.iqq.ne.11)ITYPR=2 !rho for mesons or diffraction
          if(iqq.ne.11)then
            ITYPR=2             !rho for diffraction
c          elseif(rangen().lt.1.)then
c            ITYPR=2             !check LHCf pi0 data
          endif
c          call idflav(idbeam,ifl(1),ifl(2),ifl(3),jspin,ind)
          
c         Call Chargex to get momentum of resonance (PRR) and neutron (PNN)
          ispin0=0
          ipart=110
          if(iqq.eq.11)then
            if(abs(idbeam).lt.1000)then
              if(idbeam.eq.110.or.rangen().lt.0.5)ipart=120 !can only carry one charge type so 0.5 and not 0.6666
              ispin0=1
            else                !Baryon
              if(rangen().lt.0.33333)ispin0=1 !delta resonance in 1/3 of the cases
              if(ispin0.eq.0)then
                ITYPR=2         !check LHCf pi0 data   !to have resonance state while keeping nucleon
                if(rangen().lt.0.66666)ipart=120
              else
                rrr=rangen()
                if(rrr.lt.0.16666)then
                  ipart=120
                elseif(rrr.lt.0.66666)then
                  ipart=-120
                endif
              endif
              if(abs(ipart).eq.120)then
                if(abs(idbeam).eq.1220)ipart=-ipart
                if(idbeam.lt.0)ipart=-ipart
              endif
            endif
          endif
          if(ITYPR.eq.2)ipart=sign(abs(ipart)+1,ipart)
          ispin=ispin0+mod(abs(idbeam),10)    !spin of remnant (resonance) after pion/rho emission
     
        
c 20       continue
          call idmass(ipart,m(i))
          PRR(5)=dble(m(i))
          if(iqq.eq.11)then
 20         continue
c           PNN(3)=RANNORM(0.88d0,0.15d0) !tune to LHCf and HERA
            if(abs(idbeam).lt.1000)then
c              PNN(3)=RANNORM(1.d0,0.35d0)  !tune to NA61
              PNN(3)=RANNORM(0.9d0,0.25d0)  !tune to NA61
            else
c              PNN(3)=RANNORM(1.d0,0.2d0)   !tune to LHCf and HERA and pp400
              PNN(3)=RANNORM(0.9d0,0.25d0)   !tune to LHCf and HERA and pp400
            endif
            if(drangen(PNN(3)).lt.
     .        (PNN(3)/(1d0-dble(0.5*egymin/engy)))**3)goto 20
            PNN(3)=PNN(3)*pnullx0
            PTR=0.2d0*dble(ranptcut(5.))
            PHIR=dble(2.*pi)*drangen(PTR)
            PNN(1)=PTR*cos(PHIR)
            PNN(2)=PTR*sin(PHIR)
            idbaryon=idresch(ipart,idbeam)
            if(abs(idbeam).lt.1000)
     .      idbaryon=sign(abs(idbaryon-mod(idbaryon,10))+ispin,idbaryon)
            call idmass(idbaryon,m3)          
            MP=dble(m3)
            PNN(5)=MP
            PNN(4)=SQRT(PNN(5)**2+PNN(3)**2+PNN(2)**2+PNN(1)**2)
            PRR(1)=-PNN(1)
            PRR(2)=-PNN(2)
            PRR(3)=pnullx0-PNN(3)
            PRR(4)=SQRT(PRR(5)**2+PRR(3)**2+PRR(2)**2+PRR(1)**2)
            fac=0.5d0*dble(engy)/(PRR(4)+PNN(4))
            if(ish.ge.8)write(ifch,*)'pion fac',fac
            do k=1,3
              PNN(k)=PNN(k)*fac
              PRR(k)=PRR(k)*fac
            enddo
            ...
            do k=1,5
              PNN(k)=dble(pptl(k,npd))
            enddo
            PNN(3)=abs(PNN(3))
c            MP=sqrt(PNN(1)**2+PNN(2)**2+PNN(5)**2)
c           PNN(3)=PNN(4)*sqrt((1d0-MP/PNN(4))*(1d0+MP/PNN(4)))     !to have a better precision than pptl(3) which is single precision) (always positive because sign is given back later
            call idmass(idbeam,m3)          
            MP=dble(m3)
            PRR(4)=sqrt(pnullx0**2+MP**2)-PNN(4)
            PRR(1)=-PNN(1)
            PRR(2)=-PNN(2)
            MP=sqrt(PRR(1)**2+PRR(2)**2+PRR(5)**2)
            PRR(3)=(1d0-MP/PRR(4))*(1d0+MP/PRR(4))
            if(PRR(3).gt.0d0)then
              PRR(3)=PRR(4)*sqrt(PRR(3))
              if(ish.ge.8)then
                pbeam=PRR(3)+PNN(3)
                write(ifch,*)'masses,pbeam',pptl(5,nppd),pptl(5,nptd)
     .                                     ,pbeam
                write(ifch,*)'PNN,PRR',PNN,PRR
     .  ,sqrt((PNN(4)-PNN(3))*(PNN(4)+PNN(3))-PNN(1)**2-PNN(2)**2)
              endif
            else
              PRR(3)=-PRR(4)*sqrt(abs(PRR(3)))
              if(ish.ge.1)
     .             write(ifmt,*)'Energy too low in Pomeron exchange ! '
     .                         ,'(xmindiff too low)'
              if(ish.ge.2)then
                write(ifch,*)'masses',pptl(5,nppd),pptl(5,nptd)
                write(ifch,*)'PRR',PRR,
     .  sqrt((PNN(4)-PNN(3))*(PNN(4)+PNN(3))-PNN(1)**2-PNN(2)**2)
                write(ifch,*)'Energy too low in Pom exchange, skip !'
              endif
              iret=1
              return
            endif
            fac=0.5d0*dble(engy)/(PRR(4)+PNN(4))
            if(ish.ge.8)write(ifch,*)'diff fac',fac
            do k=1,3
              PNN(k)=PNN(k)*fac
              PRR(k)=PRR(k)*fac
            enddo
...
c     copy projectile (beam, resonance and neutron) at the end of final stack (temporary)

          ior=0
c         original beam
          do n=n1,n2
            if(nbeam.eq.n)then  !modified nucleon
              if(istptl(n).eq.0)then
                istptl(n)=1
                iorptl(n)=-1
                jorptl(n)=0
              endif
              ior=n             !origin of resonance
              if(iqq.eq.11)then
                if(LCE(1).and.LCE(2))then
                  ifrptl(1,n)=istart(i)-2
                  ifrptl(2,n)=istart(i)-1
                else
                  ifrptl(1,n)=istart(i)-3
                  ifrptl(2,n)=istart(i)-2
                endif
              endif
            endif
          enddo
      
c         produced resonance (for tagging)      
          nptl=nptl+1
          call utrepla(nptl,ior)
          idptl(nptl)=sign(9900000+abs(ipart),ipart)
          pptl(1,nptl)=sngl(P(1,i))
          pptl(2,nptl)=sngl(P(2,i))
          pptl(3,nptl)=sngl(P(3,i))
          pptl(4,nptl)=sngl(P(4,i))
          pptl(5,nptl)=sngl(P(5,i))
          istptl(nptl)=41
          ityptl(nptl)=iept
          iorptl(nptl)=ior
          if(i.eq.1.or.LCE(1))then
            ifrptl(1,nptl)=istart(2)+i
            ifrptl(2,nptl)=istart(2)+i
          else
            ifrptl(1,nptl)=istart(2)+maproj+1
            ifrptl(2,nptl)=istart(2)+maproj+1
          endif
          xorptl(1,nptl)=coord(1,kchargex)
          xorptl(2,nptl)=coord(2,kchargex)
          xorptl(3,nptl)=coord(3,kchargex)
          xorptl(4,nptl)=coord(4,kchargex)
          tivptl(1,nptl)=xorptl(4,nptl)
          tivptl(2,nptl)=xorptl(4,nptl)
          PSUM=PSUM+P(3,i)
          ESUM=ESUM+P(4,i)

c         produced baryon (for tagging)
          nptl=nptl+1
          call utrepla(nptl,ior)
          idptl(nptl)=sign(9900000+abs(idbaryon),idbaryon)
          pptl(1,nptl)=sngl(PNN(1))
          pptl(2,nptl)=sngl(PNN(2))
          pptl(3,nptl)=sngl(PNN(3))
          pptl(4,nptl)=sngl(PNN(4))
          pptl(5,nptl)=sngl(PNN(5))
          istptl(nptl)=41
          ityptl(nptl)=iept
          iorptl(nptl)=ior
          if(iqq.eq.11)then
            ifrptl(1,nptl)=nptl+1
            ifrptl(2,nptl)=nptl+1
          else
            ifrptl(1,nptl)=ifrptl(1,ior)
            ifrptl(2,nptl)=ifrptl(2,ior)
            ifrptl(1,ior)=istart(i)-2
            ifrptl(2,ior)=istart(i)-1
          endif
          xorptl(1,nptl)=coord(1,kchargex)
          xorptl(2,nptl)=coord(2,kchargex)
          xorptl(3,nptl)=coord(3,kchargex)
          ...
          if(iqq.eq.11)then
c         produced baryon (final)
          nptl=nptl+1
          call utrepla(nptl,nptl-1)
          idptl(nptl)=idbaryon
          istptl(nptl)=0
          ityptl(nptl)=iept+8
          iorptl(nptl)=nptl-1
...
c       Momentum of new target (original one) and projectile (resonance)
          call idmass(idbeam,m(i)) !target mass = proton
          MP=dble(m(i))
          P(5,i)=MP  
          P(1,i)=0.0d0
          P(2,i)=0.0d0
          P(3,i)=isave*pbeam
          P(4,i)=sqrt(P(3,i)**2+MP**2)
          PSUM=PSUM+P(3,i)
          ESUM=ESUM+P(4,i)
...
c         original beam
          do n=n1,n2
            if(nbeam.eq.n)then  !modified nucleon
              ...
              pptl(4,nptl)=sngl(P(4,i))
              pptl(5,nptl)=sngl(P(5,i))
              idptl(nptl)=sign(9900000+abs(idbeam),idbeam)
              istptl(nptl)=41
              ityptl(nptl)=iept
              iorptl(nptl)=n
              if((i.eq.1.and.maproj.eq.1)
     .       .or.(i.eq.2.and.matarg.eq.1))then
                ifrptl(1,nptl)=istart(i)+i
                ifrptl(2,nptl)=istart(i)+i
              else
                ifrptl(1,nptl)=0
                ifrptl(2,nptl)=0
              endif
      ...
      if(ish.ge.5)write(ifch,*)'Check before',PSUM,ESUM-dble(engy)
        
c     Frame Momentum in cms frame of detector
      do k=1,5
        pframe(k)=0d0
      enddo
      do n=1,2
        do k=1,4
          pframe(k)=pframe(k)+P(k,n)
        enddo
      enddo
      pframe(5)=sqrt(max(0d0,(pframe(4)+pframe(3))
     .  *(pframe(4)-pframe(3))+pframe(1)*pframe(1)+pframe(2)*pframe(2)))

      ptarg=-sqrt(P(3,2)**2+P(2,2)**2+P(1,2)**2) !target
      pproj=sqrt(P(3,1)**2+P(2,1)**2+P(1,1)**2) !projectile
      
      
c     Calculations of energy of the center-of-mass in the detector frame


      e1=dsqrt(dble(m(1))**2+pproj**2)
      e2=dsqrt(dble(m(2))**2+ptarg**2)
      decms=dsqrt(((e1+e2)+(pproj+ptarg))*((e1+e2)-(pproj+ptarg)))

c      if(iqq.eq.11)print *,
      if(ish.ge.5)write(ifch,*)
     .           'Chargex input',ipart,idbaryon,idprojin,idtargin
     .                                 ,decms,pproj,ptarg,m,pframe(5)
      if(decms.lt.dble(egymin))then
        if(ntry.lt.100)then
          goto 10
        else
          if(ish.ge.2)
     .     write(ifch,*)'Energy too low in charge exchange, skip !'
          iret=1
          return
        endif
      endif
      if(abs(pframe(5)**2-((pframe(4)+pframe(3))*(pframe(4)-pframe(3))
     .     +pframe(1)*pframe(1)+pframe(2)*pframe(2)))
     .                                      .gt.1d-5*pframe(5)**2)then
        if(ish.ge.2)
     .       write(ifch,*)'Problem with energy in pion exchange, skip !'
     .      ,pframe
        iret=1
        return
      endif
      ...
      if(ish.ge.5)
     .call alistc("List before pion-nucleon interaction&",1,nptl)


c     Update some parameters value to run correctly
      if(iqq.ne.11)iomega=2     !no diffraction for hard diffraction
      if(LCE(1))maproj=1
      if(LCE(2))matarg=1
      if(LCE(1))laproj=-1               !<0 means any hadron
      if(LCE(2))latarg=-1
      noebin=1
      ecms=-1      !energy
      elab=-1      !energy lab
      ...
      pnll=-1      !momentum
      engy=sngl(decms)          !center of mass energy in GeV/c2

      ichargex=0

      
c     Initialization for newly define cms energy and projectile/target
      call ainit

      istart(2)=nptl
      do n=1,nptl
        call utrepla(mxptl-n,n) !copy at the end
      enddo

      if(sigtotaa.gt.0.)then
        xsratio=sigineaa/sigtotaa
      else
        xsratio=sigine/sigtot
      endif
      if(ish.ge.5)write(ifch,*)'call model',engy,xsratio,idproj,idtargin
     .     ,laproj,maproj,latarg,matarg
      
      if(iqq.ne.11.or.rangen().lt.xsratio)then   !inelastic pion interaction or diffraction
      
        iret=0
        
      else        !elastic pion interaction, nothing done (just resonance emission)
        maproj=1     !to simplify spectator treatment
        laproj=-1
        matarg=1
        latarg=-1
        iret=-1
        ...
        typevt=0
        nevt=1
        if(ish.ge.5)write(ifch,*)'elastic pion interaction !'
        
      endif
      
      else                      !event between resonance and baryon was successfull ----------

        if(iret.eq.0)then

c     Update typevt to indicate charge exchange by adding "10"
          if(iomega.ne.2)then
            typevt=sign(abs(typevt)+10,typevt)
          else
            typevt=typevtsave
          endif

c Define origin of virtual beam particle, update indices and boost back in original CMS frame
        ESUM=pframe(4)
        PSUM=pframe(3)
        if(maproj.gt.1)then
          ma=maproj-1
          ESUM=ESUM+ma*P(4,1)
          PSUM=PSUM+ma*P(3,1)
        elseif(matarg.gt.1)then
          ma=matarg-1
          ESUM=ESUM+ma*P(4,2)
          PSUM=PSUM+ma*P(3,2)
        endif
        if(ish.ge.8)call alistc("before boost",1,nptl)
        do i=1,nptl
          if(i.gt.maproj+matarg)iorptl(i)=iorptl(i)+istart(2)
          if(jorptl(i).gt.0)jorptl(i)=jorptl(i)+istart(2)
          if(ifrptl(1,i).gt.0)ifrptl(1,i)=ifrptl(1,i)+istart(2)
          if(ifrptl(2,i).gt.0)ifrptl(2,i)=ifrptl(2,i)+istart(2)
c        write(*,'(i6,i10,1x,4(e12.6,1x))')i,idptl(i),(xorptl(k,i),k=1,4)
c        write(*,'(i5,3x,i2,2x,2i5,2x,2i5)')i,istptl(i)
c     *        ,iorptl(i),jorptl(i),ifrptl(1,i),ifrptl(2,i)
c         write(*,'(i10,1x,4(e12.6,1x))')idptl(i),(pptl(k,i),k=1,4)
c         boost particles to original cms frame
        call utlob4(-1,pframe(1),pframe(2),pframe(3),pframe(4),pframe(5)
     +           ,xorptl(1,i),xorptl(2,i),xorptl(3,i),xorptl(4,i))
        call utlob4(-1,pframe(1),pframe(2),pframe(3),pframe(4),pframe(5)
     +       , pptl(1,i), pptl(2,i), pptl(3,i), pptl(4,i))
c      write(*,'(i10,1x,4(e12.6,1x))')idptl(i),(xorptl(k,i),k=1,4)
c         write(*,'(i5,3x,i2,2x,2i5,2x,2i5)')i,istptl(i)
c     *        ,iorptl(i),jorptl(i),ifrptl(1,i),ifrptl(2,i)
c       write(*,'(i10,1x,4(e12.6,1x))')idptl(i),(pptl(k,i),k=1,4)
      ...
      enddo
      if(ish.ge.8)call alistc("after boost",1,nptl)
c      call alistf('list after exchange&')
      if(ish.ge.5)write(ifch,*)'Check after',PSUM,ESUM

c     define status of new particle beam
      do i=1,maproj
        if(typevt.ne.10.and.maproj.gt.1)then    !if not elastic only copy new nucleus into old one
          call utrepla(mxptl-i,i)
          if(istptl(mxptl-i).eq.0)then !pt of spectator should be 0
            pptl(1,mxptl-i)=0.
            pptl(2,mxptl-i)=0.
            pptl(4,mxptl-i)=sqrt(pptl(3,mxptl-i)**2+pptl(5,mxptl-i)**2)
          endif
          iorptl(i)=i         
        endif
        if(istptl(i).ne.0.or.maproj.gt.1)then
          idptl(i)=sign(9900000+abs(idptl(i)),idptl(i))
          istptl(i)=41
        endif
        if(iomega.ne.2)then
          ityptl(i)=48
        ...
        elseif(LCE(1))then
          iorptl(i)=istart(1)-3
        elseif(typevt.eq.10)then
          iorptl(i)=istart(1)
          ifrptl(1,mxptl-iorptl(i))=iorptl(i)+1
          ifrptl(2,mxptl-iorptl(i))=iorptl(i)+1
        elseif(maproj.eq.1)then
          iorptl(i)=istart(1)
        endif
      enddo
      do i=maproj+1,maproj+matarg
        if(typevt.ne.10.and.matarg.gt.1)then    !if not elastic only copy new nucleus into old one
          call utrepla(mxptl-i,i)
          if(istptl(mxptl-i).eq.0)then !pt of spectator should be 0
            pptl(1,mxptl-i)=0.
            pptl(2,mxptl-i)=0.
        ...
        if(istptl(i).ne.0.or.matarg.gt.1)then
          idptl(i)=sign(9900000+abs(idptl(i)),idptl(i))
          istptl(i)=41
        endif
        if(iomega.ne.2)then
        ...
        elseif(typevt.eq.10)then
          iorptl(i)=istart(2)
          ifrptl(1,mxptl-iorptl(i))=iorptl(i)+2
          ifrptl(2,mxptl-iorptl(i))=iorptl(i)+2
...
c     Copy new particles before the initial beam particles in reversed order at the end of the stack
      do n=1,nptl
        call utrepla(mxptl-istart(2)-n,n)    !copy at the end
      enddo

c     Copy back from the end to the beginning of the stack
      nptl=nptl+istart(2)
      do n=1,nptl
        call utrepla(n,mxptl-n)    !copy at the beginning
      enddo

c      call alistc("List after pion-nucleon interaction&",1,nptl)
...
c     second call, restore initial conditions
        ichargex=1
        irdmpr=irdmprsave
        noebin=noebinsave
        engy=engysave
        ...
        rglevt=rglevtsave
        sglevt=sglevtsave
        if(noebin.lt.0)qsqevt=qsqevtsave
        ecms=-1
        elab=-1
        ekin=-1
        pnll=-1
        call ainit              !restore normal run

        chargex=.false.         !after ainit to avoid print out
...
c--------------------------------------------------------------------
      integer function idresch(ipart,idbeam)
c--------------------------------------------------------------------
c  returns new particle id when idbeam emit ipart
c--------------------------------------------------------------------
      real rangen
      integer ipart,ic(2),iqu,iqd,i,k

      ic(1)=0
      ic(2)=0
      idresch=0
      if(abs(ipart)/10.eq.12)then
        iqu=1
        iqd=-1
        if(ipart.lt.0.)then !could be the antiparticle
          iqu=-iqu
          iqd=-iqd
        endif
        call idtr6(idbeam,ic)
        if(abs(idbeam).gt.1000)then
          i=1
          if(idbeam.lt.0)i=2
          if(ic(i)/100000-iqu.ge.0.and.mod(ic(i)/10000,10)-iqd.ge.0)then
            ic(i)=ic(i)-iqu*100000-iqd*10000
          else
            ipart=-ipart
            iqu=-iqu
            iqd=-iqd
          ...
          iqu=1
          iqd=-1
          if(rangen().lt.0.5)then !could be pi+ or pi-
            ipart=-ipart
            i=2
            k=1
          endif
          if(ic(i)/100000-iqu.ne.0.and.mod(ic(k)/10000,10)-iqd.ne.0
     .       .and.ic(i)/10000-ic(k)/10000.ne.0)then
            ipart=-ipart
            if(ipart.lt.0)then
              i=2
              k=1
          ...
          if(mod(ic(k)/10000,10)-abs(iqd).eq.0)then
            ic(k)=100000
          elseif(ic(i)/100000-abs(iqu).eq.0)then
            ic(i)=10000
          endif
        endif
        idresch=idtr2(ic)
      elseif(ipart/10.eq.11)then
        idresch=idbeam
      else
        print *,'should not be',idbeam,ic,iqu,iqd,ipart
        call utstop ("idresch: ipart not allowed !&")
      endif
      if(abs(idresch).lt.20)then
        print *,'should not be',idbeam,ic,iqu,iqd,ipart
        call utstop ("idresch: result not allowed !&")
      endif
...
c      isp=idpj/10/iap
c      ist=idtg/10/iat
c      call idchrg(4,idpj,cp)
c      call idchrg(5,idtg,ct)
c      ac=abs(cp+ct)
...
c      parameter(itext=15)
     ...
     *              k,itpr(k),npr(0,k),npr(1,k),nprmx(k),'   '
      ...
      do k=1,koll
        do n=1,nprmx(k)
          if(nbkpr(n,k).eq.0.and.nvpr(n,k).eq.0)then    !not to count backup Pomeron
