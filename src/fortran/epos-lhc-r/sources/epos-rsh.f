...
c     *,xprh,xmrh
     ...
     *,qmin(2),iqc(2),nqc(2),ncc(2,2),amqt(4)
     ...
     &,iorprt(ntim),jorprt(ntim),nprtj
      ...
      zz=1.+zoeinc*zzzz               !<-----
      q2fin=q2fin*zz
c      zzzz=max(0.,0.5*sngl(xkolhp(ip)+xkolht(it))-1.) 
      zzzz=max(0.,0.5*float(kolp(ip)+kolt(it))-1.)
c      zzzz=max(0.,log(0.5*float(kolp(ip)+kolt(it))))
      zz=1.+zipinc*(1.5+zzzz)               !<-----
c      zz=1.+zzzz*zoeinc*sngl(xpr(ncolp,kcol)**0.2)             !<-----
      q2mini=q2min*zz
c      q2mini=max(q2min,min(0.01*xpr(ncolp,kcol)*engy**2,q2mini))
     
      qqcut=q2mini  !????????????pt2cut

c      print *,xkolhp(ip),kolp(ip),xkolht(it),kolp(it),zzzz,q2fin,q2mini
...
csp end initialisation
      ...
      nj=0
      nptl=nptl1
      if(ntryhot.gt.1000)then
        if(ish.ge.1)then
          write(ifmt,'(a)')'WARNING pasahot: ntryhot too big -> exit'
          write(ifch,'(a)')'WARNING pasahot: ntryhot too big -> exit'
        endif
        goto 16
      endif
      if(iremn.ge.2)then
...
c        print *,xmin,xmax,smin,zzzz,zzzz*zopinc*z
c     *,zopinc*fegypp*exp(-bpomr**2/b2xscr)
...
c-----------------------------------------------------------------
     ...
     & 'determine LC momenta wpi,wmi for hard Pomeron'
      ...
      if(iremn.ge.2)then
        if(iclpro.eq.2)then
          if(iabs(idptl(ip))/10.eq.112)then !proj=proton
            nquu1=jcp(1,1)+jcp(1,2)
            nqud1=jcp(2,1)+jcp(2,2)
          elseif(iabs(idptl(ip))/10.eq.122)then !proj=neutron
        ...
        elseif(iclpro.eq.1)then
          if(iabs(idptl(ip))/10.eq.12)then
            nquu1=jcp(1,1)+jcp(1,2)
            nqud1=jcp(2,1)+jcp(2,2)
          else    !to avoid flavor problem with exotic projectile (but should not happen (only gg)
            nquu1=0
        ...
        elseif(iclpro.eq.3)then
          if(iabs(idptl(ip))/10.eq.13)then !proj=Kch
            nquu1=jcp(1,1)+jcp(1,2)
            nqud1=jcp(3,1)+jcp(3,2)
          elseif(iabs(idptl(ip))/10.eq.23)then  !proj=K0
            nquu1=jcp(2,1)+jcp(2,2)
            nqud1=jcp(3,1)+jcp(3,2)
          else    !to avoid flavor problem with exotic projectile (but should not happen (only gg)
            nquu1=0
          ...
          if(iabs(idptl(ip))/10.eq.14)then
            nquu1=jcp(1,1)+jcp(1,2)
            nqud1=jcp(4,1)+jcp(4,2)
          elseif(iabs(idptl(ip))/10.eq.24)then
            nquu1=jcp(2,1)+jcp(2,2)
            nqud1=jcp(4,1)+jcp(4,2)
          elseif(iabs(idptl(ip))/10.eq.34)then
...
c       target
        if(icltar.eq.2)then
          if(iabs(idptl(maproj+it))/10.eq.112)then !proj=proton
            nquu2=jct(1,1)+jct(1,2)
            nqud2=jct(2,1)+jct(2,2)
          elseif(iabs(idptl(maproj+it))/10.eq.122)then !proj=neutron
        ...
        elseif(iclpro.eq.1)then
          if(iabs(idptl(maproj+it))/10.eq.12)then
            nquu2=jct(1,1)+jct(1,2)
            nqud2=jct(2,1)+jct(2,2)
          else    !to avoid flavor problem with exotic projectile (but should not happen (only gg)
            nquu2=0
        ...
        elseif(iclpro.eq.3)then
          if(iabs(idptl(maproj+it))/10.eq.13)then !proj=Kch
            nquu2=jct(1,1)+jct(1,2)
            nqud2=jct(3,1)+jct(3,2)
          elseif(iabs(idptl(maproj+it))/10.eq.23)then  !proj=K0
            nquu2=jct(2,1)+jct(2,2)
            nqud2=jct(3,1)+jct(3,2)
          else    !to avoid flavor problem with exotic projectile (but should not happen (only gg)
            nquu2=0
          ...
          if(iabs(idptl(maproj+it))/10.eq.14)then
            nquu2=jct(1,1)+jct(1,2)
            nqud2=jct(4,1)+jct(4,2)
          elseif(iabs(idptl(maproj+it))/10.eq.24)then
            nquu2=jct(2,1)+jct(2,2)
            nqud2=jct(4,1)+jct(4,2)
          elseif(iabs(idptl(maproj+it))/10.eq.34)then
            nquu2=jct(3,1)+jct(3,2)
            nqud2=jct(4,1)+jct(4,2)
          else
            nquu2=jct(4,1)
            nqud2=jct(4,2)
     ...
     *                   /(1.d0-dsqrt(1.d0-xpmax0)))
        ...
        if(iremn.ge.2)then
          if(nquu1.gt.nqud1.or.iclpro.ne.2)then
            uv1=psdfh4(sngl(zp0*xp1),q2mini,0.,iclpro,1)
            dv1=psdfh4(sngl(zp0*xp1),q2mini,0.,iclpro,2)
          else         !if nquu<nqud => no u or no d
            uv1=psdfh4(sngl(zp0*xp1),q2mini,0.,1,1)
            dv1=uv1
          endif
          if(nquu1.eq.0)uv1=0d0
          if(nqud1.eq.0)dv1=0d0
          if(nquu2.gt.nqud2.or.icltar.ne.2)then
            uv2=psdfh4(sngl(zp0*xp2),q2mini,0.,icltar,1)
            dv2=psdfh4(sngl(zp0*xp2),q2mini,0.,icltar,2)
          else                  !if nquu<nqud => no u or no d
            uv2=psdfh4(sngl(zp0*xp2),q2mini,0.,1,1)
            dv2=uv2
          endif
        ...
        wwuu=uv1*uv2*sqq
        if(iclpro.eq.2.and.icltar.eq.2)then
          wwdd=dv1*dv2*sqq
        elseif(iclpro.eq.1.or.icltar.eq.1)then
          wwdd=dv1*dv2*sqaq
        elseif(iclpro.eq.3.or.icltar.eq.3)then
          wwdd=dv1*dv2*sqqp
        elseif(iclpro.eq.4.or.icltar.eq.4)then
          wwuu=uv1*uv2*sqqp
          wwdd=0.
        endif
        wwud=uv1*dv2*sqqp
...
3       zpm=(xmin1+drangen(gb0)*(xmax1-xmin1))**dble(1./(delh+.4))   !zpm
     ...
     *             /(1.d0-dsqrt(1.d0-dsqrt(zpm))))
        ...
        if(iremn.ge.2)then
          if(nquu1.gt.nqud1.or.iclpro.ne.2)then
            uv1=psdfh4(sngl(xp*xp1),q2mini,0.,iclpro,1)
            dv1=psdfh4(sngl(xp*xp1),q2mini,0.,iclpro,2)
          else         !if nquu<nqud => no u or no d
            uv1=psdfh4(sngl(xp*xp1),q2mini,0.,1,1)
            dv1=uv1
          endif
          if(nquu1.eq.0)uv1=0d0
          if(nqud1.eq.0)dv1=0d0
          if(nquu2.gt.nqud2.or.icltar.ne.2)then
            uv2=psdfh4(sngl(xm*xp2),q2mini,0.,icltar,1)
            dv2=psdfh4(sngl(xm*xp2),q2mini,0.,icltar,2)
          else                  !if nquu<nqud => no u or no d
            uv2=psdfh4(sngl(xm*xp2),q2mini,0.,1,1)
            dv2=uv2
          endif
          if(nquu2.eq.0)uv2=0d0
          if(nqud2.eq.0)dv2=0d0
        ...
        if(wudt.lt.1d-16)then
          ...
          if(ish.ge.1)write(ifch,*)'No more valence quark for psahot !'
...
c          endif
      ...
      !-------------------------------------
       ...
       if(ish.ge.4)write(ifch,*)'sea-sea  val-sea  sea-val'
        ...
        if(iqq.eq.0)then    !rejection function normalization
          ...
          if(iqq.eq.1)then
            uv1=psdfh4(sngl(xpmin0*xp1),q2mini,0.,iclpro,1)
     *        *sngl(1.d0-xpmin0*xp1)**(-1.-alpq-alplea(iclpro))
            dv1=psdfh4(sngl(xpmin0*xp1),q2mini,0.,iclpro,2)
     *        *sngl(1.d0-xpmin0*xp1)**(-1.-alpq-alplea(iclpro))
          else
            uv1=psdfh4(sngl(xpmin0*xp2),q2mini,0.,icltar,1)
     *        *sngl(1.d0-xpmin0*xp2)**(-1.-alpq-alplea(icltar))
            dv1=psdfh4(sngl(xpmin0*xp2),q2mini,0.,icltar,2)
     *        *sngl(1.d0-xpmin0*xp2)**(-1.-alpq-alplea(icltar))
          endif
 ...
 4      continue
     ...
     .    ,idfpomr,iqq,1d0/xmin,gbyj,iret
          iret=1
          goto 16
        endif
        zpm=(xmin1+drangen(zpm)*(xmax1-xmin1))**dble(1./(delh-dels)) !zpm
        if(iclpro.ne.4.or.iqq.ne.1)then
     ...
     *        *(r2had(iclpro)+r2had(icltar))/z
          ...
          if(drangen(gbyj).gt.gbyj)goto 4                     !rejection
          wp1=wp0-wpi
          wm1=wm0-wmi
     ...
     *      *sngl(1.d0-xp)**(0.5+alpq)
     ...
     &     'No more valence quark in projectile for psahot !'
     &                     ,ip,nquu1,nqud1
              iret=1
              goto 16
            endif
     ...
     *        (-1.-alpq-alplea(iclpro))
            ...
            if(iremn.ge.2)then
              if(nquu2.gt.nqud2.or.icltar.ne.2)then
                uv1=psdfh4(sngl(xm*xp2),q2mini,0.,icltar,1)
                dv1=psdfh4(sngl(xm*xp2),q2mini,0.,icltar,2)
              else              !if nquu<nqud => no u or no d
                uv1=psdfh4(sngl(xm*xp2),q2mini,0.,1,1)
                dv1=uv1
              endif
           ...
           if(ish.ge.1)write(ifmt,*)'No more valence quark for psahot !'
           if(ish.ge.1)write(ifch,*)
     &      'No more valence quark in target for psahot !'
     &                     ,it,nquu2,nqud2
              iret=1
              goto 16
            endif
     ...
     *      gbyj,zpm,xp,tmax,tmin,xpmax,xpmin
          if(drangen(gbyj).gt.gbyj)goto 4
...
5           t1=t1min*(t1max/t1min)**drangen(t1)
          if(ish.ge.8)write (ifch,*)'t1,t1min,t1max',t1,t1min,t1max
          xq1=1.d0/(1.d0+t1)
          if(drangen(xq1).gt.(xq1*(1.d0-xq1))**(1.+(-alpqua)))goto 5
...
c valence quark used to start space like cascade
        ...
        if(iabs(idptl(ip))/10.eq.122)iqc(1)=3-iq1      !proj=neutron
...
c valence quark used to start space like cascade
        ...
        if(iabs(idptl(maproj+it))/10.eq.122)iqc(2)=3-iq2     !targ=neutron
        if(idptl(maproj+it).lt.0)iqc(2)=-iqc(2)  !targ=antiparticle
        ifl2=iabs(iqc(2))

c check string ends for mesons : 
c switch q<->aq if needed (done randomly in ProSeTyp)
        if(iabs(idptl(maproj+it)).lt.1000)then
          if(iqc(2).gt.0.and.idm1pr(ncolp,kcol).ne.2)then
            idm2pr(ncolp,kcol)=idm1pr(ncolp,kcol)
            idm1pr(ncolp,kcol)=2
          elseif(iqc(2).lt.0.and.idm2pr(ncolp,kcol).ne.2)then
...
7         zg=1d0-drangen(xp)*(1.d0-xp)
          if(ish.ge.8)write (ifch,*)'6-zg,xp',zg,xp
          if(drangen(zg).gt.zg**dels*((1.d0-xp/zg)/(1.d0-xp))
     *                                              **betpom)goto 7
          xg=xp/zg
          wpq=wp0*(xg-xp)
          wmq=1.d0/wpq
...
8           zg=1.d0-drangen(xm)*(1.d0-xm)
          if(ish.ge.8)write (ifch,*)'7-zg,xm',zg,xm
          if(drangen(zg).gt.zg**dels*((1.d0-xm/zg)/(1.d0-xm))**betpom)
     *      goto 8
          xg=xm/zg
          wmq=wm0*(xg-xm)
          wpq=1.d0/wmq
...
c---------------------------------------------------------------
     ...
     &  'inner partons of the hard Pomeron'
      ...
      ept(3)=pxh
      ept(4)=pyh
      qmin(1)=q2mini          !effective momentum cutoff above current la
      qmin(2)=q2mini          !effective momentum cutoff below current la
...
111     sjb=psbint(qmin(1),qmin(2),qqcut,sngl(si),iqc(1),iqc(2),0) !born parton-parton
      ...
      sjb=psbint(qmin(1),qmin(2),qqcut,sngl(si)+q2mass,iqc(1),iqc(2),0)
        if(qmin(2).eq.q2mini)then
          wwmins=2.5*q2mini*(1.+sqrt(1.+
     *    4.*(pt2+q2mass)/q2mini))
...
c        if(gb0.le.0.)stop'gb0<=0'
        if(gb0.le.0.d0)then
          if(ish.ge.1)write(ifmt,*)'gb0.le.0.  si,pt2:',si,pt2
          iret=1
          goto 16
        endif
...
10    continue
     ...
     &,kcol,ncolp,nptl,gb7
      ...
      if(rangen().gt.djl)then        !lc momentum share in the cur
        x=(xmin1+drangen(x)*(xmax1-xmin1))**(1./delh)
      else
        x=1.d0-(1.d0-xmin2)*((1.d0-xmax)/(1.d0-xmin2))**rangen()
      endif
      qq=qqmin/(1.d0+drangen(qq)*(qqmin/qqmax-1.d0))
...
! parton type selection iqc -> iqnew (space like) + iq1 (time like)
      ...
      if(rangen().lt.sj1/(sj1+sj2))then
        ...
        if(iqc(jj).ne.0)then    !q -> q + g  (jt=3)
          ...
          jt=4
          jq=int(1.5+rangen())
          nbflav=3               !suppress space like contribution for charm otheriwse too large compared to data
          if(exp(1.-(q2cmin+qqcut)/qt2).gt.rangen())nbflav=naflav !qq cut-off for charm production
c          print *,q2cmin+qqcut,qt2,qq,nbflav
          iq1=int(nbflav*rangen()+1.)*(3-2*jq)   
c          iq1=int(3*rangen()+1.)*(3-2*jq)
          iqnew=-iq1
          nqc(1)=ncc(jq,jj)
        endif
...
11    ntest=ntest+1
 ...
 12   continue !------------------- Born process------------------------
   ...
   !kkkkkkkkkkkkk out Born partons without timelike cascade
      call psphwr(iptl)   !out Born partons (pprtx) to /cptl/
      if(nprtjx.eq.2)then
       do ii=1,2
...
c------------------------------------------------------------------------
      double precision ept(4),psutz,psuds,drangen,gb,gb0
     ...
     &,iorprt(ntim),jorprt(ntim),nprtj
      ...
      if(qt2.lt..999d0*qq.and.ish.ge.2)write(ifmt,*)'qt20,qq'
     .                                              ,qt2,qq
...
14    q2=tmin/(1.-rangen()*(1.-tmin/tmax))    !q2=min(|t|,|u|)
     ...
     &*psuds(scale1,iqc(1))*psuds(qt2,iqc(2))/gb0 !rejection function
      ...
      if(drangen(gb).gt.gb)goto 14                   !rejection

c determination of the color configuration
      nbflav=3
      if(exp(1.-(q2cmin+qq)/qt2).gt.rangen())nbflav=naflav !qq cut-off for charm production
c      print *,q2cmin+qq,qt2,qq,nbflav
      nqc(2)=0
      if(iqc(1).eq.0.and.iqc(2).eq.0)then      !g+g
        ...
        if(rangen().lt.wsub(1)/fborn)then      !gg->gg
          ...
          iqc(jm)=int(nbflav*rangen()+1)*(3-2*jq) !(anti)quark flavor
          iqc(3-jm)=-iqc(jm)
          njc1=ncc(3-jq,jm)
          njc2=ncc(jq,3-jm)
          if(iqj(njc1).ne.0)then
       ...
       elseif(iqc(1)*iqc(2).gt.0)then
        ...
        if(aks.lt.(wsub(1)+wsub(2))/fborn)then
          ...
          if(aks.gt.wsub(1)/fborn)then
            iqa=iabs(iqc(jm))
            iq=int((nbflav-1)*rangen())+1
            if(iq.eq.iqa)iq=nbflav
...
c copy of subroutine pspawr(kj)
      ...
      do ipr=1,2
        ...
        enddo
        istptl(nptl)=25
        ityptl(nptl)   = 30 + idpomr
      enddo
      qsqptl(nptl-1) = qqmx1
      qsqptl(nptl)   = qqmx2

c      !if(abs(idptl(nptl)).eq.4.or.abs(idptl(nptl-1)).eq.4)
c      !.print*,'25partonsCharm IB OB:',mIB,nIB,mOB,nOB,'    '
c      !.,ityptl(nptl-1),ityptl(nptl)
c      pt=min(pprtx(1,1)**2+pprtx(2,1)**2,pprtx(1,2)**2+pprtx(2,2)**2)
c      ! phi=polar(pprtx(1,2),pprtx(2,2))-polar(pprtx(1,1),pprtx(2,1))
c      ! phi=abs(phi)
c      ! if(phi.gt.pi)phi=2*pi-phi
c      ! write(ifmt,'(2(a,2i5,4f10.2,f14.2,i7/))')
c      ! . 'psphwr: 25_1 = ',nptl-1,idprtx(1),(pprtx(k,1),k=1,4),phi/pi,jj,
c      ! . '        25_2 = ',nptl  ,idprtx(2),(pprtx(k,2),k=1,4),phi/pi,jj
c      call charmwrite(iqq)
      end
