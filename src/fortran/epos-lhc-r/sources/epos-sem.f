...
c-----------------------------------------------------------------------
      function ffsigm(t,qt,x1,x2)    !former psjy
c-----------------------------------------------------------------------
      include 'epos.incsem'
      include 'epos.inc'
...
c-----------------------------------------------------------------------
     ...
     &                                           +tgss(ig1,i1)*(m1-1.5))
          ...
          ft=ffsigm(t,qq,sngl(xx1),sngl(xx2))
...
c-------------------------------------------------------------------------------
      ...
      if(maproj.eq.1.and.matarg.eq.1)then
        ...
        do 11 j=1,matarg
          ...
          phik3(koll)=0.
          thetak3(koll)=0.
          if(iscreen.ne.0.and.bij.le.bkmxndif.and.zbrads.gt.0.)then
c            if(zbrmax.gt.0..and.bij.gt.zbcut+zbrmax*rangen())goto 11
c            if(zbrmax.gt.0..and. rangen().ge.zbcutx/bij*zbrmax)goto 11
            lproj3(i)=lproj3(i)+1
            ltarg3(j)=ltarg3(j)+1
...
c define angle for anti-shadowing
 ...
 12     continue
      ...
      coord(4,kl)=dist/vel
      enddo
      if(iscreen.ne.0.and.zbrads.gt.0.)call CalcScrPair(b)
...
c of nuclear shadowing
      ...
      bglx=0. !zbrads*sqrt(max(0.1,satrad))
      fzbrmax=1.
c      if(zbrmax.gt.0)fzbrmax=zbrmax
      fzbcut=1.
      if(zbcut.gt.0)fzbcut=zbcut*sqrt(max(0.1,satrad)) !bglx
      fzbrads=1.
      if(bglx.gt.0.05)fzbrads=bglx
...
c------------------------------------------------------------------------
     ...
     *,yqmax1,drangen
      ...
      if(max(yqm(3),yqm(4)).lt.1d-10)then
        iret=1
        return
      endif
...
c      write (*,*)'xlc',xlp(1),xlp(2),xlm(3),xlm(4)
      if(drangen(x34).gt.(xlp(1)*xlp(2)*xlm(3)*xlm(4))**(-alpqua)*
...
c
c-----------------------------------------------------------------------
c      function pssalf(qq)
c-----------------------------------------------------------------------
c pssalf - effective qcd coupling (alpha_s/2/pi)
c-----------------------------------------------------------------------
c      include "epos.incsem"
c      pssalf=2./(11.-naflav/1.5)/log(qq)
c      return
c      end
...
c number of flavors in fragmentation not less than active flavor in hard string 
      ...
      if(nrflav.gt.3)then
        ...
        difucc=(pudc*pudc*pmqq+pmqc+pmqc)**2-pmqu2
      else
        difuc=0.
        difuuc=0.
        difudc=0.
...
c   omega coeffs
c    ----------------
c      coefom0=utgam1(1.+dels-alppar)*utgam1(1.+alplea(iclpro))
c     */utgam1(2.+alplea(iclpro)+dels-alppar)
c     **utgam1(1.+dels-alppar)*utgam1(1.+alplea(icltar))
c     */utgam1(2.+alplea(icltar)+dels-alppar)
c      coefom1=1.-utgam1(1.+dels-alppar)**2*utgam1(1.+alplea(iclpro))
c     */utgam1(1.+alplea(iclpro)+2.*(1.+dels-alppar))
c     **utgam1(1.+dels-alppar)**2*utgam1(1.+alplea(icltar))
c     */utgam1(1.+alplea(icltar)+2.*(1.+dels-alppar))/coefom0**2
c      coefom2=3.*coefom1-1.
c     *+utgam1(1.+dels-alppar)**3*utgam1(1.+alplea(iclpro))
c     */utgam1(1.+alplea(iclpro)+3.*(1.+dels-alppar))
c     **utgam1(1.+dels-alppar)**3*utgam1(1.+alplea(icltar))
c     */utgam1(1.+alplea(icltar)+3.*(1.+dels-alppar))/coefom0**3
c      if(ish.ge.4)write(ifch,*)'coefom',coefom0,coefom1,coefom2,delx
...
c        if(epscrw0.ne.epscrw)write(textini,'(a,2f8.4)')
c     *  'inirj: wrong epscrw    ',epscrw0,epscrw
c        if(epscrp0.ne.epscrp)write(textini,'(a,2f8.4)')
c     *  'inirj: wrong epscrp   ',epscrp0,epscrp
...
c     *   .or.egyscr0.ne.egyscr.or.epscrw0.ne.epscrw.or.epscrp0.ne.epscrp
     *   .or.isetcs0.lt.isetcs)then
           write(ifmt,'(//a//)')'   inics has to be reinitialized!!!!'
           stop
        endif
        if(isetcs.eq.2)then
