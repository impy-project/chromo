...
c-----------------------------------------------------------------------
      ...
      include 'epos.incsem'
      include 'epos.incpar'
c      parameter (eta=0.35,etap=0.15)
...
c.....search string in pptl-list...................................
      ...
      nk1=maproj+matarg+1
 2    continue
c      if(nclean.gt.0.and.nptl.gt.mxptl/2)then
c        nnnpt=0
c        do iii=maproj+matarg+1,nptl
c          go=.true.
c          if(nclean.eq.1.and.istptl(iii).le.istmax)go=.false.
c          if(go.and.mod(istptl(iii),10).ne.0)then
c            istptl(iii)=99
c            nnnpt=nnnpt+1
c          endif
c        enddo
c        if(nnnpt.gt.mxptl-nptl)then
c          nptl0=nptl
c          call utclea(maproj+matarg+1,nptl0)
c          nkmax=nptl
c          nk1=maproj+matarg+1
cc          write(ifch,'(a)')' ---------after utclea------------'
cc          call blist('&',nk1,nkmax)
c        endif
c      endif
...
c      if(nk2-nk1.gt.1.or.iappl.eq.6)kky=1
      ...
      krm=0
      if(itp.ge.40.and.itp.le.59)krm=1
      if((itp.ge.40.and.itp.le.49.and.abs(idproj).lt.129)
     ..or.(itp.ge.50.and.itp.le.59.and.abs(idtarg).lt.129))krm=-1
      kdrm=0
      if(itp.eq.43.or.itp.eq.53)kdrm=1
      if((itp.eq.43.and.abs(idproj).lt.1000)
     ..or.(itp.eq.53.and.abs(idtarg).lt.1000))kdrm=-1
...
c fit is important not only in e+e- but for all particle production
          ...
          pb=pb0+(1.-exp(-x/20.))*(pbreakg-pb0)
          fcharm=0.8        !change x distribution of charm in e+e-
          if(icharm.eq.1)pb=pb*fcharm
...
c      write(*,*)'pb',kky,krm,kdrm,x,pb,zpaptl(1,nk1)+zpaptl(1,nk2)
      ...
      zzzz=1.
c      if(kky.eq.1)zzzz=zzzz+zoeinc*log(1.+zpaptl(2,nk2))
c      print *,zpaptl(2,nk2),zzzz
c      if(isplit.eq.1.and.iappl.ne.6)then       !for pt
c        zzzz=0.
c        zzz=0.
cc        if(krm.eq.0)then  !both hard and soft (not good for mesons for HERA-B or pp RHIC, but good for baryons)
c        if(kky.eq.1)then   !only hard
cc        if((kky.eq.1.or.(iLHC.eq.1.and.krm.eq.0)))then   
cc          zzzz=zoeinc*(zpaptl(1,nk1)+zpaptl(1,nk2))
cc          zzzz=sqrt(1.+zzzz)
c          zz=(zpaptl(1,nk1)*(1.+zodinc*log(zpaptl(2,nk2)))     !zpaptl(2,nk1)=# of connected pom
c     &       +zpaptl(1,nk2)*(1.+zodinc*log(zpaptl(2,nk2))))
cc          print *,zpaptl(2,nk1),zpaptl(2,nk2)
cc     &           ,1./cosh(0.007*max(0.,zpaptl(2,nk1)-50.))
cc reduce zz before fusion (make it proportionnal to b and E because at low energy not so many fragments are used and then if only one fragment is used all others particles are fragmented without particular effect 
c          if(iclu.eq.1)zz=zz/cosh(min(30.,
c     &                            0.0085*max(0.,zpaptl(2,nk1)-30.)))   
c          zzzz=zoeinc*zz
c          if(iLHC.eq.1)then
c            zzz=min(fkamax,fkainc*zz)
c            zzz=sqrt(1.+zzz)
c            fkap(1)=fkap(1)*zzz !fix increase of ap and K at Tevatron
c            fkap(2)=fkap(2)*zzz 
c            fkap(3)=fkap(3)*zzz
c          endif
cc          print *,bimevt,zzzz,zz,0.75*(zpaptl(1,nk1)+zpaptl(1,nk2))
cc          zzz=fkainc*zz
c        elseif(kzrm.eq.1)then
c          zz=zpaptl(1,nk1)
c          zzzz=0. !zopinc*zz
c          zzz=0.!zodinc*zz
c          zzz=sqrt(1.+zzz)
...
c      endif
     ...
     &   call utstop("fkappa<=0. not allowed !&")
      ...
      if(nrflav.gt.3.and.pudc.gt.0.)then
...
c...................light string....................................
     ...
     &         call utstop('gakfra (light): mxptl too small ...&')
          if(iadj.eq.1)p1(5)=am !otherwise it will be rejected at decay
          p1(4)=sqrt(p1(3)**2+p1(2)**2+p1(1)**2+p1(5)**2)   !put particle on-shell
          do i=1,5
            pptl(i,nptl)=p1(i)
          enddo
          do i=nk1,nk2
          ...
          if(abs(p1(5)-am).gt.0.01.and.ish.ge.1)then
            write (ifch,*) 'light string  ---  particle off-shell !!!!'
            write (ifch,*) idr,'  mass:',p1(5),' put to:',am
          endif
c          pptl(4,nptl)=sqrt(pptl(1,nptl)**2+pptl(2,nptl)**2    !done as ghost
...
c.....if string-segment on left side, check for resonance...........
      rud=eta+etap !pmqu/(pmqu+pmqd)
c      amlxx=aml
c      call idress(idl,aml,idrl,iadjl)
      if(ip(n)  .eq.0) then
...
c      amrxx=amr
c      call idress(idr,amr,idrr,iadjr)
      if(ip(n+1).eq.0) then
...
c        idptl(nptl)=ip(i)
         am=pptl(5,nptl)
         call idfix(am,ip(i),idptl(nptl),nbr)
         if(ish.ge.5.and.idptl(nptl).ne.ip(i))
     &   write(ifch,*)'idfix ',ip(i),idptl(nptl),am
...
c..... fast charm ...............................
      ...
      if(icharm.eq.1)then
        if(nbr.le.2.and.p1(5).gt.4.*qmass(4).and.nsde.lt.10000)then
          nptl=nptlini
          goto 3
        endif
      endif

c reverse order of strings for diffractive remnant to avoid forward strange baryon
c      if(abs(kdrm).eq.1.and.abs(idptl(nptlini+1)).ge.1230
c     &       .and.pptl(4,nptlini+1)/p1(5).gt.strcut)then
c        nptl=nptlini
c        do i=1,nbr+1
c          nptl=nptl+1
c          do k=1,3
c            pptl(k,nptl)=-pptl(k,nptl)
c          enddo
c        enddo
c      endif
c      nptl=nptlini

c      print*,kdrm,krm,idptl(nptlini+1),p1(5),nbr,pptl(3,iorrem)
      if(abs(kdrm).eq.1.and.strcut.gt.0..and.iorrem.gt.0)then !full rotation of string needed for forward D mesons and improve all forward spectra at low energy but not when energy becomes too large because it can create high pt particles !
        fac=sqrt(min(1.,amtot/abs(pptl(3,iorrem))/nbr))
     .     *exp(1.-max(1.,pptl(4,iorrem)/strcut)**2)
        if(fac.gt.0.05)then
c      print*,kdrm,krm,amtot,p1(5),nbr,pptl(3,iorrem),fac
          phi=2.*pi*rangen()
          theta=fac*pi*rangen()
          nptl=nptlini
          do i=1,nbr+1
           nptl=nptl+1
           call utrotb(phi,theta,pptl(3,nptl),pptl(2,nptl),pptl(1,nptl))
          enddo
        endif
      endif
      nptl=nptlini
...
c update origin
      ...
      tivptl(1,n8ptl)=xorptl(4,n8ptl)
      tivptl(2,n8ptl)=tivptl(1,n8ptl)

      do i=1,nbr+1
        nptl=nptl+1
        
        p1(4)=sqrt(p1(3)**2+p1(2)**2+p1(1)**2+p1(5)**2) !put particle on-shell first
...
c         write(ifch,*),'*****fra*****',nptl,xori(4)
c     .    ,pptl(4,nptl)/pptl(5,nptl),tauran
c     .    ,xorptl(1,nptl),xorptl(2,nptl),xorptl(3,nptl),xorptl(4,nptl)
        ityptl(nptl)=ityptl(nk1)
        itsptl(nptl)=itsptl(nk1)
        qsqptl(nptl)=qsqptl(n8ptl)
        zpaptl(1,nptl)=zpaptl(1,n8ptl)
...
c in non central strings (baryon spectra in pion collisions or NA49 data)
        if(abs(kdrm).eq.1
     .     .and.ib1.le.1.and.abs(psg(3)/psg(4)).gt.diqcut)then
          pduu=0.
          pdud=0.
          pdus=0.
          pduc=0.
...
c      print *,krm,kdrm,kky,zzzz,ps/(pu+pd+ps),pdiqu/psum
      ...
      if(rdiqu.gt.0.)then
        ...
        elseif(r.gt.pdcc+pdsc+pddc+pduc.and.pdss.gt.0.)then !bg
          ifl1=3
          ifl2=3
        elseif(r.gt.pdcc+pdsc+pddc.and.pduc.gt.0.)then !bg
          if(rangen().gt.0.5)then
            ifl1=1
            ifl2=4
          else
        ...
        elseif(r.gt.pdcc+pdsc.and.pddc.gt.0.)then !bg
          if(rangen().gt.0.5)then
            ifl1=2
            ifl2=4
          else
        ...
        elseif(r.gt.pdcc.and.pdsc.gt.0.)then !bg
          if(rangen().gt.0.5)then
            ifl1=3
            ifl2=4
          else
        ...
        elseif(pdcc.gt.0.)then !bg
          ifl1=4
          ifl2=4
        else
          ifl1=min(3,1+int(rangen()*3))
          ifl2=min(3,1+int(rangen()*3))
          write(ifmt,'(a,2i3,8e9.2)')'WARNING gakfra diquark flavor'
     .    ,ifl1,ifl2,r,pdiqua
     .    ,pdcc+pdsc+pddc+pduc
     .    ,pdcc,pdsc,pddc,pduc
        endif
      elseif(r.gt.pc+pd+ps.and.pu.gt.0.)then !bg
        ifl1=1
      elseif(r.gt.pc+ps.and.pd.gt.0.)then !bg
        ifl1=2
      elseif(r.gt.pc.and.ps.gt.0.)then !bg
        ifl1=3
      elseif(pc.gt.0.)then !bg
        ifl1=4
      else
...
c      if(abs(ifl).eq.4)print*,pc,nrflav
...
c      if(iLHC.eq.1)then
        pttra=0. !(ptfra+delptfra)*(zzzz-1.)*fnsbp
c        if(ig.ge.0)then         !quark and soft strings
c        if(ig.eq.1)then         !quark and soft strings
c        if(krm.eq.0)then         !quark and soft strings
          pt=ranpt()*(ptfra +  pttra)!/float(jqu)
c        else
c          pt=ranpt()*(ptfraqq   +  pttra)!/float(jqu)
c        endif
c      else
c        pttra=(ptfra+delptfra)*(zzzz-1.)*fnsbp
c        if(jqu.eq.1)then
c          pt=ranpt()*(ptfra                +  pttra)
c        else
c          pt=ranpt()*(ptfraqq              +  pttra)
c          pt=pt*0.5
c        endif
c      endif
...
c      goto 25
 ...
 9999 if(nbr.eq.nbrold)iok=1
      if(ish.ge.5)write(ifch,*) 'iok=',iok
...
c      db5=sqrt( db4**2-rgampr(4)**2)
     ...
     &,'id','ist','ity'
      do i=n1,n2
         ...
         chrg=0.
         if(iabs(idptl(i)).le.9999)call idchrg(6,idptl(i),chrg)
...
c---------------------------------------------------------------------
     ...
     &     ,ptb(4,0:mxnbr),iflb(0:mxnbr),ip(0:mxnbr),co,x,y
      dimension co(12)
      imix=0
c      if(kdrm.eq.1)imix=0    !for air shower, better to apply it for all remnants
c      if(krm.eq.-1.and.nbr.le.1)imix=-1   !pion remnants only ... for NA61 and NA22
      amb=am
      call idres(id,am,idr,iadj,imix)
c      if(id/10.eq.22)write (ifmt,*) ' idress  ',id,amb,imix,idr,am
      if(idr.eq.0)then
        return
      endif
...
c---------------------------------------------------------------------
      subroutine idfix(am,id,idr,nbr) !for id adjustment
c---------------------------------------------------------------------
      include 'epos.inc'
      common/czz/kky,krm,kdrm,kzrm,pui(3),pdi(3),psi(3),pci(3),zzzz,itp
     &,pduui(3),pdudi(3),pdusi(3),pduci(3),pdddi(3),pddsi(3),pddci(3)
     &,pdssi(3),pdsci(3),pdcci(3),kkc
      imix=1
      if(krm.eq.-1.and.nbr.le.1)imix=0
      idi=id
      ami=am
      id0=id/10
