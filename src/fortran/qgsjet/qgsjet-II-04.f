...
c calculate nuclear screening factors for "middle point" -> eikonals
     ...
     *  ,ip,it)
        ...
        npomtg(nbpom)=0
        do i=1,n
         if(psran(b10).lt.vv1p/vv.or.scm.le.sgap**2)then  !single Pomeron
...
c   diffraction (hadron-hadron case)
      if(ia(1).eq.1.and.ia(2).eq.1.and.iwp(1).eq.0.and.iwt(1).eq.0)then
       ...
       enddo
       enddo
       if(psran(b10).le.wel**2/winel)then
     ...
     * *(wabs(1,1)+wabs(2,2)-wabs(1,2)-wabs(2,1))**2
       aks=(wdifp+wdift+wdifd)*psran(b10)
...
c   diffraction (hadron-nucleus & nucleus-nucleus)
     ...
     *   /(cc(1,icz)*wdp(1,ip)**2+cc(2,icz)*wdp(2,ip)**2)
         if(psran(b10).lt.wdifr)iwp(ip)=3                     !LMD excitation
        endif
...
5          continue
     ...
     *   /(cc(1,2)*wdt(1,it)**2+cc(2,2)*wdt(2,it)**2)
         if(psran(b10).lt.wdifr)iwt(it)=3
        endif
...
1     continue
      xpomr=(scm/sgap**2)**(-psran(b10))/sgap   !proposed LC momentum for 3P-vertex
      rp1=(rq(iddp(ip),icz)-alfp*dlog(xpomr))*4.d0*.0389d0
      rp2=(rq(iddt(it),2)+alfp*dlog(xpomr*scm))*4.d0*.0389d0
      rp=rp1*rp2/(rp1+rp2)
      z=psran(b10)
...
3      ntry=ntry+1
       ...
       if(ip.eq.ia(1).or.ntry.gt.100)then
        ...
        do i=1,nppr0
         if(psran(b10).le.vpac0(ip)/vpac(ip).or.xpomr*sgap**2.ge.1.d0)
     *   then
          itypr0(i)=0
         else
          npprh0=npprh0+1
        ...
        do ipp=ip+1,ia(1)
         ...
         if(ninc.ne.0)then
          ...
          do i=nppr0-ninc+1,nppr0
           if(psran(b10).le.vpac0(ipp)/vpac(ipp)
     ...
     *  .or.nppr0.eq.npprh0+1.and.psran(b10).gt.1.d0+wh*wh0)goto 3
       endif
      endif
...
4      ntry=ntry+1
       ...
       if(it.eq.ia(2).or.ntry.gt.100)then
        ...
        do i=1,nptg0
         if(psran(b10).le.vtac0(it)/vtac(it).or.xpomr*scm.le.sgap**2)
     *   then
          itytg0(i)=0
         else
          nptgh0=nptgh0+1
        ...
        do itt=it+1,ia(2)
         ...
         if(ninc.ne.0)then
          ...
          do i=nptg0-ninc+1,nptg0
           if(psran(b10).le.vtac0(itt)/vtac(itt)
     ...
     *  .or.nptg0.eq.nptgh0+1.and.psran(b10).gt.1.d0+wh*wh0)goto 4
       endif
      endif
...
c     *  .and.psran(b10).lt.max(0.d0,wzgp/(vv(1)+vv(8))))nppr0=0
c      if((jt.eq.1.or.jt.eq.7)
c     *  .and.psran(b10).lt.max(0.d0,wzgt/(vv(1)+vv(7))))nptg0=0
     ...
     * -(vpac(ip)+vplc-vpac0(ip)-vplc0)*(1.d0-factor)-wng)
       if(psran(b10).ge.wgap/(wgap+wng).or.xpomr*sgap**2.gt..9d0)then
        if(psran(b10).lt.vpacng/(vpacng+vplcng)
     ...
     *         -(vpac(ip)-vpac0(ip))*(1.d0-factor))
        ...
        if(psran(b10).lt.wfg/(wfg+wlg))then
     ...
     * -(vtac(it)+vtlc-vtac0(it)-vtlc0)*(1.d0-factor)-wng)
       if(psran(b10).ge.wgap/(wgap+wng)
     * .or.xpomr*scm.lt.1.1d0*sgap**2)then
        if(psran(b10).lt.vtacng/(vtacng+vtlcng)
...
8      i=i+1
     ...
     * *(vvxs-vvx0s+vvxp0l*vvx0s))
       ...
       do in=1,nzzp
        nrej=0
32      xpomri=(xpomr*sgap**2)**psran(b10)/sgap
     ...
     *  ,xxi,yyi,int(1.5d0+psran(b10)))   !coordinates for the vertex
     ...
     *   +(vimpd+vi1pd)*vguu*(1.d0-vvxt0)*vvxp0
         ...
         nrej=0
34       xpomz=xpomr*sgap*(xpomri/xpomr/sgap**2)**psran(b10)
     ...
     *   ,int(1.5d0+psran(b10)))               !coordinates for the vertex
     ...
     *   +(vimpd+vi1pd)*vguu*(1.d0-vvxt0)*vvxp0
         ...
         nrej=nrej+1
         if(psran(b10).gt.gbz.and.nrej.lt.10000)goto 34

         aks=vvt*psran(b10)
         if(aks.gt.vvcc*(1.d0-vvxp0)*(1.d0-vvxt0)
     *   +(vimpu+vi1pu)*vgud*(1.d0-vvxp0)*vvxt0)then
          jtu=0
          if(psran(b10).lt.vimpd/(vimpd+vi1pd))then
           jtd=2
          else
           jtd=1
          endif
         elseif(aks.gt.vvcc*(1.d0-vvxp0)*(1.d0-vvxt0))then
          jtd=0
          if(psran(b10).lt.vimpu/(vimpu+vi1pu))then
           jtu=2
          else
           jtu=1
          endif
     ...
     * *(vvxs-vvx0s+vvxt0l*vvx0s))
       ...
       do in=1,nzzt
        nrej=0
33      xpomri=xpomr/sgap/(xpomr*scm/sgap**2)**psran(b10)
     ...
     *  *(vvxs-vvx0s+vvxt0l*vvx0s))
        ...
        gb=gb/gb0/z*rp/rp0
        nrej=nrej+1
        if(psran(b10).gt.gb.and.nrej.lt.10000)goto 33
     ...
     *   +(vimpd+vi1pd)*vguu*(1.d0-vvxt0)*vvxp0
         ...
         nrej=0
35       xpomz=xpomri*sgap*(xpomr/xpomri/sgap**2)**psran(b10)
     ...
     *   +(vimpd+vi1pd)*vguu*(1.d0-vvxt0)*vvxp0
         ...
         gbz=vvt/gbz0/z*rp/rp0    /1.4d0
         nrej=nrej+1
         if(psran(b10).gt.gbz.and.nrej.lt.10000)goto 35
     ...
     *.and.psran(b10).lt.(1.d0-exp(-vpac(ip)))*(1.d0-vvxpl)
     ...
     *.and.psran(b10).lt.(1.d0-exp(-vtac(it)))*(1.d0-vvxtl)
...
11    xpomm(m+1)=(xpomm(m)*sgap**2)**psran(b10)/sgap
      if(itypom.eq.4)xpomm(m+1)=(xpomm(m)*sgap**3)**psran(b10)/sgap**2
      ...
      bbi=(dsqrt(bpm)*rp2/(rp1+rp2)-b0*cos(phi))**2+(b0*sin(phi))**2
      call qgbdef(bbp,bbi,xa(ipp,1)+b,xa(ipp,2),xxm(m),yym(m)
     *,xxm(m+1),yym(m+1),int(1.5d0+psran(b10)))   !coordinates for the vertex
     ...
     * *(1.d0-vvx)*(1.d0-vvxt)**2*(1.d0-vvxtl)
      ...
      nrej=nrej+1
      if(psran(b10).gt.gb.and.nrej.le.1000)goto 11
...
12     ntry=ntry+1
        ...
        do ipi=ipp+1,ia(1)
         ...
         if(ninc.ne.0)then
          ...
          do i=nppm(m+1)-ninc+1,nppm(m+1)
           if(psran(b10).le.vpac0(ipi)/vpac(ipi)
     *     .or.xpomm(m+1)*sgap**2.gt..9d0)then
            itypm(i,m+1)=0
           else
            itypm(i,m+1)=1
     ...
     *   /(1.d0-vvxp)/(1.d0-vvxpl)
        ...
        if(nphm.eq.nppm(m+1).and.psran(b10).gt.gbt
     *  .or.nphm+1.eq.nppm(m+1).and.psran(b10).gt.1.d0+wh*gbt)then
         ntry=0
          goto 12
        endif
       endif
...
14     ntry=ntry+1
     ...
     *  -(vpac(ipp)-vpac0(ipp))*(1.d0-factor))
        if(psran(b10).le.wng.or.wng.lt.0.d0
     *  .or.xpomm(m+1)*sgap**2.gt..9d0)then
     ...
     *  -(vplc-vplc0)*(1.d0-factor))
        if(psran(b10).le.wng.or.wng.lt.0.d0)then
...
15     i=i+1
     ...
     *   ,vvxpi(nppr)
      ...
      if(jt.eq.2.and.psran(b10).lt.(1.d0-exp(-vpac(ipp)))*(1.d0-vvxpl)
...
23    xpomm(m+1)=xpomm(m)/sgap/(xpomm(m)*scm/sgap**2)**psran(b10)
      if(itypom.eq.4)xpomm(m+1)=xpomm(m)/sgap
     */(xpomm(m)*scm/sgap**3)**psran(b10)
     ...
     * *(1.d0-vvx)*(1.d0-vvxp)**2*(1.d0-vvxpl)
      ...
      nrej=nrej+1
      gb=gb/gb0/z*rp/rp0  /10.d0
      if(psran(b10).gt.gb.and.nrej.le.1000)goto 23
     ...
     *   .or.xpomm(m+1)*scm.lt.1.1d0*sgap**2)then
        ...
        do iti=itt+1,ia(2)
         ...
         if(ninc.ne.0)then
          ...
          do i=nppm(m+1)-ninc+1,nppm(m+1)
           if(psran(b10).le.vtac0(iti)/vtac(iti)
     *     .or.xpomm(m+1)*scm.lt.1.1d0*sgap**2)then
            itypm(i,m+1)=0
           else
            nphm=nphm+1
...
26     i=i+1
     ...
     *   ,vvxti(nptg)
      ...
      if(jt.eq.2.and.psran(b10).lt.(1.d0-exp(-vtac(itt)))*(1.d0-vvxtl)
...
c momentum conservation (correction for 3p-vertexes)
     ...
     *       /(wppr1(ip)+wppr2(ip))
        ...
        elseif(wppr2(ip).gt.wp0)then
         wpt=wp0/sgap/2.d0*4.d0**psran(b10)
         do nb=1,nbpom
          if(ias(nb).eq.ip.and.npomtg(nb)+npomin(nb).ne.0)then
           if(npomin(nb).ne.0)then
            do np=1,npomin(nb)
     ...
     *       /(wmtg1(it)+wmtg2(it))
        ...
        elseif(wmtg2(it).gt.wm0)then
         wmt=wm0/sgap/2.d0*4.d0**psran(b10)
         do nb=1,nbpom
          if(ibs(nb).eq.it.and.npompr(nb)+npomin(nb).ne.0)then
           if(npomin(nb).ne.0)then
            do np=1,npomin(nb)
...
c        if(zuk)2,2,3
        if(zuk.le.0.d0)then
         tt=rnuc(jj)/wsnuc(jj)*(psran(b10)**.3333d0-1.d0)
         goto 6
        else
...
c check if remnants are excited to low mass states
      if(jexpr.eq.-2.or.jexpr.gt.0.and.psran(b10)
...
c determine mass for projectile excited remnant
      if(jexip.eq.1)then
       if(jexpr.ne.-2)then                   !low mass excitation (dM/M^2)
        if(ddmax1.gt.ddmin1)then
         dmass1=ddmin1/(1.d0-psran(b10)*(1.d0-ddmin1/ddmax1))
        else
         dmass1=ddmin1
        endif
     ...
     *  /(.25d0*pi+atan(2.d0*(dmres(icz)-ddmin)/wdres(icz))))
        if(psran(b10).gt.wres)then           !PPR contribution
         dmass1=ddmax/(1.d0-psran(b10)*(1.d0-ddmax/ddmax1))
        else                                 !resonance contribution
         dmass1=dmres(icz)+.5d0*wdres(icz)
     *   *tan(atan(2.d0*(ddmax-dmres(icz))/wdres(icz))
     *   -psran(b10)*(atan(2.d0*(ddmax-dmres(icz))/wdres(icz))
...
c determine mass for target excited remnant
      if(jexit.eq.1)then
       ...
       if(jextg.ne.-2)then                   !low mass excitation (dM/M^2)
        if(ddmax2.gt.ddmin2)then
         dmass2=ddmin2/(1.d0-psran(b10)*(1.d0-ddmin2/ddmax2))
     ...
     *  *wdres(2)))/(.25d0*pi+atan(2.d0*(dmres(2)-ddmin)/wdres(2))))
        if(psran(b10).gt.wres)then           !PPR contribution
         dmass2=ddmax/(1.d0-psran(b10)*(1.d0-ddmax/ddmax2))
        else                                 !resonance contribution
         dmass2=dmres(2)+.5d0*wdres(2)*tan(atan(2.d0*(ddmax-dmres(2))
     *   /wdres(2))-psran(b10)*(atan(2.d0*(ddmax-dmres(2))/wdres(2))
...
1     sww=dsqrt(ww)
      ...
      j=int(2.d0*psran(b10))+1                !choose string end to start
     ...
     *  .and.sww.gt.restm+2.d0*amlam.and.psran(b10).lt.dc(1)*dc(2))then
c lambda generation
         restm=(restm+amlam)**2
         bet=ben
         ami=amlam**2
...
1      xp=.5d0*psran(b10)**2
       if(psran(b10).gt.(2.d0*(1.d0-xp))**(-.5d0))goto 1
       wp1=wpi*xp
...
2      xm=.5d0*psran(b10)**2
       if(psran(b10).gt.(2.d0*(1.d0-xm))**(-.5d0))goto 2
       wm1=wmi*xm
...
ctp       ich(nsh)=2*int(.5d0+psran(b10))-1
ctp       do i=1,4
ctp        esp(i,nsh)=ep1(i)
ctp        ep(i)=ep2(i)
ctp       enddo
...
ctp       call qgdec2(ep1,ep3,ep,dmmin(1)**2,am0**2,am0**2)
       if(psran(b10).lt..5d0)then  !rho0 + pi+/-
        ich(nsh)=-10
        ich(nsh+1)=ic/iab
ctp        ich(nsh+1)=2*int(.5d0+psran(b10))-1
...
ctp       nsh=nsh+2
      ...
      elseif(iab.eq.12.or.iab.eq.13)then       !N*
       am2=qgnrm(ep)
       if(6.d0*psran(b10).lt.1.d0)then         !delta + pi
...
ctp        call qgdec2(ep1,ep3,ep,dmmin(1)**2,am0**2,am0**2)
        if(3.d0*psran(b10).lt.1.d0)then  !rho0
         ich(nsh)=ic-10*ic/iab
         ich(nsh+1)=-10
c         ich(nsh+1)=2*int(.5d0+psran(b10))-1
c         ich(nsh+2)=-ich(nsh+1)
        ...
        if(3.d0*psran(b10).lt.1.d0)then
         ich(nsh)=0
         if(3.d0*psran(b10).lt.1.d0)then
          ich(nsh+1)=ic-10*ic/iab
          ich(nsh+2)=0
...
c       ic2=-ic/iab*int(.64d0+psran(b10))
c       ic1=3*ic/iab+ic2
