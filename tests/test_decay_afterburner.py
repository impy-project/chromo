...
class Pythia8DecayAfterburner(object):
    ...
    def __call__(self, event):
        nappend = 0
        id, status, px, py, pz, en, m = [], [], [], [], [], [], []

        for ip in range(event.npart):
            if event.status[ip] != 1 or abs(event.id[ip]) in self.stable_list:
                continue
            ...
            self.pythia.particleData.mayDecay(int(event.id[ip]), True)
            # Decay it
