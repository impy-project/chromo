...
def test_generator_cascade(projectile, target):
    """Probabilistic histogram test for Pythia8Cascade with nuclear targets."""
    kin = FixedTarget(TotalEnergy(100 * GeV), projectile, target)

    h = run_in_separate_process(run_model, im.Pythia8Cascade, kin)
    if h is None:
        pytest.skip(f"Pythia8Cascade rejected {projectile}+{target}")

    fn = Path(f"Pythia8Cascade_{projectile}_{target}_ft")
    path_ref = REFERENCE_PATH / fn.with_suffix(".pkl.gz")
