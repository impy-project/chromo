...
class DpmjetIIMCRun(MCRun):
    ...
    def init_generator(self, config):
        ...
        try:
            ...
            self.log_man.create_log('{0}_{1}_{2}'.format(k.A1, k.Z1,
                                                         self.lib.mcihad(k.p1pdg)),
                                    '{0}_{1}'.format(k.A2, k.Z2), k.ecm)
        except ImportError:
        ...
        try:
            if self.init_done:
                raise Exception(self.class_name + 
                                "::init_generator(): Init already performed")
        except:
            pass
...
#             try:
            [hist_d[pid].fill_event(event) for pid in np.unique(event.p_ids) 
                                            if pid in avail_pid]
#             except KeyError:
