...
standard_particles = [11, 13, 15, 211, 321, 2212, 2112, 3122, 
                      411, 421, 431]
...
class EventKinematics():
    def __init__(self, ecm=None, plab=None,
                 p1pdg=None, p2pdg=None, beam=None,
                 nuc1_prop=None, nuc2_prop=None,
                 pdata=None):
        ...
        masses = {2212: 0.93827, 2112: 0.93957, 211: 0.13957, 
                  321: 0.49368, 3122: 1.11568, 22: 0.}
        ...
        if ecm and not (plab or beam):
            ...
            self.plab = self.e2p(self.elab, pmass1)
            print ('EventKinematics: beam, ecms, elab, plab', self.beam, self.ecm,
                self.elab, self.plab)
        else:
            raise Exception(self.__class__.__name__ + 
        ...
        s = self.ecm ** 2
        self.s = s
        self.pcm = np.sqrt(s ** 2  
                           - 2 * (s * (pmass1 + pmass2) + pmass1 * pmass2)
    ...
    def __hash__(self):
        return hash('_'.join(['{0}_{1}'.format(key, self.__dict__[key])
...
class MCRun():
    ...
    def __init__(self, libref, label, n_events, DEBUG,
                 event_class, event_config, event_kinematics,
        ...
        if 'stable' in self.event_config.keys():
            self.nondef_stable = np.sum(np.abs(np.array(self.event_config['stable'])))
    ...
    def trigger_event(self, event):
        return [trigger for trigger in self.triggers.itervalues()
    ...
    def _init_progress_bar(self, maxval=None):
        ...
        self.progressBar = ProgressBar(maxval=maxval,
                                       widgets=[Percentage(), ' ',
                                                Bar(), ' ',
                                                ETA()])
    ...
    def start_fixed(self):
        ...
        if not len(self.triggers):
            raise Exception('No trigger defined in ' + 
                            self.class_name + "!")
        ...
        for i in xrange(self.n_events):  # @UnusedVariable
            ...
            active_histograms = [hist for trigger in fired_triggers
        ...
        try:
            ...
            pass
        print (r"...completed {0:3.2f}\% of the events have been rejected.").format(
                100. * float(rej_counter) / float(self.n_events))
    ...
    def start_variable(self):
        ...
        for evkin in self.evkin.e_range:
            ...
            for i in xrange(self.n_events):  # @UnusedVariable
                ...
                active_histograms = [hist for trigger in fired_triggers
                                            for hist in trigger.histogram_list.values()]
                [hist.fill_event(event, evkin.ecm) for hist in active_histograms]
